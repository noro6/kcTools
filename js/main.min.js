...
f.value,proficiency_Changed($('.init_prof[data-typeid="'+e.id+'"').find('.prof_option[data-prof="'+e.prof+'"]').parent(),!0);if(setting.contentsOrder.length){b=$("#main_contents");e=$("#side_index");var n=[],r=[];$(".contents").each(function(a,b){n.push($(b))});b.empty();e.find("li").each(function(a,b){r.push($(b))});e.empty();f={};g=$jscomp.makeIterator(setting.contentsOrder);for(h=g.next();!h.done;f={$jscomp$loop$prop$id$126$274:f.$jscomp$loop$prop$id$126$274},h=g.next())f.$jscomp$loop$prop$id$126$274=
h.value,h=n.find(function(a){return function(b){return b.attr("id")===a.$jscomp$loop$prop$id$126$274}}(f)),b.append(h),h=r.find(function(a){return function(b){return b.attr("id")==="li_"+a.$jscomp$loop$prop$id$126$274}}(f)),e.append(h)}loadMainPreset();setting.simulateCount?$("#calculate_count").val(setting.simulateCount):$("#calculate_count").val(5E3);setting.initialProf120&&(initialProf120Plane=setting.initialProf120);b=$jscomp.makeIterator(initialProf120Plane);for(f=b.next();!f.done;f=b.next())$("#prof120_"+
f.value).prop("checked",!0);setting.displayMode?Object.keys(setting.displayMode).forEach(function(a){$("#"+a+' .toggle_display_type[data-mode="'+setting.displayMode[a]+'"]').addClass("selected")}):$('.toggle_display_type[data-mode="single"]').addClass("selected");setting.orderBy||(setting.orderBy={modal_plane_select:["default","desc"]});Object.keys(setting.orderBy).forEach(function(a){$("#"+a+" .sort_select").val(setting.orderBy[a][0]);$("#"+a+" .order_"+setting.orderBy[a][1]).prop("checked",!0)});
...
"font_size_12 font_color_777 align-self-center",w.textContent=PLANE_TYPE.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$plane$278.type}}(q)).name,x=document.createElement("div"),x.className="flex-grow-1 border-bottom align-self-center mx-2",m.appendChild(w),m.appendChild(x),e.appendChild(m));y.appendChild(p);u.appendChild(y);u.appendChild(A);(0<r||h&&k)&&u.appendChild(B);"single"===f&&(u.appendChild(z),u.appendChild(t));e.appendChild(u);m=q.$jscomp$loop$prop$plane$278.type}d.innerHTML=
...
n+" slot"+v)[0];var w=v.getElementsByClassName("info_plane")[0],x=(t.abbr?t.abbr:t.name)+(0!==t.remodel?"\u2605"+t.remodel:"");w.classList="info_plane "+getPlaneCss(t.type).replace("css","shoot_table");w.textContent=x?x+" "+getProfString(t.level):"-";w=v.getElementsByClassName("info_slot")[0];var u=castInt(w.textContent);drawChangeValue(w,u,t.slot);x||(w.textContent="");w=v.getElementsByClassName("info_ap")[0];u=castInt(w.textContent);drawChangeValue(w,u,t.ap);x||(w.textContent="");c+=0===t.id?0:
...
castInt(f.dataset.shipid);q=k.$jscomp$loop$prop$shipId$309?SHIP_DATA.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$shipId$309}}(k)):null;v=document.createDocumentFragment();p=document.createDocumentFragment();var t=!0,w=document.getElementById("empty_slot_invisible").checked;q&&!w&&(r=q.slot.length);f=(q?q.name:"\u672a\u6307\u5b9a")+"\uff1a";for(var x=0;x<c.length;x++){var u=c[x],y=u.name;if(q||0!==u.id&&0!==u.slot){if(q&&x===q.slot.length)break;f+=" "+y+" "+getProfString(u.level)+
",";if(!q||!w||0!==u.id&&0!==u.slot){var A=getPlaneCss(u.type).replace("css","shoot_table"),B=document.createElement("tr");B.className="slot"+x+" shipNo"+m;B.dataset.shipid=q?q.id:0;B.dataset.shipindex=a.length-1;B.dataset.slotindex=x;B.dataset.css=A+"_hover";if(t){var z=document.createElement("td");z.rowSpan=r;z.className="td_name align-middle";z.textContent=q?q.name:"\u672a\u6307\u5b9a";B.appendChild(z)}z=document.createElement("td");z.className="pl-1 td_plane_name align-middle "+A;z.textContent=
y;B.appendChild(z);for(z=1;10>=z;z++){var C=document.createElement("td");C.className="td_battle battle"+z+" align-middle";B.appendChild(C)}z=document.createElement("td");z.className="td_battle battle_end align-middle";B.appendChild(z);z=document.createElement("td");z.className="td_battle battle_death align-middle";B.appendChild(z);v.appendChild(B);B=document.createElement("tr");B.className="ship_info_tr slot"+(x+1);t&&(z=document.createElement("td"),z.rowSpan=r,z.className="info_name",z.textContent=
...
function rateCalculate(a){var b=a.landBaseData,c=a.friendFleetData,d=a.battleData,e=castInt($("#calculate_count").val());$("#calculate_count").val(0===e?++e:e);var f=isDefMode?0:castInt($("#landBase_target").val())-1,g=c.concat(),h=g.length,k=d.concat();c=isDefMode?0:displayBattle-1;var l=[0,0,0,0,0,0,0],m=[0,0,0,0,0,0],n=[],r=[],p=[];a=[];for(var q=[],v=[],t=[],w=isDefMode?1:battleCount,x=chartData.own[0],u=$jscomp.makeIterator(k),y=u.next();!y.done;y=u.next())y=y.value,y.enemies=y.enemies.filter(function(a){return 0<
a.slots.length});c=c<d.length?c:d.length-1;for(d=0;6>d;d++)p.push([0,0,0,0,0,0]);b=$jscomp.makeIterator(b);for(d=b.next();!d.done;d=b.next())d=d.value,n.push(d.ap),r.push(d.mode);for(b=0;b<h;b++)for(d=0;d<g[b].length;d++)q.push(Array(e));for(b=0;b<w;b++)v.push(0);for(b=0;b<k[c].enemies.length;b++)if(d=k[c].enemies[b],!d.isSpR)for(u=0===t.length?0:t[t.length-1][0]+1,y=0;y<d.slots.length;y++)t.push([u,y,0,0,999,[]]);b=$jscomp.makeIterator(g);for(d=b.next();!d.done;d=b.next())for(d=$jscomp.makeIterator(d.value),
u=d.next();!u.done;u=d.next())u=u.value,u.slot=u.origSlot,u.ap=u.origAp;for(b=0;b<e;b++){d=$jscomp.makeIterator(k[f].enemies);for(u=d.next();!u.done;u=d.next())u=u.value,u.slots=u.orgSlots.concat(),u.ap=u.origAp,u.lbAp=u.origLbAp;calculateLandBase(n,r,k[f].enemies,l,p);for(d=0;d<w;d++){u=k[d].enemies;y=k[d].cellType;var A=isDefMode?x:getFriendFleetAP(g,y);y=getEnemyFleetAP(u);var B=getAirStatusIndex(A,y);d===c&&m[B]++;v[d]+=A;shootDownFriend(B,g,k[d]);if(d===c)for(shootDownEnemy(B,u),B=A=0;B<u.length;B++){var z=
u[B];if(!z.isSpR){for(var C=z.slots,F=0;F<C.length;F++){var D=C[F],E=t[A++];E[2]+=D;E[3]<D&&(E[3]=D);E[4]>D&&(E[4]=D);E[5].push(y)}z.slots=z.orgSlots.concat();z.ap=z.origAp;z.lbAp=z.origLbAp}}}for(d=0;d<h;d++)for(u=g[d],y=u.length,A=0;A<y;A++)q[5*d+A][b]=u[A].slot,u[A].slot=u[A].origSlot,u[A].ap=u[A].origAp}k=l.map(function(a){return Math.round(a/e)});l=castInt($("#shoot_down_table").find(".fap.battle"+(f+1)).text());n=castInt(k[6]);if(0<l||0<n)$("#shoot_down_table").find(".eap.battle"+(f+1)).text(n),
...
a}).length,deathRate:0},l.deathRate=l.death/k.length,chartData.slots.push(l);for(g=0;g<w;g++)h=Math.round(v[g]/e),q=castInt($("#shoot_down_table").find(".eap.battle"+(g+1)).text()),q=getAirStatusIndex(h,q),$("#shoot_down_table").find(".fap.battle"+(g+1)).text(h),$("#shoot_down_table").find(".cond.battle"+(g+1)).text(AIR_STATUS[q].abbr).addClass("airStatus"+q);isDefMode||(chartData.own[chartData.own.length-1]=Math.round(v[c]/e));chartData.rate=a.concat();chartData.enSlots=t.concat()}
...
n[t];if(0<q[t]){var x=getEnemyFleetLBAP(v);x=getLBAirStatusIndex(w,x);shootDownEnemy(x,v)}2===q[t]&&(x=getEnemyFleetLBAP(v),w=getLBAirStatusIndex(w,x),shootDownEnemy(w,v))}}for(v=0;v<battleCount;v++)t=f[v].enemies,w=f[v].cellType,w=isDefMode?defAp:getFriendFleetAP(e,w),t=getEnemyFleetAP(t),t=getAirStatusIndex(w,t),shootDownFriend(t,e,f[v]);for(v=0;v<e.length;v++)for(t=e[v],w=t.length,x=0;x<w;x++)v===a&&x===b&&l.push(t[x].slot),t[x].slot=t[x].origSlot,t[x].ap=t[x].origAp}updateDetailChart(l,"\u6b8b\u6a5f\u6570 [\u6a5f]",
...
$jscomp.makeIterator(q.value.planes);for(var v=q.next();!v.done;v=q.next())v=v.value,v.origSlot=v.slot,v.origAp=v.ap}for(q=0;q<e;q++){p=$jscomp.makeIterator(k);for(v=p.next();!v.done;v=p.next())v=v.value,v.slots=v.orgSlots.concat(),v.ap=v.origAp,v.lbAp=v.origLbAp;for(p=0;3>p;p++){v=c[p];var t=v.ap;if(0<v.mode){var w=getEnemyFleetLBAP(k);w=getLBAirStatusIndex(t,w);shootDownEnemy(w,k);if(r&&p===a){for(var x=0,u=0;u<v.planes.length;u++){var y=v.planes[u],A=y.origSlot;5===w&&(w=0);y.slot-=getShootDownSlotFF(w,
y.slot);if(y.canAttack&&m){var B=Math.floor(Math.random()*n);y.slot-=Math.floor(h.stage2[y.avoid][0][B]*y.slot)*Math.floor(2*Math.random());y.slot-=h.stage2[y.avoid][1][B]*Math.floor(2*Math.random())}B=0>y.slot?0:y.slot;u===b&&g.push(B);x+=B;y.slot=A}f.push(x)}}if(2===v.mode&&(w=getEnemyFleetLBAP(k),t=getLBAirStatusIndex(t,w),shootDownEnemy(t,k),!r&&p===a)){for(x=w=0;x<v.planes.length;x++)u=v.planes[x],y=u.origSlot,5===t&&(t=0),u.slot-=getShootDownSlotFF(t,u.slot),u.canAttack&&m&&(A=Math.floor(Math.random()*
n),u.slot-=Math.floor(h.stage2[u.avoid][0][A]*u.slot)*Math.floor(2*Math.random()),u.slot-=h.stage2[u.avoid][1][A]*Math.floor(2*Math.random())),A=0>u.slot?0:u.slot,x===b&&g.push(A),w+=A,u.slot=y;f.push(w)}}}var z=l[b].id,C=l[b].remodel,F=l.find(function(a){return 312===a.id})?1.15:l.find(function(a){return 311===a.id})?1.125:1,D=h.cellType===CELL_TYPE.grand?1.1:1;updateDetailChart(g,"\u6b8b\u6a5f\u6570 [\u6a5f]",{mode:"index",callbacks:{title:function(a,b){var c=a[0].xLabel,d="\u822a\u7a7a\u6226\u706b\u529b\uff1a"+
...
w.slots=w.orgSlots.concat(),w.ap=w.origAp,w.lbAp=w.origLbAp;t=e[h].enemies;for(w=0;3>w;w++){var x=q[w];if(0<v[w]){var u=getEnemyFleetLBAP(t);u=getLBAirStatusIndex(x,u);shootDownEnemy(u,t)}2===v[w]&&(u=getEnemyFleetLBAP(t),x=getLBAirStatusIndex(x,u),shootDownEnemy(x,t))}}for(t=0;t<=g;t++)if(w=e[t].enemies,x=e[t].cellType,x=isDefMode?defAp:getFriendFleetAP(d,x),u=getEnemyFleetAP(w),x=getAirStatusIndex(x,u),t===g)for(shootDownEnemy(x,w),n?k.push(l.slots[b]):l.slots[b]?k.push(Math.floor(r[Math.floor(Math.random()*
r.length)]*(p*Math.sqrt(l.slots[b])+25))):k.push(0),x=0;x<w.length;x++)u=w[x],u.isSpR||(u.slots=u.orgSlots.concat(),u.ap=u.origAp,u.lbAp=u.origLbAp);else shootDownFriend(x,d,e[t]);for(t=0;t<d.length;t++)for(w=d[t],x=w.length,u=0;u<x;u++)w[u].slot=w[u].origSlot,w[u].ap=w[u].origAp}updateDetailChart(k,n?"\u6b8b\u6a5f\u6570 [\u6a5f]":"\u822a\u7a7a\u6226\u706b\u529b",{mode:"index",callbacks:{title:function(a,b){var c=a[0].xLabel;if($("#slot_detail").prop("checked")){if(!c)return"\u6b8b\u6a5f\u6570\uff1a"+
