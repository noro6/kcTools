...
castInt(f.dataset.shipid);q=k.$jscomp$loop$prop$shipId$310?SHIP_DATA.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$shipId$310}}(k)):null;v=document.createDocumentFragment();p=document.createDocumentFragment();var t=!0,w=document.getElementById("empty_slot_invisible").checked;q&&!w&&(r=q.slot.length);f=(q?q.name:"\u672a\u6307\u5b9a")+"\uff1a";for(var x=0;x<c.length;x++){var u=c[x],y=u.name;if(q||0!==u.id&&0!==u.slot){if(q&&x===q.slot.length)break;f+=" "+y+" "+getProfString(u.level)+
...
for(var r=$jscomp.makeIterator(n.getElementsByClassName("enemy_content")),p=r.next();!p.done;p=r.next()){var q=p.value,v=castInt(q.dataset.enemyid);p=createEnemyObject(v);0===v||q.classList.contains("d-none")||(-1===v&&(q=q.getElementsByClassName("enemy_ap")[0],q=castInt(q.textContent),p.ap=q,p.lbAp=q,p.origAp=q,p.origLbAp=q),c.enemies.push(p),d+=p.ap,e+=p.lbAp,f+=p.aabo)}l.$jscomp$loop$prop$formationId$312=castInt(n.getElementsByClassName("formation")[0].value);r=(r=FORMATION.find(function(a){return function(b){return b.id===
...
function updateDetailChart(a,b,c){for(var d=[],e=[],f=[],g=getArrayMax(a),h=getArrayMin(a),k=a.length,l=0,m=0,n=0,r=0,p=0,q={$jscomp$loop$prop$i$326:h};q.$jscomp$loop$prop$i$326<=g;q={$jscomp$loop$prop$i$326:q.$jscomp$loop$prop$i$326},q.$jscomp$loop$prop$i$326++){var v=a.filter(function(a){return function(b){return b===a.$jscomp$loop$prop$i$326}}(q)).length;v&&(d.push(q.$jscomp$loop$prop$i$326),e.push((100*v/k).toFixed(1E5<=k?5:1E4<=k?4:1E3<=k?3:2)),p+=100*v/k,f.push(p.toFixed(1)),!l&&50<=p&&(l=q.$jscomp$loop$prop$i$326),
