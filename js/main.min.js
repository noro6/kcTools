...
this.onSettledCallbacks_=null}};var f=new b;e.prototype.settleSameAsPromise_=function(a){var b=this.createResolveAndReject_();a.callWhenSettled_(b.resolve,b.reject)};e.prototype.settleSameAsThenable_=function(a,b){var c=this.createResolveAndReject_();try{a.call(b,c.resolve,c.reject)}catch(m){c.reject(m)}};e.prototype.then=function(a,b){function c(a,b){return"function"==typeof a?function(b){try{d(a(b))}catch(u){f(u)}}:b}var d,f,g=new e(function(a,b){d=a;f=b});this.callWhenSettled_(c(a,d),c(b,f));return g};
...
prof:INITIAL_MAX_LEVEL_PLANE.includes(b.id)?7:0},setting.defaultProf.push(b)}setting.hasOwnProperty("displayMode")||(setting.displayMode={modal_plane_select:"single",modal_ship_select:"single",modal_enemy_select:"single"});setting.hasOwnProperty("orderBy")||(setting.orderBy={modal_plane_select:["default","desc"]});setting.hasOwnProperty("copyToClipbord")&&(setting.copyToClipboard=setting.copyToClipbord,delete setting.copyToClipbord);setting.hasOwnProperty("visibleEquiped")&&(setting.visibleEquipped=
setting.visibleEquiped,delete setting.visibleEquiped);saveLocalStorage("setting",setting)}
function initialize(a){var b=document.createDocumentFragment();b="";$.widget.bridge("uibutton",$.ui.button);$.widget.bridge("uitooltip",$.ui.tooltip);for(var c=0;c<PLANE_TYPE.length;c++){b=new Image;var d=PLANE_TYPE[c].id;b.src="./img/type/type"+d+".png";IMAGES["type"+d]=b;c===PLANE_TYPE.length-1&&b.addEventListener("load",function(){setPlaneStockTable()},!1)}initializeSetting();c=CHANGE_LOG[CHANGE_LOG.length-1];document.getElementById("latest_version").textContent=c.id;if(setting.version!==c.id){b=
document.createDocumentFragment();d=$jscomp.makeIterator(c.changes);for(var e=d.next();!e.done;e=d.next()){e=e.value;var f=document.createElement("div");f.className="mt-3";var g=document.createElement("div"),h=document.createElement("span");h.className="mr-1 pt-1 badge badge-pill badge-"+(0===e.type?"success":1===e.type?"info":"danger");h.textContent=0===e.type?"\u65b0\u898f":1===e.type?"\u5909\u66f4":"\u4fee\u6b63";var k=document.createElement("span");k.innerHTML=e.title;g.appendChild(h);g.appendChild(k);
f.appendChild(g);e.content&&(g=document.createElement("div"),g.className="font_size_12 pl-3",g.innerHTML=e.content,f.appendChild(g));b.appendChild(f)}document.getElementById("version").textContent=c.id;document.getElementById("version_inform_body").appendChild(b);deleteLocalStorage("version");deleteLocalStorage("autoSave");deleteLocalStorage("simulateCount");deleteLocalStorage("emptySlotInvisible");deleteLocalStorage("modalDisplayMode");deleteLocalStorage("defaultProf");(b=loadLocalStorage("planeStock"))&&
0<b.length&&!b[0].num.length&&deleteLocalStorage("planeStock")}setPlaneType(document.getElementById("plane_type_select"),PLANE_TYPE.filter(function(a){return 0<a.id}).map(function(a){return a.id}));setPlaneType(document.getElementById("stock_type_select"),PLANE_TYPE.filter(function(a){return 0<a.id}).map(function(a){return a.id}));b=document.getElementById("plane_type_select").getElementsByTagName("option");d={};b=$jscomp.makeIterator(b);for(e=b.next();!e.done;d={$jscomp$loop$prop$option$299:d.$jscomp$loop$prop$option$299},
e=b.next())d.$jscomp$loop$prop$option$299=e.value,basicSortedPlanes=basicSortedPlanes.concat(PLANE_DATA.filter(function(a){return function(b){return Math.abs(b.type)===castInt(a.$jscomp$loop$prop$option$299.value)}}(d)));setShipType(SHIP_TYPE.filter(function(a){return 15>a.id}).map(function(a){return a.id}));setEnemyType(ENEMY_TYPE.filter(function(a){return 0<a.id}).map(function(a){return a.id}));document.getElementById("display_final").checked=setting.visibleFinal;document.getElementById("frequent_ship").checked=
setting.orderByFrequency;b=document.getElementById("lb_info_table").getElementsByTagName("tbody")[0].innerHTML;for(d=0;2>d;d++)document.getElementById("lb_info_table").getElementsByTagName("tbody")[0].innerHTML+=b;b=document.getElementsByClassName("lb_info_tr");for(d=0;d<b.length;d++)e=b[d],e.classList.add("lb_info_lb_"+(Math.floor(d/4)+1)),e=e.getElementsByClassName("info_name"),e.length&&(e[0].textContent="\u7b2c"+(Math.floor(d/4)+1)+"\u57fa\u5730\u822a\u7a7a\u968a");b=document.getElementsByClassName("display_ship_count");
...
$("#auto_save").prop("checked",setting.autoSave);$("#backup_enabled").prop("checked",setting.backUpEnabled);$("#suspend_backup").prop("checked",!1);$("#backup_count").prop("disabled",!setting.backUpEnabled);$("#backup_count").val(setting.backUpCount);$("#union_fleet").prop("checked",setting.isUnion);$("#clipboard_mode").prop("checked",setting.copyToClipboard);$("#air_raid_max").prop("checked",setting.airRaidMax);$("#empty_slot_invisible").prop("checked",setting.emptySlotInvisible);$("#disp_in_stock").prop("checked",
setting.inStockOnly);$("#disp_equipped").prop("checked",setting.visibleEquipped);$("#enemy_display_image").prop("checked",setting.enemyDisplayImage);$("#enemy_display_text").prop("checked",!setting.enemyDisplayImage);$("#enemy_fleet_display_image").prop("checked",setting.enemyFleetDisplayImage);$("#enemy_fleet_display_text").prop("checked",!setting.enemyFleetDisplayImage);$("#plane_word").val("");$("#ship_word").val("");$("#enemy_word").val("");$("#stock_word").val("");$("#modal_auto_expand").find(".custom-control-input").prop("checked",
!1);$("#btn_start_auto_expand").prop("disabled",!0);$("#mode_1").prop("checked",!0);$("#dest_ap").val(100);$("#dest_range").val(6);"none"===$("#lb_tab_select").css("display")||$("#lb_item1").attr("class").includes("tab-pane")||($(".lb_tab").addClass("tab-pane fade"),$(".baseNo").removeClass("cur_move sortable_handle"),$(".lb_tab:first").addClass("show active"),$("#lb_item1").addClass("active"));b=document.createDocumentFragment();d=0;e=$jscomp.makeIterator(CHANGE_LOG);for(f=e.next();!f.done;f=e.next())for(f=
f.value,g=document.createElement("div"),g.className="my-3 ver_log border-bottom",h=document.createElement("div"),h.className="py-2",h.textContent="v"+f.id,c.id===f.id&&(k=document.createElement("span"),k.className="ml-2 pt-1 badge badge-pill badge-danger",k.textContent="New",h.appendChild(k)),g.appendChild(h),h=0;h<f.changes.length;h++){k=f.changes[h];var p="log_"+d++ +"_"+h,t=document.createElement("div");t.className="py-2 px-2 d-flex collapsed history_item history_item_no_content";var n=document.createElement("div");
n.className="d-flex flex-nowrap align-self-center";var q=document.createElement("div");q.className="align-self-center";var u=document.createElement("span");u.className="mr-2 pt-1 badge badge-pill badge-"+(0===k.type?"success":1===k.type?"info":"danger");u.textContent=0===k.type?"\u65b0\u898f":1===k.type?"\u5909\u66f4":"\u4fee\u6b63";var r=document.createElement("div");r.className="align-self-center";r.innerHTML=k.title;q.appendChild(u);n.appendChild(q);n.appendChild(r);t.appendChild(n);g.appendChild(t);
k.content&&(t.dataset.toggle="collapse",t.dataset.target="#"+p,t.classList.remove("history_item_no_content"),t=document.createElement("div"),t.id=p,t.className="border-top collapse history_content",p=document.createElement("div"),p.className="pl-3 py-2 font_size_12",p.innerHTML=k.content,t.appendChild(p),g.appendChild(t));b.appendChild(g)}c=document.createElement("div");c.className="mt-2 font_size_12";c.textContent=LAST_UPDATE_DATE;b.appendChild(c);document.getElementById("site_history_body").appendChild(b);
...
setting.themeColor?(document.getElementById("normal_theme").checked=!1,document.getElementById("dark_theme").checked=!0,document.getElementById("dark_gradient_theme").checked=!1,document.getElementById("deep_blue_theme").checked=!0):(document.getElementById("normal_theme").checked=!0,document.getElementById("dark_theme").checked=!0,document.getElementById("dark_gradient_theme").checked=!1,document.getElementById("deep_blue_theme").checked=!1);site_theme_Changed(!1);$("#loading_text").addClass("load_end_text");
$("#loading_text").text("\u5b8c\u4e86");$("#loading_line").addClass("loading_line_expand");$("#loading_back").addClass("load_end");setTimeout(function(){$("#loading_back").remove()},1E3);$('[data-toggle="tooltip"]').tooltip();$("#modal_version_inform").find(".modal-body").html()&&$("#modal_version_inform").modal("show");a()}
...
function setEnemyType(a){for(var b=$jscomp.makeIterator(ENEMY_TYPE),c=b.next();!c.done;c=b.next())if(c=c.value,a.includes(c.id)){var d=document.createElement("option");d.value=c.id;d.textContent=c.name;document.getElementById("enemy_type_select").appendChild(d)}}
...
function createPlaneTable(a){var b=$("#modal_plane_select").find(".modal-dialog"),c=$("#plane_tbody"),d=document.getElementById("plane_tbody"),e=document.createDocumentFragment(),f=b.find(".toggle_display_type.selected").data("mode"),g=loadPlaneStock(),h=setting.inStockOnly,k=setting.visibleEquipped,m=$("#plane_sort_select").val();"multi"===f?(b.addClass("modal-xl"),c.addClass("d-flex flex-wrap"),c.prev().addClass("d-none").removeClass("d-flex")):(b.removeClass("modal-xl"),c.removeClass("d-flex flex-wrap"),
c.prev().addClass("d-flex").removeClass("d-none"));h?($("#disp_equipped").closest("div").removeClass("d-none"),$("#disp_in_stock").prop("checked",!0)):($("#disp_equipped").closest("div").addClass("d-none"),$("#disp_in_stock").prop("checked",!1));b=a.length;for(var l=0,p=0,t=0,n=0,q={},u=0;u<b;q={$jscomp$loop$prop$plane$306:q.$jscomp$loop$prop$plane$306},u++){q.$jscomp$loop$prop$plane$306=a[u];var r=q.$jscomp$loop$prop$plane$306.antiAir+1.5*q.$jscomp$loop$prop$plane$306.interception,w=q.$jscomp$loop$prop$plane$306.antiAir+
q.$jscomp$loop$prop$plane$306.interception+2*q.$jscomp$loop$prop$plane$306.antiBomber,x=0<q.$jscomp$loop$prop$plane$306.antiBomber||0<q.$jscomp$loop$prop$plane$306.interception;if(h&&g){var v=g.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$plane$306.id}}(q)),y=usedPlane.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$plane$306.id}}(q)),A=y?getArraySum(y.num):0;n=v?getArraySum(v.num):0;if(0>=n||!k&&A>=n)continue;t=n-A;for(n=10;0<=n;n--)if(y&&(v.num[n]-=
y.num[n]),0<v.num[n]){p=n;break}}v=document.createElement("div");v.className="plane plane_tr d-flex py-2 py-lg-1"+("multi"===f?" tr_multi":"");v.dataset.planeid=q.$jscomp$loop$prop$plane$306.id;v.dataset.type=q.$jscomp$loop$prop$plane$306.type;v.dataset.remodel=p;h&&k&&0>=t&&(v.classList.remove("plane_tr","plane"),v.classList.add("plane_tr_disabled"));y=document.createElement("div");y.className="align-self-center size-25";n=document.createElement("canvas");A=n.getContext("2d");n.width=25;n.height=
25;A.drawImage(IMAGES["type"+q.$jscomp$loop$prop$plane$306.type],0,0,25,25);A=document.createElement("div");A.className="pl-1 plane_td_name align-self-center";A.textContent=q.$jscomp$loop$prop$plane$306.name;var B=document.createElement("div");B.className="ml-1 plane_td_stock align-self-center";B.textContent="\u00d7"+(0<t?t:0);var z=document.createElement("div");z.className="ml-auto plane_td_basic align-self-center "+(x?"text_existTooltip":"");x&&(z.dataset.toggle="tooltip",z.title="\u51fa\u6483\u6642:"+
r+" , \u9632\u7a7a\u6642:"+w);switch(m){case "battle_anti_air":z.textContent=r;break;case "defense_anti_air":z.textContent=w;break;default:z.textContent=q.$jscomp$loop$prop$plane$306.antiAir}r=document.createElement("div");r.className="plane_td_basic align-self-center";r.textContent=q.$jscomp$loop$prop$plane$306.radius;"multi"===f&&l!==q.$jscomp$loop$prop$plane$306.type&&"default"===m&&(l=document.createElement("div"),l.className="d-flex w-100 mt-3 mb-1",w=document.createElement("div"),w.className=
"font_size_12 font_color_half align-self-center",w.textContent=PLANE_TYPE.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$plane$306.type}}(q)).name,x=document.createElement("div"),x.className="flex-grow-1 border-bottom align-self-center mx-2",l.appendChild(w),l.appendChild(x),e.appendChild(l));y.appendChild(n);v.appendChild(y);v.appendChild(A);(0<t||h&&k)&&v.appendChild(B);"single"===f&&(v.appendChild(z),v.appendChild(r));e.appendChild(v);l=q.$jscomp$loop$prop$plane$306.type}d.innerHTML=
...
m=d.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$plane$308.id}}(f)),l=document.createElement("div");l.className="stock_tr py-1 d-flex type_"+Math.abs(f.$jscomp$loop$prop$plane$308.type);l.dataset.planeid=f.$jscomp$loop$prop$plane$308.id;var p=document.createElement("div");p.className="stock_td_type align-self-center";var t=document.createElement("canvas"),n=t.getContext("2d");t.width=24;t.height=24;n.drawImage(IMAGES["type"+f.$jscomp$loop$prop$plane$308.type],0,0,24,24);n=
...
99;p.appendChild(t);l.appendChild(p);l.appendChild(n);l.appendChild(q);l.appendChild(h);l.appendChild(k);b.appendChild(l)}a.innerHTML="";a.appendChild(b);$("#stock_type_select").change()}
...
$("#battle_count").val(c);$(isDefMode?"#air_raid_enemies":".battle_content").each(function(b,c){var d=a[2].find(function(a){return a[0]===b});if(d){3<=d.length?$(c)[0].dataset.celldata=d[2]:$(c)[0].dataset.celldata="";if(4<=d.length)if(isDefMode){var e=castInt(d[3]);$(c).find(".cell_type").val([0,5].includes(e)?5:3)}else $(c).find(".cell_type").val(d[3]);else $(c).find(".cell_type").val(1);5<=d.length?$(c).find(".formation").val(d[4]):$(c).find(".formation").val(1);e=0;d=$jscomp.makeIterator(d[1]);
...
function drawResult(){$("#rate_table tbody").find(".rate_tr").addClass("d-none disabled_tr");$(".progress_area").addClass("d-none disabled_bar").removeClass("d-flex");for(var a=Object.create(chartData),b=a.own.length,c={},d=0;d<b;c={$jscomp$loop$prop$abbr$333:c.$jscomp$loop$prop$abbr$333},d++){var e=a.own[d],f=a.enemy[d],g=a.rate[d],h=getAirStatusBorder(f),k=isDefMode?getAirStatusIndex(e,f):a.rate[d].indexOf(getArrayMax(a.rate[d])),m=0,l=!1,p=isDefMode?8:1===b?7:d+1,t=$("#result_bar_"+p),n=$("#rate_row_"+
p);4===k?m=e/h[3]*10:3===k?m=e/h[2]*20:2===k?m=e/h[1]*45:1>=k&&(m=e/h[0]*90);if(5===k&&(7>p||8===p))k=0,m=100;else if(5===k){c.$jscomp$loop$prop$abbr$333=$("#shoot_down_table").find(".cond.battle"+displayBattle).text();var q=AIR_STATUS.find(function(a){return function(b){return b.abbr===a.$jscomp$loop$prop$abbr$333}}(c));q&&(k=q.id);0===k&&(m=100)}q=t.data("airstatus");t.removeClass("bar_status"+q).addClass("bar_status"+k).css({width:(100<m?100:m)+"%"}).data("airstatus",k);n.find(".rate_td_ap").text(e);
f='<span class="mr-1">'+f+'</span><span class="text-nowrap">( ';for(m=0;m<h.length-1;m++)0!==m&&(f+=" / "),f=h[m]<=e?f+("<b>"+h[m]+"</b>"):f+h[m];n.find(".rate_td_eap").html(f+" )</span>");for(e=0;e<g.length;e++)n.find(".rate_td_status"+e).text("-"),isDefMode?n.find(".rate_td_status"+k).text("100 %"):0<g[e]&&(l=100*g[e],n.find(".rate_td_status"+e).text((100===l?100:l.toFixed(2))+" %"),l=!0);if(l||isDefMode&&8===p)n.removeClass("d-none disabled_tr"),t.closest(".progress_area").addClass("d-flex").removeClass("d-none disabled_bar")}d=
c=b=0;a=castInt($("#calculate_count").val());for(g=0;g<chartData.slots.length;g++)if(k=chartData.slots[g],p=100*k.deathRate,l=document.getElementsByClassName("slot"+k.slot+" shipNo"+k.shipNo)[0])t=castInt(l.getElementsByClassName("battle1")[0].textContent),l.getElementsByClassName("battle1")[0].textContent&&(n='\n    <div class="text-left">\n      <div>\u6700\u5927\u6b8b\u6570\uff1a'+k.max+"</div>\n      <div>\u6700\u5c0f\u6b8b\u6570\uff1a"+k.min+"</div>\n      <div>\u5168\u6ec5\u56de\u6570\uff1a"+
k.death+" / "+a+"</div>\n      <div>\u203b\u30af\u30ea\u30c3\u30af\u3067\u8a73\u7d30\u8a08\u7b97</div>\n    </div>",e=l.getElementsByClassName("battle_end")[0],e.textContent=Math.round(k.avg),e.dataset.toggle="tooltip",e.dataset.html="true",e.title=n,l=l.getElementsByClassName("battle_death")[0],l.textContent=0===k.deathRate?"-":10<=p?p.toFixed(1)+" %":p.toFixed(2)+" %",l.dataset.toggle="tooltip",l.dataset.html="true",l.title=n,0<t&&(b+=5*(t-k.avg),c+=5*(t-k.max),d+=5*(t-k.min)));$(".battle_end.fap").html('<img src="./img/util/bauxite.png" class="img-size-18" alt="\u30dc\u30fc\u30ad\u30b5\u30a4\u30c8"> \u6d88\u8cbb');
...
function updateLandBaseInfo(a,b){b=void 0===b?!0:b;for(var c=0,d=0,e=0,f="",g,h=castInt(document.getElementById("landBase_target").value),k=document.getElementsByClassName("lb_tab"),m=0;m<k.length;m++){var l=k[m],p=m+1,t={baseNo:p,planes:[],ap:0,mode:-1};g="\u7b2c"+p+"\u57fa\u5730\u822a\u7a7a\u968a\uff1a";for(var n=l.getElementsByClassName("lb_plane"),q=0;q<n.length;q++){var u=q+1;if(4<=q)break;var r=createLBPlaneObject(n[q],q);t.planes.push(r);t.ap+=r.ap;if(b){u=document.getElementsByClassName("lb_info_lb_"+
p+" slot"+u)[0];var w=u.getElementsByClassName("info_plane")[0],x=(r.abbr?r.abbr:r.name)+(0!==r.remodel?"\u2605"+r.remodel:"");w.classList="info_plane "+getPlaneCss(r.type).replace("css","shoot_table");w.textContent=x?x+" "+getProfString(r.level):"-";w=u.getElementsByClassName("info_slot")[0];var v=castInt(w.textContent);drawChangeValue(w,v,r.slot);x||(w.textContent="");w=u.getElementsByClassName("info_ap")[0];v=castInt(w.textContent);drawChangeValue(w,v,r.ap);x||(w.textContent="");c+=0===r.id?0:
Math.ceil(r.slot*(101===r.type?1.5:1));d+=0===r.id?0:101===r.type?Math.floor(.7*r.slot):Math.ceil(.6*r.slot);e+=r.cost*(RECONNAISSANCES.includes(r.type)?4:18);g+=" "+(x?x+" "+getProfString(r.level):"")+","}}g+="\n";q=!0;r=$jscomp.makeIterator(t.planes);for(n=r.next();!n.done;n=r.next())0<n.value.id&&(q=!1);if(q)for(t.mode=-1,l.getElementsByClassName("ohuda_select")[0].value="-1",n=$jscomp.makeIterator(l.getElementsByClassName("ohuda_select")[0].getElementsByTagName("option")),q=n.next();!q.done;q=
n.next())q=q.value,-1!==castInt(q.value,-1)&&(q.disabled=!0);else for(n=$jscomp.makeIterator(l.getElementsByClassName("ohuda_select")[0].getElementsByTagName("option")),q=n.next();!q.done;q=n.next())q.value.disabled=!1;t.mode=castInt(l.getElementsByClassName("ohuda_select")[0].value);updateLBAirPower(t);a.push(t);if(b){r=l.getElementsByClassName("resource")[0];n=r.getElementsByClassName("bauxite")[0];q=r.getElementsByClassName("fuel")[0];r=r.getElementsByClassName("ammo")[0];1>t.mode&&(d=c=0);drawChangeValue(q,
castInt(q.textContent),c,!0);drawChangeValue(r,castInt(r.textContent),d,!0);drawChangeValue(n,castInt(n.textContent),e,!0);e=d=c=0;p=document.getElementsByClassName("lb_info_lb_"+p);q=p[0].getElementsByClassName("info_sumAp")[0];drawChangeValue(q,castInt(q.textContent),t.ap);q=l.getElementsByClassName("ap")[0];drawChangeValue(q,castInt(q.textContent),t.ap);n=getRange(t);q=p[0].getElementsByClassName("info_range")[0];drawChangeValue(q,castInt(q.textContent),n);q=l.getElementsByClassName("range")[0];
drawChangeValue(q,castInt(q.textContent),n);0<t.mode&&(l=document.getElementById("battle_container").getElementsByClassName("enemy_range"),l.length>=h-1&&castInt(l[h-1].textContent)>n&&(f+=(f.length?",":"")+(m+1)));p=$jscomp.makeIterator(p);for(l=p.next();!l.done;l=p.next())l=l.value,-1!==t.mode?l.classList.remove("d-none"):l.classList.add("d-none");-1!==t.mode&&t.planes.length&&(document.getElementById("input_summary").value+=g.trim()+"\n")}}if(b){c=0;d=document.getElementById("lb_info_table").getElementsByTagName("tbody")[0].getElementsByClassName("lb_info_tr");
...
d=0;g=$jscomp.makeIterator(a);for(h=g.next();!h.done;h=g.next())if(h=h.value,-1!==h.mode)for(d+=h.ap,h=$jscomp.makeIterator(h.planes),n=h.next();!n.done;n=h.next())ROCKETS.includes(n.value.id)&&e++;g=(0===e?.5:1===e?.8:2===e?1.1:1.2)*d;document.getElementById("lb_info").classList.add("col-lg-8","col-xl-7");document.getElementById("lb_info").classList.remove("col-lg-6");h=document.getElementById("lb_info_table");k=$jscomp.makeIterator(h.getElementsByClassName("info_range"));for(e=k.next();!e.done;e=
...
function updateFriendFleetInfo(a,b){b=void 0===b?!0:b;var c=[],d=0,e=[0,0],f="",g=null,h=$("#union_fleet").prop("checked");g=h?document.getElementsByClassName("ship_tab"):$(".friendFleet_tab.show.active")[0].getElementsByClassName("ship_tab");setting.isUnion=h;saveLocalStorage("setting",setting);for(var k={},m=0;m<g.length;k={$jscomp$loop$prop$shipId$335:k.$jscomp$loop$prop$shipId$335},m++){f=g[m];var l=m+1;if(!f.classList.contains("d-none")&&!f.getElementsByClassName("ship_disabled")[0].checked){for(var p=
c.length=0,t=$jscomp.makeIterator(f.getElementsByClassName("ship_plane")),n=t.next();!n.done;n=t.next())n=n.value,n.classList.contains("ui-draggable")||(n=createFleetPlaneObject(n,l,p++),c.push(n));n=!1;t=p=0;for(var q=$jscomp.makeIterator(c),u=q.next();!u.done;u=q.next())u=u.value,p+=u.ap,!n&&0<u.id&&(n=!0),0<u.id&&0<u.slot&&t++;n&&a.push(c.concat());if(b)if(n=f.getElementsByClassName("ap")[0],q=castInt(n.textContent),drawChangeValue(n,q,p),e[6>=l?0:1]+=p,d+=p,0<t){k.$jscomp$loop$prop$shipId$335=
castInt(f.dataset.shipid);q=k.$jscomp$loop$prop$shipId$335?SHIP_DATA.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$shipId$335}}(k)):null;u=document.createDocumentFragment();n=document.createDocumentFragment();var r=!0,w=document.getElementById("empty_slot_invisible").checked;q&&!w&&(t=q.slot.length);f=(q?q.name:"\u672a\u6307\u5b9a")+"\uff1a";for(var x=0;x<c.length;x++){var v=c[x],y=v.name;if(q||0!==v.id&&0!==v.slot){if(q&&x===q.slot.length)break;f+=" "+y+" "+getProfString(v.level)+
",";if(!q||!w||0!==v.id&&0!==v.slot){var A=getPlaneCss(v.type).replace("css","shoot_table"),B=document.createElement("tr");B.className="slot"+x+" shipNo"+l;B.dataset.shipid=q?q.id:0;B.dataset.shipindex=a.length-1;B.dataset.slotindex=x;B.dataset.css=A+"_hover";if(r){var z=document.createElement("td");z.rowSpan=t;z.className="td_name align-middle";z.textContent=q?q.name:"\u672a\u6307\u5b9a";B.appendChild(z)}z=document.createElement("td");z.className="pl-1 td_plane_name align-middle "+A;z.textContent=
y;B.appendChild(z);for(z=1;10>=z;z++){var C=document.createElement("td");C.className="td_battle battle"+z+" align-middle";B.appendChild(C)}z=document.createElement("td");z.className="td_battle battle_end align-middle";B.appendChild(z);z=document.createElement("td");z.className="td_battle battle_death align-middle";B.appendChild(z);u.appendChild(B);B=document.createElement("tr");B.className="ship_info_tr slot"+(x+1);r&&(z=document.createElement("td"),z.rowSpan=t,z.className="info_name",z.textContent=
q?q.name:"\u672a\u6307\u5b9a",B.appendChild(z));z=document.createElement("td");z.className="info_plane "+A;A=(v.abbr?v.abbr:v.name)+(0!==v.remodel?"\u2605"+v.remodel:"");z.textContent=(A?A:y)+" "+getProfString(v.level);B.appendChild(z);y=document.createElement("td");y.className="info_slot";y.textContent=v.slot;B.appendChild(y);y=document.createElement("td");y.className="info_ap";y.textContent=v.ap;B.appendChild(y);r&&(r=document.createElement("td"),r.rowSpan=t,r.className="info_sumAp",r.textContent=
p,B.appendChild(r));n.appendChild(B);r=!1}}}document.getElementById("shoot_down_table_tbody").appendChild(u);l=document.getElementById("ship_info_table").getElementsByTagName("tbody")[0];l.appendChild(n);l=l.getElementsByTagName("tr");if(0<l.length)for(l=$jscomp.makeIterator(l[l.length-1].getElementsByTagName("td")),p=l.next();!p.done;p=l.next())p.value.classList.add("last_slot");document.getElementById("input_summary").value+=f.slice(0,-1)+"\n"}else if(0===document.getElementById("shoot_down_table_tbody").getElementsByTagName("tr").length){l=
document.createElement("tr");l.className="slot0 shipNo0";p=document.createElement("td");p.className="td_name align-middle";p.textContent="\u672a\u9078\u629e";l.appendChild(p);p=document.createElement("td");p.className="td_plane_name text-nowrap align-middle text-center";p.textContent="-";l.appendChild(p);for(p=1;10>=p;p++)t=document.createElement("td"),t.className="td_battle battle"+p+" align-middle",l.appendChild(t);p=document.createElement("td");p.className="td_battle battle_end align-middle";p.textContent=
d;l.appendChild(p);document.getElementById("shoot_down_table_tbody").appendChild(l)}}}if(b){if(h)for(c=0;c<e.length;c++)document.getElementsByClassName("fleet_ap")[c].textContent=e[c];else $(".friendFleet_tab.show.active")[0].getElementsByClassName("fleet_ap")[0].textContent=e[0];e=document.getElementById("ship_info_table").getElementsByTagName("tbody")[0];(c=e.getElementsByTagName("tr").length)?(g=document.createElement("td"),g.className="info_fleetAp",g.rowSpan=c,g.textContent=d,e.getElementsByClassName("ship_info_tr")[0].appendChild(g),
...
function updateEnemyFleetInfo(a,b){b=void 0===b?!0:b;var c=null,d=0,e=0,f=0,g="999-1",h="",k=!1,m=!0,l={},p=$jscomp.makeIterator(document.getElementsByClassName("battle_content"));for(d=p.next();!d.done;l={$jscomp$loop$prop$formationId$337:l.$jscomp$loop$prop$formationId$337},d=p.next()){var t=d.value;isDefMode&&(t=document.getElementById("air_raid_enemies"));if(isDefMode&&0<a.length)break;if(!t.classList.contains("d-none")){c={enemies:[],cellType:0,stage2:[],isAllSubmarine:!1};f=e=d=0;m=!0;c.cellType=
castInt(t.getElementsByClassName("cell_type")[0].value);for(var n=$jscomp.makeIterator(t.getElementsByClassName("enemy_content")),q=n.next();!q.done;q=n.next()){var u=q.value,r=castInt(u.dataset.enemyid);q=createEnemyObject(r);if(!u.classList.contains("d-none")){if(c.cellType!==CELL_TYPE.grand&&6===c.enemies.length)break;-1===r&&(u=u.getElementsByClassName("enemy_ap")[0],u=castInt(u.textContent),q.ap=u,q.lbAp=u,q.origAp=u,q.origLbAp=u);m&&!q.type.includes(18)&&(m=!1);c.enemies.push(q);d+=q.ap;e+=
q.lbAp;f+=q.aabo}}c.enemies.length&&(c.isAllSubmarine=m);l.$jscomp$loop$prop$formationId$337=castInt(t.getElementsByClassName("formation")[0].value);m=(m=FORMATION.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$formationId$337}}(l)))?m.correction:1;c.stage2=createStage2Table(c.enemies,c.cellType,l.$jscomp$loop$prop$formationId$337);a.push(c);b&&(c=t.getElementsByClassName("enemy_sum_ap")[0],drawChangeValue(c,c.textContent,d,!0),c=t.getElementsByClassName("enemy_sum_lbap")[0],
...
(g.includes("999-")?document.getElementById("route").value="\u6d77\u57df\u672a\u6307\u5b9a":document.getElementById("route").value=k?"\u5225\u6d77\u57df\u306e\u30bb\u30eb\u304c\u6df7\u5728\u3057\u3066\u3044\u307e\u3059\u3002":g+"\uff1a"+h);h=a[isDefMode?0:displayBattle-1].enemies.filter(function(a){return 0<a.slots.length});g=document.createDocumentFragment();k=0;l={};for(p=0;p<h.length;l={$jscomp$loop$prop$enemy$339:l.$jscomp$loop$prop$enemy$339},p++)if(l.$jscomp$loop$prop$enemy$339=h[p],!l.$jscomp$loop$prop$enemy$339.isSpR&&
l.$jscomp$loop$prop$enemy$339.slots.length){d="";e=document.createDocumentFragment();for(f={$jscomp$loop$prop$j$341:0};f.$jscomp$loop$prop$j$341<l.$jscomp$loop$prop$enemy$339.slots.length;f={$jscomp$loop$prop$j$341:f.$jscomp$loop$prop$j$341},f.$jscomp$loop$prop$j$341++)if(n=ENEMY_PLANE_DATA.find(function(a,b){return function(c){return c.id===a.$jscomp$loop$prop$enemy$339.eqp[b.$jscomp$loop$prop$j$341]}}(l,f)))m=l.$jscomp$loop$prop$enemy$339.slots[f.$jscomp$loop$prop$j$341],q=getPlaneCss(n.type).replace("css",
"shoot_table"),t=f.$jscomp$loop$prop$j$341===l.$jscomp$loop$prop$enemy$339.slots.length-1?" last_slot":"",c=document.createElement("tr"),c.className="enemy_no_"+k+" slot_"+f.$jscomp$loop$prop$j$341,c.dataset.rowindex=k,c.dataset.slotindex=f.$jscomp$loop$prop$j$341,c.dataset.css=q+"_hover",d+='<img src="./img/plane/'+n.id+'.png" class="img-size-36" title="'+n.name+'">',u=document.createElement("td"),u.className="pl-1 td_plane_name align-middle "+q+t,u.textContent=n.name,c.appendChild(u),n=document.createElement("td"),
n.className="td_init_slot align-middle"+t,n.textContent=m,c.appendChild(n),n=document.createElement("td"),n.className="td_avg_slot align-middle"+t,n.textContent=m,c.appendChild(n),n=document.createElement("td"),n.className="td_max_slot align-middle"+t,n.textContent=m,c.appendChild(n),n=document.createElement("td"),n.className="td_min_slot align-middle"+t,n.textContent=m,c.appendChild(n),isDefMode?document.getElementsByClassName("td_detail_slot")[0].classList.add("d-none"):(m=document.createElement("td"),
m.className="td_detail_slot align-middle"+t,m.innerHTML='<i class="fas fa-file-signature"></i>',c.appendChild(m),document.getElementsByClassName("td_detail_slot")[0].classList.remove("d-none")),e.appendChild(c);f=document.createElement("td");f.rowSpan=l.$jscomp$loop$prop$enemy$339.slots.length;f.className="td_name align-middle last_slot";f.innerHTML="<div>"+drawEnemyGradeColor(l.$jscomp$loop$prop$enemy$339.name)+"</div><div>"+d+"</div>";e.firstChild.insertBefore(f,e.firstChild.firstChild);g.appendChild(e);
...
c});g=Math.floor((h?h.correction:1)*g);for(h=0;h<e;h++)if(0!==a[h].id)for(k=f.shift(),m=0;m<AVOID_TYPE.length;m++){l=Math.floor(k*AVOID_TYPE[m].adj[0]);var p=2*Math.floor(g*AVOID_TYPE[m].adj[1]);b===CELL_TYPE.grand?6>h?d[m][1].push(Math.floor(.8*(l+p)*.09375)):d[m][1].push(Math.floor(.045*(l+p))):d[m][1].push(Math.floor(.09375*(l+p)))}return d}
...
e&&(e=0);if(!f)return;for(var k=0,m=0;1E3>m;m++){var l=h.slot,p=getShootDownSlotFF(e,Math.round(l));l-=Math.floor(9===h.type?.6*p:p);h.canAttack&&d.cellType<=CELL_TYPE.grand&&(p=Math.floor(Math.random()*f),l-=Math.floor(d.stage2[h.avoid][0][p]*l)*Math.floor(2*Math.random()),l-=d.stage2[h.avoid][1][p]*Math.floor(2*Math.random()));k+=0<l?l:0}h.slot=k/1E3;h.ap=updateAp(h)}}$("#shoot_down_table").find(".eap.battle"+(c+1)).text(b)}
...
function rateCalculate(a){var b=a.landBaseData,c=a.friendFleetData,d=a.battleData,e=castInt($("#calculate_count").val());$("#calculate_count").val(0===e?++e:e);var f=isDefMode?0:castInt($("#landBase_target").val())-1,g=c.concat(),h=g.length,k=d.concat();c=isDefMode?0:displayBattle-1;var m=[0,0,0,0,0,0,0],l=[0,0,0,0,0,0],p=[],t=[],n=[];a=[];for(var q=[],u=[],r=[],w=isDefMode?1:battleCount,x=chartData.own[0],v=$jscomp.makeIterator(k),y=v.next();!y.done;y=v.next())y=y.value,y.enemies=y.enemies.filter(function(a){return 0<
a.slots.length});c=c<d.length?c:d.length-1;for(d=0;6>d;d++)n.push([0,0,0,0,0,0]);b=$jscomp.makeIterator(b);for(d=b.next();!d.done;d=b.next())d=d.value,p.push(d.ap),t.push(d.mode);for(b=0;b<h;b++)for(d=0;d<g[b].length;d++)q.push(Array(e));for(b=0;b<w;b++)u.push(0);for(b=0;b<k[c].enemies.length;b++)if(d=k[c].enemies[b],!d.isSpR)for(v=0===r.length?0:r[r.length-1][0]+1,y=0;y<d.slots.length;y++)r.push([v,y,0,0,999,[]]);b=$jscomp.makeIterator(g);for(d=b.next();!d.done;d=b.next())for(d=$jscomp.makeIterator(d.value),
v=d.next();!v.done;v=d.next())v=v.value,v.slot=v.origSlot,v.ap=v.origAp;for(b=0;b<e;b++){d=$jscomp.makeIterator(k[f].enemies);for(v=d.next();!v.done;v=d.next())v=v.value,v.slots=v.orgSlots.concat(),v.ap=v.origAp,v.lbAp=v.origLbAp;calculateLandBase(p,t,k[f].enemies,m,n);for(d=0;d<w;d++){doJetPhase(g,k[d]);v=k[d].enemies;y=k[d].cellType;var A=isDefMode?x:getFriendFleetAP(g,y);y=getEnemyFleetAP(v);var B=getAirStatusIndex(A,y);d===c&&l[B]++;u[d]+=A;shootDownFriend(B,g,k[d]);if(d===c)for(shootDownEnemy(B,
v),B=A=0;B<v.length;B++){var z=v[B];if(!z.isSpR){for(var C=z.slots,F=0;F<C.length;F++){var D=C[F],E=r[A++];E[2]+=D;E[3]<D&&(E[3]=D);E[4]>D&&(E[4]=D);E[5].push(y)}z.slots=z.orgSlots.concat();z.ap=z.origAp;z.lbAp=z.origLbAp}}}for(d=0;d<h;d++)for(v=g[d],y=v.length,A=0;A<y;A++)q[5*d+A][b]=v[A].slot,v[A].slot=v[A].origSlot,v[A].ap=v[A].origAp}k=m.map(function(a){return Math.round(a/e)});m=castInt($("#shoot_down_table").find(".fap.battle"+(f+1)).text());p=castInt(k[6]);if(0<m||0<p)$("#shoot_down_table").find(".eap.battle"+
(f+1)).text(p),$("#shoot_down_table").find(".cond.battle"+(f+1)).text(AIR_STATUS[getAirStatusIndex(m,p)].abbr);f===c&&(a=n.concat(),chartData.enemy=k);a.push(l);f=$jscomp.makeIterator(a);for(l=f.next();!l.done;l=f.next())for(l=l.value,n=l.length,k=0;k<n;k++)l[k]/=e;isDefMode&&(chartData.enemy[0]=chartData.enemy[6],a[0]=a[6]);for(l=f=0;l<h;l++)for(n=0;n<g[l].length;n++)k=q[f++],m={shipNo:g[l][n].fleetNo,slot:g[l][n].slotNo,max:getArrayMax(k),min:getArrayMin(k),avg:k.reduce(function(a,b){return a+b})/
e,death:k.filter(function(a){return 0===a}).length,deathRate:0},m.deathRate=m.death/k.length,chartData.slots.push(m);for(g=0;g<w;g++)h=Math.round(u[g]/e),q=castInt($("#shoot_down_table").find(".eap.battle"+(g+1)).text()),q=getAirStatusIndex(h,q),$("#shoot_down_table").find(".fap.battle"+(g+1)).text(h),$("#shoot_down_table").find(".cond.battle"+(g+1)).text(AIR_STATUS[q].abbr).addClass("airStatus"+q);isDefMode||(chartData.own[chartData.own.length-1]=Math.round(u[c]/e));chartData.rate=a.concat();chartData.enSlots=
r.concat()}function calculateLandBase(a,b,c,d,e){for(var f=0;3>f;f++){var g=a[f];if(0<b[f]){var h=2*f,k=getEnemyFleetLBAP(c);d[h]+=k;k=getLBAirStatusIndex(g,k);e[h][k]++;shootDownEnemy(k,c)}2===b[f]&&(h=2*f+1,k=getEnemyFleetLBAP(c),d[h]+=k,g=getLBAirStatusIndex(g,k),e[h][g]++,shootDownEnemy(g,c))}d[6]+=isDefMode?getEnemyFleetLBAP(c):getEnemyFleetAP(c)}
...
function fleetSlotDetailCalculate(a,b,c){c=void 0===c?0:c;setPreCalculateTable();var d=[],e=[],f=[];updateLandBaseInfo(d,!1);updateFriendFleetInfo(e,!1);updateEnemyFleetInfo(f,!1);var g=castInt($("#calculate_count").val()),h=isDefMode?0:displayBattle-1,k=isDefMode?0:castInt($("#landBase_target").val())-1,m=[],l=e[a],p=PLANE_DATA.find(function(a){return a.id===l[b].id});h=h<f.length?h:f.length-1;var t=2===Math.abs(p.type)?[.8,1.5]:ATTACKERS.includes(p.type)?[1]:[0],n=.2*(3===Math.abs(p.type)?0:l[b].remodel)+
(2===Math.abs(p.type)?p.torpedo:p.bomber);p=$jscomp.makeIterator(f);for(var q=p.next();!q.done;q=p.next())q=q.value,q.enemies=q.enemies.filter(function(a){return 0<a.slots.length});p=[];q=[];d=$jscomp.makeIterator(d);for(var u=d.next();!u.done;u=d.next())u=u.value,p.push(u.ap),q.push(u.mode);for(d=0;d<g;d++){if(k<=h){u=$jscomp.makeIterator(f[k].enemies);for(var r=u.next();!r.done;r=u.next())r=r.value,r.slots=r.orgSlots.concat(),r.ap=r.origAp,r.lbAp=r.origLbAp;u=f[k].enemies;for(r=0;3>r;r++){var w=
p[r];if(0<q[r]){var x=getEnemyFleetLBAP(u);x=getLBAirStatusIndex(w,x);shootDownEnemy(x,u)}2===q[r]&&(x=getEnemyFleetLBAP(u),w=getLBAirStatusIndex(w,x),shootDownEnemy(w,u))}}for(u=0;u<battleCount;u++)doJetPhase(e,f[u]),r=f[u].enemies,w=f[u].cellType,w=isDefMode?defAp:getFriendFleetAP(e,w),r=getEnemyFleetAP(r),r=getAirStatusIndex(w,r),shootDownFriend(r,e,f[u]);for(u=0;u<e.length;u++)for(r=e[u],w=r.length,x=0;x<w;x++)u===a&&x===b&&m.push(r[x].slot),r[x].slot=r[x].origSlot,r[x].ap=r[x].origAp}updateDetailChart(m,
"\u6b8b\u6a5f\u6570 [\u6a5f]",{mode:"index",callbacks:{title:function(a,b){var c=a[0].xLabel;if(!c)return"\u6b8b\u6a5f\u6570\uff1a"+c+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0";var d=2===t.length?Math.floor(t[1]*(n*Math.sqrt(c)+25)):0;return"\u6b8b\u6a5f\u6570\uff1a"+c+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a"+(Math.floor(t[0]*(n*Math.sqrt(c)+25))+(d?" or "+d:""))},label:function(a,b){return[a.yLabel+" %"]}}});e=$("#show_detail_slot .nav-item");g=SHIP_DATA.find(function(a){return a.id===
...
function landBaseDetailCalculate(a,b){setPreCalculateTable();var c=[],d=[];updateLandBaseInfo(c,!1);updateEnemyFleetInfo(d,!1);var e=castInt($("#calculate_count").val()),f=isDefMode?0:castInt($("#landBase_target").val())-1,g=[],h=d[f],k=h.enemies,m=c[a].planes;for(f=d=0;f<m.length;f++){var l=m[f];0===m[b].id&&0!==l.id&&(b=f);d+=l.slot}l=h.cellType<=CELL_TYPE.grand;var p=h.stage2[0][0].length,t=$("#detail_wave1").prop("checked");f=[];for(var n=$jscomp.makeIterator(c),q=n.next();!q.done;q=n.next()){q=
$jscomp.makeIterator(q.value.planes);for(var u=q.next();!u.done;u=q.next())u=u.value,u.origSlot=u.slot,u.origAp=u.ap}for(q=0;q<e;q++){n=$jscomp.makeIterator(k);for(u=n.next();!u.done;u=n.next())u=u.value,u.slots=u.orgSlots.concat(),u.ap=u.origAp,u.lbAp=u.origLbAp;for(n=0;3>n;n++){u=c[n];var r=u.ap;if(0<u.mode){var w=getEnemyFleetLBAP(k);w=getLBAirStatusIndex(r,w);shootDownEnemy(w,k);if(t&&n===a){for(var x=0,v=0;v<u.planes.length;v++){var y=u.planes[v],A=y.origSlot;5===w&&(w=0);var B=getShootDownSlotFF(w,
y.slot);y.slot-=Math.floor(9===y.type?.6*B:B);y.canAttack&&l&&(B=Math.floor(Math.random()*p),y.slot-=Math.floor(h.stage2[y.avoid][0][B]*y.slot)*Math.floor(2*Math.random()),y.slot-=h.stage2[y.avoid][1][B]*Math.floor(2*Math.random()));B=0>y.slot?0:y.slot;v===b&&g.push(B);x+=B;y.slot=A}f.push(x)}}if(2===u.mode&&(w=getEnemyFleetLBAP(k),r=getLBAirStatusIndex(r,w),shootDownEnemy(r,k),!t&&n===a)){for(x=w=0;x<u.planes.length;x++)v=u.planes[x],y=v.origSlot,5===r&&(r=0),A=getShootDownSlotFF(r,v.slot),v.slot-=
Math.floor(9===v.type?.6*A:A),v.canAttack&&l&&(A=Math.floor(Math.random()*p),v.slot-=Math.floor(h.stage2[v.avoid][0][A]*v.slot)*Math.floor(2*Math.random()),v.slot-=h.stage2[v.avoid][1][A]*Math.floor(2*Math.random())),A=0>v.slot?0:v.slot,x===b&&g.push(A),w+=A,v.slot=y;f.push(w)}}}var z=m[b].id,C=m[b].remodel,F=m.find(function(a){return 312===a.id})?1.15:m.find(function(a){return 311===a.id})?1.125:1,D=h.cellType===CELL_TYPE.grand?1.1:1;updateDetailChart(g,"\u6b8b\u6a5f\u6570 [\u6a5f]",{mode:"index",
callbacks:{title:function(a,b){var c=a[0].xLabel,d="\u822a\u7a7a\u6226\u706b\u529b\uff1a"+(getLandBasePower(z,c,C)*F*D).toFixed(1);return c?"\u6b8b\u6a5f\u6570\uff1a"+c+" \u6a5f\n"+d:"\u6b8b\u6a5f\u6570\uff1a0 \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0"},label:function(a,b){return[a.yLabel+" %"]}}});k=h=g=0;f=$jscomp.makeIterator(f);for(q=f.next();!q.done;q=f.next())m=q.value,l=m/d,0===m?k++:.25>l?h++:.4>l&&g++;$("#status_s").text(100*g/e+" %");$("#status_m").text(100*h/e+" %");$("#status_l").text(100*
...
function enemySlotDetailCalculate(a,b){setPreCalculateTable();var c=[],d=[],e=[];updateLandBaseInfo(c,!1);updateFriendFleetInfo(d,!1);updateEnemyFleetInfo(e,!1);var f=castInt($("#calculate_count").val()),g=isDefMode?0:displayBattle-1;g=g<e.length?g:e.length-1;var h=isDefMode?0:castInt($("#landBase_target").val())-1,k=[],m=e[g].enemies.filter(function(a){return 0<a.slots.length&&!a.isSpR})[a],l=ENEMY_DATA.find(function(a){return a.id===m.id}),p=ENEMY_PLANE_DATA.find(function(a){return a.id===l.eqp[b]}),
t=2===Math.abs(p.type)?[.8,1.5]:1===Math.abs(p.type)?[0]:[1],n=2===Math.abs(p.type)?p.torpedo:p.bomber;p=$("#slot_detail").prop("checked");for(var q=$jscomp.makeIterator(e),u=q.next();!u.done;u=q.next())u=u.value,u.enemies=u.enemies.filter(function(a){return 0<a.slots.length});q=[];u=[];c=$jscomp.makeIterator(c);for(var r=c.next();!r.done;r=c.next())r=r.value,q.push(r.ap),u.push(r.mode);for(c=0;c<f;c++){if(h<=g){r=$jscomp.makeIterator(e[h].enemies);for(var w=r.next();!w.done;w=r.next())w=w.value,
w.slots=w.orgSlots.concat(),w.ap=w.origAp,w.lbAp=w.origLbAp;r=e[h].enemies;for(w=0;3>w;w++){var x=q[w];if(0<u[w]){var v=getEnemyFleetLBAP(r);v=getLBAirStatusIndex(x,v);shootDownEnemy(v,r)}2===u[w]&&(v=getEnemyFleetLBAP(r),x=getLBAirStatusIndex(x,v),shootDownEnemy(x,r))}}for(r=0;r<=g;r++)if(w=e[r].enemies,x=e[r].cellType,x=isDefMode?defAp:getFriendFleetAP(d,x),v=getEnemyFleetAP(w),x=getAirStatusIndex(x,v),r===g)for(shootDownEnemy(x,w),p?k.push(m.slots[b]):m.slots[b]?k.push(Math.floor(t[Math.floor(Math.random()*
t.length)]*(n*Math.sqrt(m.slots[b])+25))):k.push(0),x=0;x<w.length;x++)v=w[x],v.isSpR||(v.slots=v.orgSlots.concat(),v.ap=v.origAp,v.lbAp=v.origLbAp);else shootDownFriend(x,d,e[r]);for(r=0;r<d.length;r++)for(w=d[r],x=w.length,v=0;v<x;v++)w[v].slot=w[v].origSlot,w[v].ap=w[v].origAp}updateDetailChart(k,p?"\u6b8b\u6a5f\u6570 [\u6a5f]":"\u822a\u7a7a\u6226\u706b\u529b",{mode:"index",callbacks:{title:function(a,b){var c=a[0].xLabel;if($("#slot_detail").prop("checked")){if(!c)return"\u6b8b\u6a5f\u6570\uff1a"+
c+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0";var d=2===t.length?Math.floor(t[1]*(n*Math.sqrt(c)+25)):0;return"\u6b8b\u6a5f\u6570\uff1a"+c+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a"+(Math.floor(t[0]*(n*Math.sqrt(c)+25))+(d?" or "+d:""))}return"\u822a\u7a7a\u6226\u706b\u529b\uff1a"+c},label:function(a,b){return[a.yLabel+" %"]}}});e=$("#show_detail_slot .nav-item");e.eq(b).find(".nav-link").tab("show");if("enemy_slot_detail"!==$("#detail_info").data("mode")||$("#detail_info").data("base_no")!=
...
function updateDetailChart(a,b,c){for(var d=[],e=[],f=[],g=getArrayMax(a),h=getArrayMin(a),k=a.length,m=0,l=0,p=0,t=0,n=0,q={$jscomp$loop$prop$i$351:h};q.$jscomp$loop$prop$i$351<=g;q={$jscomp$loop$prop$i$351:q.$jscomp$loop$prop$i$351},q.$jscomp$loop$prop$i$351++){var u=a.filter(function(a){return function(b){return b===a.$jscomp$loop$prop$i$351}}(q)).length;u&&(d.push(q.$jscomp$loop$prop$i$351),e.push((100*u/k).toFixed(1E5<=k?5:1E4<=k?4:1E3<=k?3:2)),n+=100*u/k,f.push(n.toFixed(1)),!m&&50<=n&&(m=q.$jscomp$loop$prop$i$351),
!l&&90<=n&&(l=q.$jscomp$loop$prop$i$351),!p&&95<=n&&(p=q.$jscomp$loop$prop$i$351),!t&&99<=n&&(t=q.$jscomp$loop$prop$i$351))}$("#detail_max").text(g);$("#detail_min").text(h);$("#detail_50").text(m);$("#detail_90").text(l);$("#detail_95").text(p);$("#detail_99").text(t);chartDataSet?(chartDataSet.data.labels=d,chartDataSet.data.datasets[0].data=e,chartDataSet.data.datasets[1].data=f,chartDataSet.options.scales.xAxes[0].scaleLabel.labelString=b,chartDataSet.options.tooltips=c,chartDataSet.update()):
...
(g[l]=[]);for(var p=0;p<h.$jscomp$loop$prop$stock$353.num.length;p++)for(var t=0;t<h.$jscomp$loop$prop$stock$353.num[p];t++){var n={id:m.id,type:m.type,antiAir:m.antiAir+1.5*m.interception,torpedo:m.torpedo,bomber:m.bomber,remodel:p};n.antiAir=getBonusAA(n,n.antiAir);g[l].push(n)}}Object.keys(g).forEach(function(a){"type1"===a?g[a].sort(function(a,b){return a.antiAir===b.antiAir?b.radius-a.radius:b.antiAir-a.antiAir}):"type2"===a?g[a].sort(function(a,b){return a.torpedo===b.torpedo?b.radius-a.radius:
...
b){setLBPlaneDiv($(b),q[a])});$("#lb_item1").find(".ohuda_select").val(0<q.length?2:-1)}if(d){var u=autoLandBaseExpand(g);$("#lb_item2").find(".lb_plane").each(function(a,b){setLBPlaneDiv($(b),u[a])});$("#lb_item2").find(".ohuda_select").val(0<u.length?2:-1)}if(e){var r=autoLandBaseExpand(g);$("#lb_item3").find(".lb_plane").each(function(a,b){setLBPlaneDiv($(b),r[a])});$("#lb_item3").find(".ohuda_select").val(0<r.length?2:-1)}}}
...
b){return b.antiAir-a.antiAir}):"type2"===a?e[a].sort(function(a,b){return a.torpedo===b.torpedo?b.antiAir-a.antiAir:b.torpedo-a.torpedo}):["type3","type6"].includes(a)&&e[a].sort(function(a,b){return a.avoid===b.avoid?b.bomber-a.bomber:b.avoid-a.avoid})});var p=[],t=0;c.each(function(a,b){var c=SHIP_DATA.find(function(a){return a.id===castInt($(b)[0].dataset.shipid)});if(c){var d=[];$(b).find(".ship_plane:not(.d-none)").each(function(b,f){var g=castInt($(f).find(".slot").text()),h=$(f).find(".plane_unlock").hasClass("d-none"),
k={shipId:c.id,shipType:c.type,slotNo:b,num:g,plane:{id:0,ap:0},order:t++,num_:g,slotId:"ship"+a+"_slot"+b,isLock:h,lockedPlane:null};if(h){var l=PLANE_DATA.find(function(a){return a.id===castInt($(f)[0].dataset.planeid)});if(l){var m=castInt($(f).find(".remodel_value").text());k.lockedPlane={id:l.id,remodel:m};Object.keys(e).forEach(function(a){var b=e[a].findIndex(function(a){return a.id===l.id&&a.remodel===m});-1<b&&e[a].splice(b,1)})}}else 0===g&&(k.isLock=!0);[1,2,3].includes(c.type)?10>g&&(k.num_=
g-100):k.num_=g-100;d.push(k)});p=p.concat(d)}});g=[2,6];f=0;p.sort(function(a,b){return b.num_-a.num_});h=$jscomp.makeIterator(p);for(a=h.next();!a.done;a=h.next())for(k=a.value,l=!1,m=$jscomp.makeIterator(g),a=m.next();!a.done;a=m.next()){a=a.value;if(k.isLock){k.lockedPlane&&(a=getShipPlaneObject(k.num,k.lockedPlane),k.plane=a,f+=a.ap);l=!0;break}var n=k.shipType;d&&[1,2,3].includes(n)&&1===k.slotNo&&(a=3);n={};var q=$jscomp.makeIterator(e["type"+a]);for(a=q.next();!a.done;n={$jscomp$loop$prop$plane$255$357:n.$jscomp$loop$prop$plane$255$357},
a=q.next())if(n.$jscomp$loop$prop$plane$255$357=a.value,!(0>=n.$jscomp$loop$prop$plane$255$357.stock)){checkInvalidPlane(k.shipId,PLANE_DATA.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$plane$255$357.id}}(n)))&&(a=getShipPlaneObject(k.num,n.$jscomp$loop$prop$plane$255$357),k.plane=a,f+=a.ap,n.$jscomp$loop$prop$plane$255$357.stock--,l=!0);break}if(l)break}p.sort(function(a,b){return a.num_===b.num_?b.slotNo-a.slotNo:a.num_-b.num_});g=f;h=[];d=[];k=[];for(m=0;m<p.length&&!(f>=
b);m++)if(!p[m].isLock){d=[];k=[];f=$jscomp.makeIterator(e.type1);for(a=f.next();!a.done;a=f.next())a=a.value,k.push({id:a.id,remodel:a.remodel,antiAir:a.antiAir,stock:a.stock});f=g;h.push(p[m]);for(l=h.length-1;0<=l;l--){n=h[l];q=!1;var u={},r=$jscomp.makeIterator(k);for(a=r.next();!a.done;u={$jscomp$loop$prop$plane$262$359:u.$jscomp$loop$prop$plane$262$359},a=r.next())if(u.$jscomp$loop$prop$plane$262$359=a.value,!(0>=u.$jscomp$loop$prop$plane$262$359.stock)){if(checkInvalidPlane(n.shipId,PLANE_DATA.find(function(a){return function(b){return b.id===
a.$jscomp$loop$prop$plane$262$359.id}}(u)))){a=getShipPlaneObject(n.num,u.$jscomp$loop$prop$plane$262$359);var w=n.plane.ap;if(w<a.ap){d.push(a);f+=a.ap-w;u.$jscomp$loop$prop$plane$262$359.stock--;q=!0;break}}if(q)break}q||d.push(n.plane)}}b=0;f=d.length;for(a=0;a<p.length;a++)if(!p[a].isLock){if(b>=f)break;p[a].plane=d[f-1-b++]}p.sort(function(a,b){return a.order-b.order});t=0;c.each(function(a,b){$(b).find(".ship_plane:not(.d-none)").each(function(b,c){var d=p.find(function(c){return c.slotId===
...
PLANE_DATA.find(function(a){return function(b){return b.id===a.$jscomp$loop$prop$stock$365.id}}(h)),RECONNAISSANCES.includes(k.type))for(var m=10;0<=m;m--){if(!(1>h.$jscomp$loop$prop$stock$365.num[m])){var l={id:k.id,antiAir:k.antiAir,type:k.type,adjust:1,scout:k.scout,remodel:m};l.adjust=getReconnaissancesAdjust(l,!0);for(var p=0;p<h.$jscomp$loop$prop$stock$365.num[m];p++)f.push(l)}}else for(m=10;0<=m;m--)if(!(1>h.$jscomp$loop$prop$stock$365.num[m])&&(l={id:k.id,type:k.type,remodel:m,antiAir:k.antiAir+
k.interception+2*k.antiBomber,interception:k.interception,antiBomber:k.antiBomber,isRocket:0},l.antiAir=getBonusAA(l,l.antiAir),0!==l.antiAir))for(p=0;p<h.$jscomp$loop$prop$stock$365.num[m];p++)e.push(Object.assign({},l))}if(d){e.sort(function(a,b){return b.antiAir-a.antiAir});for(g=0;3>g;g++)h=e.findIndex(function(a){return 0===a.isRocket&&ROCKETS.includes(a.id)}),-1<h&&(e[h].isRocket=1);e.sort(function(a,b){return b.isRocket-a.isRocket})}else e.sort(function(a,b){return a.antiAir===b.antiAir?b.radius-
a.radius:b.antiAir-a.antiAir});f.sort(function(a,b){return b.adjust===a.adjust?b.scout-a.scout:b.adjust-a.adjust});e=e.filter(function(b,c){return c<4*a});g=[];for(h=0;h<a;h++){c={ap:0,planes:[]};for(k=0;4>k;k++)c.planes.push({id:0});g.push(c)}for(k=c=h=0;k<a;k++){m={ap:0,planes:[]};l={ap:0,planes:[]};for(p=0;4>p;p++){var t=e[p],n=getLandBasePlaneObject(t);m.planes.push(n);m.ap+=n.ap;l.planes.push(n);l.ap+=n.ap;h+=n.ap;if(d){if(ROCKETS.includes(t.id)&&c++,Math.floor((0===c?.5:1===c?.8:2===c?1.1:1.2)*
h)>=b)break}else if(h>=b)break}f.length>k&&!(3>l.planes.length&&h>=b)&&(p=getLandBasePlaneObject(f[k]),t=Math.min(l.planes.length,3),(n=l.planes[t])||(n={id:0,ap:0}),n=n.ap,l.planes[t]=p,l.ap+=p.ap-n,updateLBAirPower(l));if(l.ap>=m.ap||l.ap>=b)p=m.ap,m=l,h+=l.ap-p;l=m.planes.length;if(4!==l)for(p=0;p<4-l;p++)m.planes.push({id:0,ap:0});g[k]=m;l={};m=$jscomp.makeIterator(m.planes);for(p=m.next();!p.done;l={$jscomp$loop$prop$isFirst$367:l.$jscomp$loop$prop$isFirst$367,$jscomp$loop$prop$plane$290$368:l.$jscomp$loop$prop$plane$290$368},
p=m.next())l.$jscomp$loop$prop$plane$290$368=p.value,l.$jscomp$loop$prop$isFirst$367=!0,e=e.filter(function(a){return function(b){return a.$jscomp$loop$prop$isFirst$367&&b.id===a.$jscomp$loop$prop$plane$290$368.id?a.$jscomp$loop$prop$isFirst$367=!1:!0}}(l));if(d){if(Math.floor((0===c?.5:1===c?.8:2===c?1.1:1.2)*h)>=b)break}else if(h>=b)break}return g}
...
d.id}))&&(setPlaneDiv(a,d),isCtrlPress||$("#drag_drop_copy").prop("checked")||setPlaneDiv(c,e))}}function cell_type_Changed(a){var b=a.closest(".battle_content"),c=castInt(a.val());b.find(".enemy_content").each(function(a,b){c!==CELL_TYPE.grand&&5<a?$(b).addClass("d-none").removeClass("d-flex"):$(b).removeClass("d-none").addClass("d-flex")});a=b.find(".formation");changeFormationSelectOption(a,c);calculate()}
...
$("#config_content").on("click","#btn_reset_selected_ship_history",btn_reset_selected_ship_history_Clicked);$("#config_content").on("click",'input[name="site_theme"]',function(){site_theme_Changed()});$("#plane_stock").on("change","#stock_type_select",function(){stock_type_select_Changed($(this))});$("#plane_stock").on("click",".stock_tr",function(){stock_tr_Clicked($(this))});$("#plane_stock").on("click","#btn_load_equipment_json",btn_load_equipment_json_Clicked);$("#plane_stock").on("input","#input_equipment_json",
function(){input_equipment_json_Changed($(this))});$("#plane_stock").on("focus","#input_equipment_json",function(){$(this).select()});$("#plane_stock").on("click","#btn_stock_all_clear",btn_stock_all_clear_Clicked);$("#plane_stock").on("input","#stock_word",stock_word_TextChanged);$("#modal_plane_select").on("click",".plane",function(){modal_plane_Selected($(this))});$("#modal_plane_select").on("click",".btn_remove",function(){modal_plane_select_btn_remove_Clicked($(this))});$("#modal_plane_select").on("change",
"#plane_type_select",plane_type_select_Changed);$("#modal_plane_select").on("click","#disp_in_stock",disp_in_stock_Checked_Chenged);$("#modal_plane_select").on("click","#disp_equipped",disp_equipped_Checked_Chenged);$("#modal_plane_select").on("input","#plane_word",plane_word_TextChanged);$("#modal_plane_select").on("change","#plane_sort_select",function(){sort_Changed($(this))});$("#modal_plane_select").on("click","#plane_asc",function(){sort_Changed($(this))});$("#modal_plane_select").on("click",
