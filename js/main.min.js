...
b=$jscomp.makeIterator(setting.defaultProf);for(f=b.next();!f.done;f=b.next())e=f.value,proficiency_Changed($('.init_prof[data-typeid="'+e.id+'"').find('.prof_option[data-prof="'+e.prof+'"]').parent(),!0);if(setting.contentsOrder.length){b=$("#main_contents");e=$("#side_index");var n=[],r=[];$(".contents").each(function(a,b){n.push($(b))});b.empty();e.find("li").each(function(a,b){r.push($(b))});e.empty();f={};g=$jscomp.makeIterator(setting.contentsOrder);for(h=g.next();!h.done;f={$jscomp$loop$prop$id$126$275:f.$jscomp$loop$prop$id$126$275},
h=g.next())f.$jscomp$loop$prop$id$126$275=h.value,h=n.find(function(a){return function(b){return b.attr("id")===a.$jscomp$loop$prop$id$126$275}}(f)),b.append(h),h=r.find(function(a){return function(b){return b.attr("id")==="li_"+a.$jscomp$loop$prop$id$126$275}}(f)),e.append(h)}loadMainPreset();$("#stage2_detail_slot").val(18);$("#stage2_detail_avoid").val(0);setting.simulateCount?$("#calculate_count").val(setting.simulateCount):$("#calculate_count").val(5E3);setting.initialProf120&&(initialProf120Plane=
...
setting.airRaidMax);$("#empty_slot_invisible").prop("checked",setting.emptySlotInvisible);$("#disp_in_stock").prop("checked",setting.inStockOnly);$("#disp_equipped").prop("checked",setting.visibleEquipped);$("#enemy_display_image").prop("checked",setting.enemyDisplayImage);$("#enemy_display_text").prop("checked",!setting.enemyDisplayImage);$("#plane_word").val("");$("#ship_word").val("");$("#enemy_word").val("");$("#stock_word").val("");$("#modal_auto_expand").find(".custom-control-input").prop("checked",
...
b.includes(e.value));e=d.next());c&&a.append('<optgroup label="\u6c34\u4e0a\u6a5f" id="optg_sp">');c=$jscomp.makeIterator(PLANE_TYPE);for(e=c.next();!e.done;e=c.next())d=e.value,b.includes(d.id)&&(e='<option value="'+d.id+'">'+d.name+"</option>",a.find("#"+([1,2,3,4,9].includes(d.id)?"optg_cb":[5,6,7,8].includes(d.id)?"optg_sp":"optg_lb")).append(e))}
