...
function initialize(callback) {
  ...
  if (!localVersion || localVersion != VERSION) {
    ...
    // 変更通知
    $('#modal_version_inform').find('#version').text(VERSION);
...
function createPlaneTable(planes) {
  ...
  for (let i = 0; i < max_i; i++) {
    ...
    if (displayMode === "multi" && prevType != plane.type) {
      ...
      $type.textContent = planeType.find(v => v.id === plane.type).name;
...
function createMapSelect() {
  ...
  const maps = mapData.filter(v => v.world === world)
...
function createNodeSelect() {
  ...
  const nodes = enemyPattern.filter(v => v.world === world && v.map === map)
...
function createEnemyPattern() {
  ...
  const enemies = enemyPattern.find(v => v.world === world && v.map === map && v.name === node).enemies;
  const len = enemies.length;
  let text = '';
  for (let index = 0; index < len; index++) {
    const enemy = enemyData.find(v => v.id === enemies[index]);
    text += `
...
function getShootDownSlotHalf(airStateIndex, slot) {
  const avg_rate = airStatus[airStateIndex].rate / 2;
...
function shootDownHalfFriend(friendFleetData, eap, index) {
  ...
  $('#shoot_down_table').find('.cond.battle' + (index + 1)).text(airStatus[airStatusIndex].abbr).addClass(airStatusColor);
...
$(function () {
  ...
  $(document).on('change', '.ohuda_select', function () {
    ...
    if (ohudaValue === 0) {
      ...
      isDefMode = true;
      // 出撃中の部隊は防空に
      $('.ohuda_select').each((i, e) => { if (Number($(e).val()) > 0) $(e).val(0); })
    }
    else if (ohudaValue > 0) {
      // 防空モード終了
      isDefMode = false;
      // 防空中の部隊は集中に
      $('.ohuda_select').each((i, e) => { if (Number($(e).val()) === 0) $(e).val(2); })
    }
  ...
  // 基地航空隊リセットボタン
  $(document).on('click', '.btn_reset_landBase', function () {
  ...
  // 艦娘装備リセットボタン
  $(document).on('click', '.btn_reset_ship', function () {
  ...
  /* 表示形式変更 */
  $(document).on('click', '.toggle_display_type', function () {
  ...
  // 機体外すボタン
  $(document).on('click', '.lb_plane .btn_remove_plane', function () {
    clearPlaneDiv($(this).closest('.lb_plane'));
    caluclate();
  });
  $(document).on('click', '.ship_plane .btn_remove_plane', function () {
  ...
  $(document).on('click', '.plane_name', function () {
    ...
    if (selectedID) {
      ...
      $('#modal_plane_select_btn_remove').prop('disabled', false);
    }
    else {
      $('#modal_plane_select_btn_remove').prop('disabled', true);
    }
  ...
  $('#modal_plane_select_btn_remove').on('click', function () {
  ...
  // 表示隻数更新
  $('.display_ship_count').change(function () {
  ...
  // 機体プリセットモーダル展開
  $(document).on('click', '.btn_plane_preset', function () {
    let parentId = -1;
  ...
  // プリセット保存押下時
  $('#btn_commit_preset').click(function () {
  ...
  // プリセット展開押下時
  $('#btn_expand_preset').click(function () {
    const presetId = Number($('.preset_selected').data('presetid'));
  ...
  $('#modal_ship_select').on('change', '#shipType_select', function () {
  ...
  // はずすボタンクリック(機体選択モーダル内)
  $('#modal_ship_select_btn_remove').on('click', function () {
    if ($(this).prop('disabled')) return false;
    // 選択状況をリセット
    clearShipDiv($target);
    $('#modal_ship_select').modal('hide');
  ...
  // 基地派遣先変更
  $('#landBase_target').change(caluclate);

  // X戦目の横リセットをクリック
  $(document).on('click', '.btn_reset_battle', function () {
  ...
  $('#modal_enemy_select_btn_remove').on('click', function () {
    if ($(this).prop('disabled')) return false;
    // 選択状況をリセット
  ...
  // 海域選択モーダル展開
  $(document).on('click', '.btn_enemy_preset', function () {
    $target = $(this).closest('.battle_content');
    $('#modal_enemy_pattern').modal('show');
    $(this).blur();
  });
  // 海域変更
  $('#world_select').change(createMapSelect);
  $('#map_select').change(createNodeSelect);
  $('#node_select').change(createEnemyPattern);
  // 機体プリセット押下時
  ...
  // 敵艦展開押下時
  $('#btn_expand_enemies').click(function () {
    // 念のため敵艦隊選択チェック
    if (!$('.enemy_pattern_tr').length) {
      $(this).prop('disabled', true);
    }
    else {
      $target.find('.btn_reset_battle').click();
      $('.enemy_pattern_tr').each((i, e) => {
        setEnemyDiv($target.find('.enemy_content:last()'), Number($(e).data('enemyid')));
      });
  ...
  // 機体選択モーダル終了時
  $('#modal_plane_select').on('hide.bs.modal', () => {
    $target = undefined;
    caluclate();
  });
  // 艦娘選択モーダル終了時
  $('#modal_ship_select').on('hide.bs.modal', () => {
    $target = undefined;
    caluclate();
  });
  // 敵艦選択モーダル終了時
  $('#modal_enemy_select').on('hide.bs.modal', () => {
    $target = undefined;
    caluclate();
  });
  // 機体プリセットモーダル終了時
  $('#modal_plane_preset').on('hide.bs.modal', () => {
    $target = undefined;
    caluclate();
  });
  // 機体プリセットモーダル終了時
  $('#modal_enemy_pattern').on('hide.bs.modal', () => {
    $target = undefined;
    caluclate();
  });
  // 各種モーダル展開終了時
  $('#modal_plane_preset').on('show.bs.modal', () => {
  ...
  // localStrage全削除ボタンクリック
  $('#btn_reset_localStrage').click(() => {
    const $modal = $('#modal_confirm');
    $modal.find('.modal-body').html(`
      <div>ブラウザに保存されている、以下の設定を削除します。</div>
      <div class="mt-3 ml-2">・装備、艦娘、及び敵艦のプリセット情報</div>
