...
function getParentShipNo($element) {
  ...
  if ($shipTab.attr('class')) {
    return castInt($shipTab.attr('id').replace(/shipNo_/g, ''));
  }
  else return NaN;
}
...
/**
 * 引数で渡されたコンテンツ内の艦載機を全てクリアする
 * @param {JqueryDomObject} $div クリアするコンテンツ .contents または #landBase #friendFleet
 */
...
function setPlaneDiv($div, inputPlane = { id: 0, remodel: 0, prof: -1 }, canEditSlot = false) {
  ...
  if ($div.closest('.ship_tab').hasClass('ship_tab')) {
    ...
    let shipId = castInt($div.closest('.ship_tab').data('shipid'));
...
function loadPlanePreset() {
  ...
  $modalParent.find('.preset_tr').removeClass('preset_selected');
  $modalParent.find('.preset_tbody').html(presetText);
  $modalParent.find('.preset_preview_name')
    .val('左のプリセット一覧をクリック')
    .prop('disabled', true);
  $modalParent.find('.preset_preview_tbody').html('');
...
function drawPlanePresetPreview(preset) {
  ...
  let planes = [];
  const $modalParent = $('#modal_plane_preset');
  $modalParent.find('.btn_expand_preset').prop('disabled', !preset);
  $modalParent.find('.btn_delete_preset').prop('disabled', !preset);

  if (preset) {
    $modalParent.find('.preset_preview_name').val(preset.name);
    for (const id of preset.planes) planes.push(PLANE_DATA.find(v => v.id === id));
  }
  else {
    // プリセットが見つからなかったので新規登録画面呼び出し
    $modalParent.find('.preset_preview_name').val('');
  ...
  $modalParent.find('.btn_commit_preset').prop('disabled', true);
  $modalParent.find('.preset_preview_name').prop('disabled', planes.length === 0);
...
function updatePlanePreset() {
  ...
  const planeIds = [];
  $('.preset_preview_tr').each((i, e) => planeIds.push(castInt($(e).data('planeid'))));

  let max_id = 0;
  for (const preset of planePreset) if (max_id < preset.id) max_id = preset.id;
  const newPreset = {
    id: (presetId === -1 ? max_id + 1 : presetId),
    name: $modalParent.find('.preset_preview_name').val().replace(/^\s+|\s+$/g, ''),
    planes: planeIds
  };
 ...
 * 第一引数にデータがなかった場合は新規作成として DOM 構築を行う
 * @param {Object} preset プリセットオブジェクト
 */
function drawMainPresetPreview(preset) {
  let text = '';
  const $modalParent = $('#modal_main_preset');
  $modalParent.find('.btn_expand_preset').prop('disabled', !preset);
  $modalParent.find('.btn_delete_preset').prop('disabled', !preset);
  ...
  $('.lb_tab').each((index, element) => {
    ...
    if (isEmpty) {
      ...
      $lb_tab.find('.ohuda_select option').each((i, e) => {
        if (castInt($(e).val(), -1) !== -1) $(e).prop('disabled', true);
      });
    }
    else $lb_tab.find('.ohuda_select option').prop('disabled', false);

    // 出撃 or 防空
    tmpLandBaseDatum.mode = castInt($lb_tab.find('.ohuda_select').val());
    ...
    drawChangeValue($fuel, castInt($fuel.text()), sumFuel, true);
    drawChangeValue($ammo, castInt($ammo.text()), sumAmmo, true);
    drawChangeValue($bauxite, castInt($bauxite.text()), sumBauxite, true);
    sumFuel = 0;
    sumAmmo = 0;
    sumBauxite = 0;
    ...
    drawChangeValue($target_td, castInt($target_td.text()), tmpLandBaseDatum.ap);
    let $targetSpan = $lb_tab.find('.ap');
    drawChangeValue($targetSpan, castInt($targetSpan.text()), tmpLandBaseDatum.ap);

    // 半径
    const range = getRange(tmpLandBaseDatum);
    $target_td = $target_lb.find('.info_range');
    drawChangeValue($target_td, castInt($target_td.text()), range);
    $targetSpan = $lb_tab.find('.range');
    drawChangeValue($targetSpan, castInt($targetSpan.text()), range);

    // 半径が足りていないなら文言追加
    if (tmpLandBaseDatum.mode > 0) {
      $('#battle_container').find('.enemy_range').each((i, e) => {
        if (i === castInt($('#landBase_target').val()) - 1 && range < castInt($(e).text())) {
  ...
  if (visiblePlaneCount > 0) {
    ...
    if (isDefMode) {
      ...
      $('#lb_info_table tbody').find('.lb_info_tr:first').append(`
        ...
        <td class="info_defAp info_defApEx" rowspan="${visiblePlaneCount}">${Math.floor(sumAp_ex)}</td>
      `);
    }
    else {
    ...
    $('#lb_info_table').find('.info_warning').addClass('d-none');
  }
  else {
    $('.lb_range_warning').addClass('d-none');
    $('#lb_info_table').find('.info_warning').removeClass('d-none');
  }
}
...
function createLBPlaneObject($lb_plane) {
  const id = castInt($lb_plane.data('planeid'));
  const type = castInt($lb_plane.data('type'));
  // undefined来る可能性あり
...
function updateFriendFleetInfo(friendFleetData) {
  ...
  $('.ship_tab').each((index, element) => {
    ...
    const prevAp = castInt(txtAP.text());
    let sumAp = 0;
    let exist = false;
    let planeCount = 0;
    // 総制空値と艦載機の有無チェック
    ...
    if (planeCount > 0) {
      ...
      let text = '';
      let info_table_text = '';
      let shipId = castInt($this.data('shipid'));
  ...
  else $('#ship_info_table').find('.info_warning').removeClass('d-none');
}
...
function createFleetPlaneObject($ship_plane, shipNo, index) {
  const id = castInt($ship_plane.data('planeid'));
  const type = castInt($ship_plane.data('type'));
  // undefined来る可能性あり
  const plane = getPlanes(type).find(v => v.id === id);
...
function updateEnemyFleetInfo(enemyFleetData) {
  ...
  $('.battle_content').each(function () {
    ...
    $(this).find('.enemy_content').each(function () {
      let enemyId = $(this).data('enemyid');
      if (isNaN(enemyId)) enemyId = 0;
      const tmpEnemy = createEnemyObject(enemyId);

      // 直接入力の制空値を代入
      if (enemyId === -1) {
        let inputAp = castInt($(this).find('.enemy_ap').text());
        ...
        $(this).find('.enemy_ap').text(inputAp);
      }
    ...
    if (!castInt($enmAp.text())) $enmAp.addClass('font_color_ccc');
...
function getShootDownSlotHalfFF(index, slot) {
  return SHOOT_DOWN_TABLE_AVE[Math.ceil(slot)][index];
}
...
function ship_plane_DragOver($this, ui) {
  ...
  const shipID = castInt($this.closest('.ship_tab').data('shipid'));
  const planeID = castInt($original.data('planeid'));
  const type = castInt($original.data('type'));
  // 挿入先が装備不可だった場合暗くする
...
function ship_plane_Drop($this, ui) {
  ...
  if (ui.draggable.closest('.ship_plane').data('planeid')) {
    ...
    // 挿入先が装備不可だった場合中止
    let shipID = castInt($this.closest('.ship_tab').data('shipid'));
...
function enemy_ap_Clicked($this) {
  ...
  // 敵艦選択時は変更不可
  const enemyId = castInt($this.closest('.enemy_content').data('enemyid'));
...
function plane_name_Clicked($this) {
  ...
  const selectedID = castInt($target.data('planeid'));
...
function modal_plane_Selected($this) {
  const plane = { id: castInt($this.data('planeid')) };
...
function btn_plane_preset_Clicked($this) {
  ...
  if (!$target.attr('class')) {
    ...
    parentId = castInt($target.data('shipid'));
  }
...
function ship_name_span_Clicked($this) {
  $target = $this.closest('.ship_tab');
  const selectedID = castInt($target.data('shipid'));
...
function modal_ship_Selected($this) {
  setShipDiv($target, castFloat($this.data('shipid')));
...
function modal_enemy_Selected($this) {
  // 敵艦セット
  setEnemyDiv($target, castInt($this.data('enemyid')));
...
function btn_reset_localStrage_Clicked() {
  ...
  $modal.find('.modal-body').html(`
    ...
    <div class="ml-2">・シミュレート回数</div>
    <div class="ml-2">・一覧画面の表示形式</div>
    <div class="ml-2">・etc...</div>
...
function btn_commit_main_preset_Clicked() {
  ...
  $('.lb_plane').each((i, e) => {
    const $e = $(e);
    const plane = {
      id: castInt($e.data('planeid')),
      prof: castInt($e.data('prof')),
      slot: castInt($e.find('.slot').text()),
      remodel: castInt($e.find('.remodel_value').text()),
      index: i
    };
    // 装備があれば追加
    if (plane.id !== 0) landBasePreset.push(plane);
  });
  ...
  $('.ship_tab').each((i, e) => {
    ...
    const ship = { id: castInt($(e).data('shipid')), planes: [], index: shipIndex }
    $(e).find('.ship_plane').each((j, ce) => {
      const $ce = $(ce);
      const plane = {
        id: castInt($ce.data('planeid')),
        prof: castInt($ce.data('prof')),
        slot: castInt($ce.find('.slot').text()),
        remodel: castInt($ce.find('.remodel_value').text()),
        index: j
      };
      // 装備されていない場合飛ばす
      if (plane.id !== 0) ship.planes.push(plane);
    });

    // 艦娘か機体が1件でもあれば追加
    if (ship.id !== 0 || ship.planes.length !== 0) {
  ...
  $('.battle_content').each((i, e) => {
    ...
    const enemyFleet = { battle: i, enemies: [] };
    $(e).find('.enemy_content').each((j, ce) => {
      const enemy = {
        id: castInt($(ce).data('enemyid')),
        ap: castInt($(ce).find('.enemy_ap').text()),
      };
      // 敵艦じゃなければ飛ばす
      if (enemy.id !== 0) enemyFleet.enemies.push(enemy);
    });

    // 空じゃなければ追加
    if (enemyFleet.enemies.length !== 0) enemyFleetPreset.push(enemyFleet);
  });

  mainPreset.landBase = landBasePreset;
  mainPreset.friendFleet = friendFleetPreset;
  mainPreset.enemyFleet = enemyFleetPreset;
}
...
function btn_expand_main_preset_Clicked() {
  // 基地航空隊展開
  $('.lb_plane').each((i, e) => {
    // i と plane.index の一致したデータを挿入
    const plane = mainPreset.landBase.find(v => v.index === i);
    if (plane) setLBPlaneDiv($(e), plane);
  });

  // 艦娘展開
  // 必要な表示数算出 (2 <= x <= 6)
  const displayShip = Math.min(Math.max(mainPreset.friendFleet.length, 2), 6);
  // クリア
  clearShipDivAll((displayShip % 2 > 1 ? displayShip + 1 : displayShip));

  $('.ship_tab').each((i, e) => {
    const ship = mainPreset.friendFleet.find(v => v.index === i);
    if (!ship) return;
    setShipDiv($(e), ship.id);
    $(e).find('.ship_plane').each((j, e) => {
      const plane = ship.planes.find(v => v.index === j);
      if (plane) setPlaneDiv($(e), plane, true);
    });
  });

  // 敵艦展開
  const battle = mainPreset.enemyFleet.length;
  // クリア
  clearEnemyDivAll(battle);
  $('#battle_count').val(battle);
  $('.battle_content').each((i, e) => {
    const enemyFleet = mainPreset.enemyFleet.find(v => v.battle === i);
    if (enemyFleet) {
      for (const enemy of enemyFleet.enemies) {
        setEnemyDiv($(e).find('.enemy_content:last()'), enemy.id, enemy.ap);
      }
    }
  });
...
$(function () {
  ...
  $(document).on('input', '.preset_preview_name', function () { preset_preview_name_Changed($(this)); });
  ...
  $('#modal_main_preset').on('click', '.btn_commit_preset', function () { btn_commit_main_preset_Clicked($(this)); });
  $('#modal_main_preset').on('click', '.btn_delete_preset', function () { btn_delete_main_preset_Clicked($(this)); });
