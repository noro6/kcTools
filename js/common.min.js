...
[],castInt(w.replace("s",""))-1+6*n,0,x];Object.keys(m.items).forEach(function(q){var u=m.items[q];if(u&&u.hasOwnProperty("id")&&ITEM_DATA.find(function(t){return t.id===castInt(u.id)})){q=castInt(q.replace("i",""))-1;var A=0;-1>q||q>=p.slot.length?q=-1:A=castInt(p.slot[q]);var y=[0,0,0,A,q];Object.keys(u).forEach(function(t){"id"===t?y[0]=castInt(u[t]):"mas"===t?y[1]=castInt(u[t]):"rf"===t&&(y[2]=castInt(u[t]))});z[1].push(y)}});r[1].push(z)}});c.push(r)}}}});var e=c.find(function(h){return 0===
h[0]})[1];if(2<=c.length){var f=c.find(function(h){return 1===h[0]})[1],g=e.concat(f);return[d,g,[]]}return[d,e,[]]}catch(h){return null}}
