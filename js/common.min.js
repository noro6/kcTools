...
(setting.planeFilter={landBase:["radius",0],friendFleet:["antiAir",0]});adaptUpdater();saveSetting();setting.reducedDisplay&&reduced_display_Clicked()}
...
castInt(w.replace("i",""))-1+4*l];Object.keys(m).forEach(function(p){"id"===p?v[0]=castInt(m[p]):"mas"===p?v[1]=castInt(m[p]):"rf"===p&&(v[2]=castInt(m[p]))});d[0].push(v)}})}if(0===h.indexOf("f")){var n=castInt(h.replace("f",""))-1;if(!(1<n)){var q=[n,[]];Object.keys(k).forEach(function(w){var m=k[w];if(m&&m.hasOwnProperty("items")){var v=castInt(m.id),p=SHIP_DATA.find(function(t){return t.api===v});p||(p={id:0,slot:[0,0,0,0]});var x=99;m.hasOwnProperty("lv")&&(x=(x=castInt(m.lv))?x:99);var z=[p.id,
[],castInt(w.replace("s",""))-1+6*n,0,x];Object.keys(m.items).forEach(function(t){var u=m.items[t];if(u&&u.hasOwnProperty("id")&&PLANE_DATA.find(function(r){return r.id===castInt(u.id)})){t=castInt(t.replace("i",""))-1;var y=[0,0,0,p.slot[t],t];Object.keys(u).forEach(function(r){"id"===r?y[0]=castInt(u[r]):"mas"===r?y[1]=castInt(u[r]):"rf"===r&&(y[2]=castInt(u[r]))});z[1].push(y)}});q[1].push(z)}});c.push(q)}}}});var e=c.find(function(h){return 0===h[0]})[1];if(2<=c.length){var f=c.find(function(h){return 1===
h[0]})[1],g=e.concat(f);return[d,g,[]]}return[d,e,[]]}catch(h){return null}}
function readShipJson(a){try{for(var b=JSON.parse(a),c=loadShipStock(),d=$jscomp.makeIterator(c),e=d.next();!e.done;e=d.next())e.value.num=0;a={};for(var f=$jscomp.makeIterator(b),g=f.next();!g.done;a={$jscomp$loop$prop$shipId$24:a.$jscomp$loop$prop$shipId$24},g=f.next()){var h=g.value;if(!h.hasOwnProperty("api_ship_id"))return!1;a.$jscomp$loop$prop$shipId$24=h.api_ship_id;var k=c.find(function(l){return function(n){return n.deck===l.$jscomp$loop$prop$shipId$24}}(a));k&&k.num++}c.sort(function(l,
...
n.$jscomp$loop$prop$planeId$26}}(a));l&&l.num[k]++}c.sort(function(n,q){return n.id-q.id});saveLocalStorage("planeStock",c);setting.inStockOnly=!0;saveSetting()}catch(n){return!1}return!0}function decodePreset(a){try{var b=b64_to_utf8(a);return JSON.parse(b)}catch(c){return[[],[],[]]}}
...
function setTab(){var a=loadLocalStorage("activePresets");a||(a={activeId:"",presets:[]});for(var b=loadLocalStorage("presets"),c=document.createDocumentFragment(),d={},e=$jscomp.makeIterator(a.presets),f=e.next();!f.done;d={$jscomp$loop$prop$tabData$32:d.$jscomp$loop$prop$tabData$32},f=e.next()){d.$jscomp$loop$prop$tabData$32=f.value;var g=b?b.find(function(n){return function(q){return q[0]===n.$jscomp$loop$prop$tabData$32.id}}(d)):null;f=document.createElement("div");f.className="d-flex fleet_tab "+
