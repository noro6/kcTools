...
l="bar_ex_status"+f.$jscomp$loop$prop$status$332;isDefMode&&(k=$("#simple_lb_def_bar"),n=k.find(".result_bar"),x=n.data("airstatus"),n.removeClass("bar_status"+x+" bar_ex_status"+x).addClass(t+" "+l).css({width:q+"%"}).data("airstatus",f.$jscomp$loop$prop$status$332),k.find(".simple_label").text(AIR_STATUS.find(function(v){return function(r){return r.id===v.$jscomp$loop$prop$status$332}}(f)).abbr),k.removeClass("d-none"));7===h&&u&&-1<f.$jscomp$loop$prop$status$332&&(h=$("#fleet_simple_bar"),u=h.find(".result_bar"),
x=u.data("airstatus"),u.removeClass("bar_status"+x+" bar_ex_status"+x).addClass(t+" "+l).css({width:q+"%"}).data("airstatus",f.$jscomp$loop$prop$status$332),m=AIR_STATUS.find(function(v){return function(r){return r.id===v.$jscomp$loop$prop$status$332}}(f)).abbr+"("+Math.floor(100*m[f.$jscomp$loop$prop$status$332])+"%)",h.find(".simple_label").text(m),h.removeClass("d-none"));if(isDefMode)break}for(e=d=c=b=0;e<resultData.fleetSlots.length;e++)if(f=resultData.fleetSlots[e],g=100*f.deathRate,m=document.getElementById("s"+
f.shipNo+"sl"+f.slot))q=castInt(m.getElementsByClassName("battle1")[0].textContent),m.getElementsByClassName("battle_end")[0].textContent=Math.round(f.avg),m.getElementsByClassName("battle_death")[0].textContent=0===f.deathRate?"-":10<=g?g.toFixed(1)+" %":Math.ceil(10*g)/10+" %",0<q&&(b+=5*(q-f.avg),c+=5*(q-f.max),d+=5*(q-f.min));resultData.usedSteels?(c=resultData.usedSteels/a,$(".battle_end.fap").html()||$(".battle_end.fap").html("\u6d88\u8cbb\u4e88\u6e2c"),$(".battle_end.eap").html('\n\t\t<div><img src="../img/util/bauxite.png" class="img-size-18" alt="\u30dc\u30fc\u30ad\u30b5\u30a4\u30c8">: '+
b.toFixed(1)+'</div>\n\t\t<div><img src="../img/util/steel.png" class="img-size-18" alt="\u92fc\u6750">: '+c.toFixed(1)+"</div>\n\t\t")):($(".battle_end.fap").html()||$(".battle_end.fap").html('<img src="../img/util/bauxite.png" class="img-size-18" alt="\u30dc\u30fc\u30ad\u30b5\u30a4\u30c8">\u6d88\u8cbb\u4e88\u6e2c'),$(".battle_end.eap").html(c+" ~ "+d+" ( "+b.toFixed(1)+" )"));for(b=0;b<resultData.enemySlots.length;b++)c=resultData.enemySlots[b],d=document.getElementsByClassName("enemy_no_"+c[0]+
" slot_"+c[1])[0],e=Math.floor(c[2]/a*10),d.getElementsByClassName("td_avg_slot")[0].textContent=0===e?"0":e/10,d.getElementsByClassName("td_max_slot")[0].textContent=c[3],d.getElementsByClassName("td_min_slot")[0].textContent=c[4];if(!isDefMode&&document.getElementById("adapt_stage2").checked){for(b=0;b<resultData.enemySlotAllDead.length;b++)c=Math.round(resultData.enemySlotAllDead[b]/a*1E3)/10,document.getElementsByClassName("enemy_no_"+b+" slot_0")[0].getElementsByClassName("td_all_dead")[0].textContent=
Math.floor(c)+"%";document.getElementById("enemy_slot_result_mode").textContent="\uff08\u822a\u7a7a\u6226 stage2 \u5f8c\uff09"}else document.getElementById("enemy_slot_result_mode").textContent="\uff08\u822a\u7a7a\u6226 stage1 \u5f8c\uff09";c=resultData.enemyAirPowerResults[0];b=resultData.enemyAirPowerResults[1];if(c.length)for(c=getEnemyApDistribution(c),d=document.getElementById("enemy_ap_bf_dist_min"),e=$jscomp.makeIterator(c),c=e.next();!c.done;c=e.next())d.textContent=c.value,d=d.nextElementSibling;
if(b.length)for(c=getEnemyApDistribution(b),b=document.getElementById("enemy_ap_af_dist_min"),d=$jscomp.makeIterator(c),c=d.next();!c.done;c=d.next())b.textContent=c.value,b=b.nextElementSibling;b=$jscomp.makeIterator(inputItems.landBase.landBases);for(c=b.next();!c.done;c=b.next())if(c=c.value,e=c.target[0],f=c.target[1],-1!==e&&-1!==f&&!isDefMode)for(d=e<=f?[e,f]:[f,e],e=e<=f?[0,1]:[1,0],f={},g=0;1>=g;f={$jscomp$loop$prop$status$189$334:f.$jscomp$loop$prop$status$189$334},g++)m=c.results[d[g]].airStatusIndex[e[g]],
t=c.results[d[g]].mainAirPower[e[g]]/a,h=c.results[d[g]].enemyAirPower[e[g]]/a,f.$jscomp$loop$prop$status$189$334=m.indexOf(getArrayMax(m)),q=0,h=getAirStatusBorder(h),4===f.$jscomp$loop$prop$status$189$334?q=t/h[3]*10:3===f.$jscomp$loop$prop$status$189$334?q=t/h[2]*20:2===f.$jscomp$loop$prop$status$189$334?q=t/h[1]*45:1>=f.$jscomp$loop$prop$status$189$334&&(q=t/h[0]*90),5===f.$jscomp$loop$prop$status$189$334&&(f.$jscomp$loop$prop$status$189$334=0,q=100),h="bar_status"+f.$jscomp$loop$prop$status$189$334,
u="bar_ex_status"+f.$jscomp$loop$prop$status$189$334,t=$("#simple_lb_bar_"+c.baseNo+"_"+(g+1)),l=t.find(".result_bar"),k=l.data("airstatus"),l.removeClass("bar_status"+k+" bar_ex_status"+k).addClass(h+" "+u).css({width:(100<q?100:q)+"%"}).data("airstatus",f.$jscomp$loop$prop$status$189$334),q=AIR_STATUS.find(function(v){return function(r){return r.id===v.$jscomp$loop$prop$status$189$334}}(f)).abbr,m=Math.floor(getArrayMax(m)/a*100),m=q+"("+m+"%)"+(100===m?"":10<=m?'<span class="opacity0">1</span>':
'<span class="opacity0">11</span>'),t.find(".simple_label").html(m),t.removeClass("d-none");display_result_Changed();resultData.enemyAirPowerResults=null;resultData.fleetSlots=null}
