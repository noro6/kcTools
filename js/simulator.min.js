...
g.callWhenSettled_(h.resolve,h.reject)};e.prototype.settleSameAsThenable_=function(g,h){var l=this.createResolveAndReject_();try{g.call(h,l.resolve,l.reject)}catch(k){l.reject(k)}};e.prototype.then=function(g,h){function l(q,p){return"function"==typeof q?function(u){try{k(q(u))}catch(t){m(t)}}:p}var k,m,n=new e(function(q,p){k=q;m=p});this.callWhenSettled_(l(g,k),l(h,m));return n};e.prototype.catch=function(g){return this.then(void 0,g)};e.prototype.callWhenSettled_=function(g,h){function l(){switch(k.state_){case 1:g(k.result_);
...
m){function n(u){return function(t){q[u]=t;p--;0==p&&k(q)}}var q=[],p=0;do q.push(void 0),p++,c(l.value).callWhenSettled_(n(q.length-1),m),l=h.next();while(!l.done)})};return e},"es6","es3");$jscomp.findInternal=function(a,b,c){a instanceof String&&(a=String(a));for(var d=a.length,e=0;e<d;e++){var f=a[e];if(b.call(c,f,e,a))return{i:e,v:f}}return{i:-1,v:void 0}};$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(b,c){return $jscomp.findInternal(this,b,c).v}},"es6","es3");
...
function setPlaneTypeIconSelect(a,b){a.innerHTML=(void 0===b?0:b)?"":'\n\t\t<div class="general_tr item_type active p-2" data-type="0">\n\t\t\t<img src="../img/type/all.png" class="img-size-32" alt="\u5168\u3066">\n\t\t</div>';b=document.createDocumentFragment();for(var c={},d=$jscomp.makeIterator([6,7,8,9,57,10,45,41,1,2,3,4,5,12,14,21,24,47,48,49,17]),e=d.next();!e.done;c={$jscomp$loop$prop$typeId$300:c.$jscomp$loop$prop$typeId$300},e=d.next()){c.$jscomp$loop$prop$typeId$300=e.value;e=ITEM_API_TYPE.find(function(l){return function(k){return k.id===
l.$jscomp$loop$prop$typeId$300}}(c));var f=document.createElement("div");f.className="general_tr item_type p-2";f.dataset.type=e.id;var g=e.name;switch(c.$jscomp$loop$prop$typeId$300){case 10:g="\u6c34\u4e0a\u5075\u5bdf\u6a5f / \u6c34\u4e0a\u7206\u6483\u6a5f";break;case 12:g="\u96fb\u63a2";break;case 14:g="\u5bfe\u6f5c\u88c5\u5099";break;case 24:g="\u8f38\u9001 / \u4e0a\u9678\u7528\u5175\u88c5";break;case 48:g="\u5c40\u5730\u6226\u95d8\u6a5f / \u9678\u8ecd\u6226\u95d8\u6a5f";break;case 17:g="\u305d\u306e\u4ed6"}var h=
...
[14,15].includes(b)?t=["asw","fire","accuracy","avoid2","armor"]:[5,22,32].includes(b)?t=["torpedo","fire","accuracy","avoid2","armor"]:21===b?t=["antiAir","antiAirWeight","antiAirBonus","fire","torpedo"]:LB_ATTACKERS.includes(b)?t=["antiAir","radius","torpedo","avoid","cost"]:48===b&&(t=["antiAir","radius","battleAntiAir","defenseAntiAir","cost"]);$target&&!$target.hasClass("ship_plane")&&(t[1]="radius",t[4]="cost",0===b&&(t=["antiAir","radius","battleAntiAir","cost","avoid"]));var x=0;$("#plane_table_thead").find(".plane_td_basic").each(function(M,
J){$(J).text(convertHeaderText(t[x]));$(J)[0].dataset.sortkey=t[x++];q===$(J)[0].dataset.sortkey?$(J).addClass("active"):$(J).removeClass("active")});"multi"===g?(c.addClass("modal-xl"),c.removeClass("modal-lg"),d.addClass("multi_mode"),c.find(".scroll_thead").addClass("d-none").removeClass("d-flex")):(c.addClass("modal-lg"),c.removeClass("modal-xl"),d.removeClass("multi_mode"),c.find(".scroll_thead").addClass("d-flex").removeClass("d-none"));l?($("#divide_stock").closest("div").removeClass("d-none"),
$("#disp_equipped").closest("div").removeClass("d-none"),$("#disp_in_stock").prop("checked",!0)):($("#divide_stock").closest("div").addClass("d-none"),$("#disp_equipped").closest("div").addClass("d-none"),$("#disp_in_stock").prop("checked",!1));b=a.length;var v=0;c=0;d=-1;for(var r=0,w={},B=0;B<b;w={$jscomp$loop$prop$item$304:w.$jscomp$loop$prop$item$304},B++)if(w.$jscomp$loop$prop$item$304=a[B],!n||setting.favoritePlane.includes(w.$jscomp$loop$prop$item$304.id)){if(l&&h){var D=p.find(function(M){return function(J){return J.id===
M.$jscomp$loop$prop$item$304.id}}(w));if(k){if(-1===w.$jscomp$loop$prop$item$304.stock)continue;d=w.$jscomp$loop$prop$item$304.stock-(D?D.num[w.$jscomp$loop$prop$item$304.remodel]:0);if(!m&&1>d)continue;c=w.$jscomp$loop$prop$item$304.remodel}else{if(0<=w.$jscomp$loop$prop$item$304.stock)continue;var y=h.find(function(M){return function(J){return J.id===M.$jscomp$loop$prop$item$304.id}}(w)),C=D?getArraySum(D.num):0;r=y?getArraySum(y.num):0;if(0===r||!m&&C>=r)continue;d=r-C;for(r=10;0<=r;r--)if(D&&
(y.num[r]-=D.num[r]),0<y.num[r]){c=r;break}}}else if(0<w.$jscomp$loop$prop$item$304.remodel||0<=w.$jscomp$loop$prop$item$304.stock)continue;y="";y="multi"===g?"tr_multi px-1":"px-2";D=document.createElement("div");D.className="plane plane_tr general_tr d-flex py-2 py-lg-1 "+y;D.dataset.planeid=w.$jscomp$loop$prop$item$304.id;D.dataset.type=w.$jscomp$loop$prop$item$304.type;D.dataset.remodel=c;l&&m&&0>=d&&(D.classList.remove("plane_tr","plane","general_tr"),D.classList.add("plane_tr_disabled"));y=
document.createElement("div");y.className="align-self-center size-25 relative";r=document.createElement("canvas");C=r.getContext("2d");r.width=25;r.height=25;C.drawImage(IMAGES["type"+w.$jscomp$loop$prop$item$304.itype],0,0,25,25);if(specialOperator.enabled){C=!1;for(var z=$jscomp.makeIterator(specialOperator.specialInfo),A=z.next();!A.done;A=z.next())if(A=A.value,A.items.includes(w.$jscomp$loop$prop$item$304.id)){C=document.createElement("div");C.className="item_special flash";C.innerHTML=A.name+
'<span class="font_size_12">'+A.remarks+"</span>";y.appendChild(C);C=!0;break}if(u&&!C)continue}C=document.createElement("div");C.className="pl-1 d-flex align-self-center flex-grow-1";z=!1;if(16===w.$jscomp$loop$prop$item$304.itype&&8<=w.$jscomp$loop$prop$item$304.antiAir)z=!0;else if(21===w.$jscomp$loop$prop$item$304.type&&9<=w.$jscomp$loop$prop$item$304.antiAir)z=!0;else if(350===w.$jscomp$loop$prop$item$304.id||351===w.$jscomp$loop$prop$item$304.id||352===w.$jscomp$loop$prop$item$304.id)z=!0;A=
document.createElement("div");A.className="pl-1 plane_td_name align-self-center"+(z?" special_item":"");A.textContent=w.$jscomp$loop$prop$item$304.name;C.appendChild(A);z=document.createElement("div");z.className="ml-2 plane_td_stock align-self-center";z.textContent="\u00d7"+(0<d?d:0);A=document.createDocumentFragment();var E=document.createElement("div");E.className="ml-auto plane_td_basic align-self-center";var F=document.createElement("div");F.className="plane_td_basic align-self-center";var G=
document.createElement("div");G.className="plane_td_basic align-self-center d-none d-lg-block";var I=document.createElement("div");I.className="plane_td_basic align-self-center d-none d-lg-block";var K=document.createElement("div");K.className="plane_td_basic align-self-center d-none d-lg-block";E.textContent=sFormat(w.$jscomp$loop$prop$item$304[t[0]],t[0]);F.textContent=sFormat(w.$jscomp$loop$prop$item$304[t[1]],t[1]);G.textContent=sFormat(w.$jscomp$loop$prop$item$304[t[2]],t[2]);I.textContent=sFormat(w.$jscomp$loop$prop$item$304[t[3]],
t[3]);K.textContent=sFormat(w.$jscomp$loop$prop$item$304[t[4]],t[4]);if("multi"===g&&v!==w.$jscomp$loop$prop$item$304.type&&"default"===q){v=document.createElement("div");v.className="type_divider mt-3 mb-1";var L=document.createElement("div");L.className="font_size_12 font_color_half align-self-center";L.textContent=ITEM_API_TYPE.find(function(M){return function(J){return J.id===M.$jscomp$loop$prop$item$304.type}}(w)).name;var H=document.createElement("div");H.className="flex-grow-1 border-bottom align-self-center mx-2";
v.appendChild(L);v.appendChild(H);f.appendChild(v)}y.appendChild(r);D.appendChild(y);k&&0<w.$jscomp$loop$prop$item$304.remodel&&(v=document.createElement("div"),v.className="plane_td_remodel text_remodel align-self-center",v.dataset.remodel=w.$jscomp$loop$prop$item$304.remodel,y="",y="multi"===g?"\u2605"+w.$jscomp$loop$prop$item$304.remodel:10>w.$jscomp$loop$prop$item$304.remodel?"\u2605+"+w.$jscomp$loop$prop$item$304.remodel:"\u2605MAX",v.textContent=y,C.appendChild(v),C.appendChild(z));0<d&&C.appendChild(z);
D.appendChild(C);"single"===g&&(A.appendChild(E),A.appendChild(F),A.appendChild(G),A.appendChild(I),A.appendChild(K),D.appendChild(A));f.appendChild(D);v=w.$jscomp$loop$prop$item$304.type}f.childNodes.length||(a=document.createElement("div"),a.className="text-center",l&&!m&&(a.textContent+="\u88c5\u5099\u6240\u6301\u6570\u304c\u767b\u9332\u3055\u308c\u3066\u3044\u306a\u3044\u304b\u3001\u5168\u3066\u306e\u88c5\u5099\u304c\u914d\u5099\u6e08\u307f\u3067\u3059\u3002"),l&&!m&&(a.textContent+="\u88c5\u5099\u6240\u6301\u6570\u304c\u767b\u9332\u3055\u308c\u3066\u3044\u306a\u3044\u304b\u3001\u5168\u3066\u306e\u88c5\u5099\u304c\u914d\u5099\u6e08\u307f\u3067\u3059\u3002"));
...
l;saveSetting();g?$("#disp_equipped_ship").closest("div").removeClass("d-none"):$("#disp_equipped_ship").closest("div").addClass("d-none");var q=SHIP_DATA.filter(function(z){return e?z.name.includes(e):n.includes(z.type)&&(d?0<z.final:!0)});if(l){var p=setting.favoriteShip;q=q.filter(function(z){return p.includes(z.id)})}if(g){l={};m=$jscomp.makeIterator(k);for(var u=m.next();!u.done;l={$jscomp$loop$prop$stock$306:l.$jscomp$loop$prop$stock$306,$jscomp$loop$prop$baseLv$307:l.$jscomp$loop$prop$baseLv$307,
$jscomp$loop$prop$baseArea$308:l.$jscomp$loop$prop$baseArea$308},u=m.next())if(l.$jscomp$loop$prop$stock$306=u.value,!(0>=l.$jscomp$loop$prop$stock$306.details.length||(l.$jscomp$loop$prop$stock$306.details.sort(function(z,A){return z.lv-A.lv}),u=q.findIndex(function(z){return function(A){return A.id===z.$jscomp$loop$prop$stock$306.id}}(l)),0>u))){var t=q[u];l.$jscomp$loop$prop$baseLv$307=l.$jscomp$loop$prop$stock$306.details[0].lv;var x=l.$jscomp$loop$prop$stock$306.details[0].st[4];l.$jscomp$loop$prop$baseArea$308=
l.$jscomp$loop$prop$stock$306.details[0].area<=MAX_AREA&&0<l.$jscomp$loop$prop$stock$306.details[0].area?l.$jscomp$loop$prop$stock$306.details[0].area:-1;t.lv=l.$jscomp$loop$prop$baseLv$307;t.area=l.$jscomp$loop$prop$baseArea$308;t.upLuck=x;t.stock=l.$jscomp$loop$prop$stock$306.details.filter(function(z){return function(A){return A.lv===z.$jscomp$loop$prop$baseLv$307&&A.area===z.$jscomp$loop$prop$baseArea$308}}(l)).length;x=[{lv:l.$jscomp$loop$prop$baseLv$307,area:l.$jscomp$loop$prop$baseArea$308}];
if(2<=l.$jscomp$loop$prop$stock$306.details.length)for(var v={},r=1;r<l.$jscomp$loop$prop$stock$306.details.length;v={$jscomp$loop$prop$lv$312:v.$jscomp$loop$prop$lv$312,$jscomp$loop$prop$area$313:v.$jscomp$loop$prop$area$313},r++){v.$jscomp$loop$prop$lv$312=l.$jscomp$loop$prop$stock$306.details[r].lv;var w=l.$jscomp$loop$prop$stock$306.details[r].st[4];v.$jscomp$loop$prop$area$313=l.$jscomp$loop$prop$stock$306.details[r].area<=MAX_AREA&&0<l.$jscomp$loop$prop$stock$306.details[r].area?l.$jscomp$loop$prop$stock$306.details[r].area:
-1;if(!x.find(function(z){return function(A){return A.lv===z.$jscomp$loop$prop$lv$312&&A.area===z.$jscomp$loop$prop$area$313}}(v))){var B=Object.assign({},t);B.lv=v.$jscomp$loop$prop$lv$312;B.area=v.$jscomp$loop$prop$area$313;B.stock=l.$jscomp$loop$prop$stock$306.details.filter(function(z){return function(A){return A.lv===z.$jscomp$loop$prop$lv$312&&A.area===z.$jscomp$loop$prop$area$313}}(v)).length;B.upLuck=w;q.splice(u,0,B);x.push({lv:v.$jscomp$loop$prop$lv$312,area:v.$jscomp$loop$prop$area$313})}}}}if(f&&
setting.selectedHistory[1]){var D=setting.selectedHistory[1];q.sort(function(z,A){return D.filter(function(E){return E===A.id}).length-D.filter(function(E){return E===z.id}).length})}q.sort(function(z,A){return 7===z.type&&7!==A.type||7===A.type&&7!==z.type?A.type-z.type:6===z.type&&6!==A.type||6===A.type&&6!==z.type?A.type-z.type:10===z.type&&10!==A.type||10===A.type&&10!==z.type?A.type-z.type:z.type-A.type});l=document.createDocumentFragment();u=x=0;m={};q=$jscomp.makeIterator(q);for(t=q.next();!t.done;m=
{$jscomp$loop$prop$ship$317:m.$jscomp$loop$prop$ship$317},t=q.next()){m.$jscomp$loop$prop$ship$317=t.value;v=1;if(g&&k){if(!m.$jscomp$loop$prop$ship$317.lv)continue;m.$jscomp$loop$prop$ship$317.stock&&(v=m.$jscomp$loop$prop$ship$317.stock);t=usedShip.filter(function(z){return function(A){return A.id===z.$jscomp$loop$prop$ship$317.id&&A.lv===z.$jscomp$loop$prop$ship$317.lv&&A.area===z.$jscomp$loop$prop$ship$317.area}}(m)).length;v-=t;if(0>=v&&!h)continue}t=document.createElement("div");t.className=
...
r.className="ml-1 align-self-center pt-1";w=document.createElement("img");w.width=120;w.height=30;w.src="../img/ship/"+m.$jscomp$loop$prop$ship$317.id+".png";r.appendChild(w);g&&0<m.$jscomp$loop$prop$ship$317.area&&(r.classList.add("exist_area"),w=document.createElement("img"),w.className="area_banner",w.src="../img/util/area"+m.$jscomp$loop$prop$ship$317.area+".png",r.appendChild(w));w=document.createElement("div");w.className="ml-1 align-self-center";B=document.createElement("div");B.className=
"text-primary font_size_11";B.textContent=g?"Lv : "+m.$jscomp$loop$prop$ship$317.lv:"ID : "+m.$jscomp$loop$prop$ship$317.id;var y=document.createElement("div");y.className="d-flex";y.textContent=m.$jscomp$loop$prop$ship$317.name;var C=document.createElement("div");C.className="ml-1 ship_td_stock align-self-center "+(g?"":"d-none");C.textContent="\u00d7"+v;y.appendChild(C);w.appendChild(B);w.appendChild(y);t.appendChild(r);t.appendChild(w);if("multi"!==c)for(v=0;5>v;v++)r=document.createElement("div"),
r.className="ship_td_slot font_size_12 align-self-center "+(0===v?"ml-auto":""),r.textContent=v<m.$jscomp$loop$prop$ship$317.slot.length?m.$jscomp$loop$prop$ship$317.slot[v]:"",t.appendChild(r);x!==m.$jscomp$loop$prop$ship$317.type&&(x=document.createElement("div"),x.className="type_divider mt-4",v=document.createElement("div"),v.className="align-self-center font_size_16",v.textContent=API_SHIP_TYPE.find(function(z){return function(A){return A.id===z.$jscomp$loop$prop$ship$317.type}}(m)).name,r=document.createElement("div"),
r.className="flex-grow-1 border-bottom align-self-center mx-2",x.appendChild(v),x.appendChild(r),l.appendChild(x));f||u===m.$jscomp$loop$prop$ship$317.type2||(u=document.createElement("div"),u.className="type_divider mt-2 ml-1",x=document.createElement("div"),x.className="font_size_12 font_color_half align-self-center",v=API_CTYPE.find(function(z){return function(A){return A.id===z.$jscomp$loop$prop$ship$317.type2}}(m))||{name:""},x.textContent=v.name,u.appendChild(x),l.appendChild(u));x=m.$jscomp$loop$prop$ship$317.type;
...
function expandMainPreset(a,b,c,d){b=void 0===b?!0:b;c=void 0===c?!0:c;d=void 0===d?!0:d;if(a)try{var e=a[0],f=!1;$(".lb_plane").each(function(y,C){if(e&&0!==e.length){var z=e[0].find(function(A){return A[4]===y});z&&0!==z.length?(z={id:z[0],prof:z[1],remodel:z[2],slot:z[3]},setLBPlaneDiv($(C),z),f=!0):b&&setLBPlaneDiv($(C))}});$(".ohuda_select").each(function(y,C){$(C).find("option").prop("disabled",!1);f&&e&&2<=e.length?(y=castInt(e[1][y],-1),$(C).val(1===y?2:y)):b&&$(C).val(-1)});if(e&&3<=e.length)for(var g=
0;2>=g;g++)document.getElementById("lb"+(g+1)+"_target_1").value=e[2][2*g],document.getElementById("lb"+(g+1)+"_target_2").value=e[2][2*g+1];5<=a.length?toggleDefenseMode(a[4]):toggleDefenseMode(!1);if(c){var h=6;a[1].length&&a[1][0]&&6<=a[1][0].length&&(h=2===a[1][0][5]?7:6,document.getElementById("union_fleet").checked=1===a[1][0][5],document.getElementById("yugeki_fleet").checked=2===a[1][0][5]);clearShipDivAll(h);var l=0,k=0;$(".ship_tab:not(.d-none)").each(function(y,C){var z=a[1].find(function(E){return E[2]===
y});if(z){C=$(C);setShipDiv(C,z[0],99,8<=z.length&&z[7]?z[7]:0);C.find(".ship_plane").each(function(E,F){F=$(F);var G=z[1].find(function(I){return I[4]===E});!G&&F.hasClass("expanded_slot")&&(G=z[1].find(function(I){return-1===I[4]}));G&&!F.hasClass("d-none")&&(setPlaneDiv(F,{id:G[0],prof:G[1],remodel:G[2],slot:G[3]},!0),6<=G.length&&G[5]&&plane_unlock_Clicked(F.find(".plane_unlock")))});if(4<=z.length){var A=C.find(".ship_disabled");z[3]?(A.addClass("disabled"),A.children().addClass("fa-eye-slash").removeClass("fa-eye no_capture"),
C.addClass("opacity4")):(A.removeClass("disabled"),A.children().removeClass("fa-eye-slash").addClass("fa-eye no_capture"),C.removeClass("opacity4"))}5<=z.length&&(z[4]?C.find(".ship_level").text(z[4]):C.find(".ship_level").text(99));7<=z.length&&0<z[6]&&C.find(".ship_luck").text(z[6]);y<h?l+=1:k+=1}});display_ship_count_Changed($("#friendFleet_item1"),Math.max(l,1),!0);display_ship_count_Changed($("#friendFleet_item2"),Math.max(k,1),!0)}document.getElementById("fleet_formation").value=1;if(4<=a.length&&
a[3]){document.getElementById("adapt_stage2").checked=!0;try{c=1;selectedFormations=[];for(var m=$jscomp.makeIterator(a[3]),n=m.next();!n.done;n=m.next()){var q=castInt(n.value);if(0>=q){document.getElementById("adapt_stage2").checked=!1;selectedFormations=[];c=1;break}else selectedFormations.push(q),c=q}document.getElementById("fleet_formation").value=c}catch(y){var p=castInt(a[3]);if(p){selectedFormations=[];for(m=0;10>m;m++)selectedFormations.push(p);document.getElementById("adapt_stage2").checked=
!0;document.getElementById("fleet_formation").value=p}}}else document.getElementById("adapt_stage2").checked=!1,selectedFormations=[];p=1;for(m=0;m<a[2].length;m++)p<a[2][m][0]+1&&(p=a[2][m][0]+1);p=Math.min(p,10);if(0!==a[2].length||d)clearEnemyDivAll(p),$("#battle_count").val(p);var u=isDefMode&&6>a.length;$(u?"#air_raid_enemies":".battle_content").each(function(y,C){var z=a[2].find(function(G){return G[0]===y});if(z){3<=z.length?$(C)[0].dataset.celldata=z[2]:$(C)[0].dataset.celldata="";if(4<=z.length){var A=
castInt(z[3]);u?$(C).find(".cell_type").val([0,5].includes(A)?5:3):$(C).find(".cell_type").val(A);cell_type_Changed($(C).find(".cell_type"),!1)}else $(C).find(".cell_type").val(1);5<=z.length?$(C).find(".formation").val(z[4]):$(C).find(".formation").val(1);6<=z.length?$(C).find(".enemy_range").text(z[5]):$(C).find(".enemy_range").text(0);A=0;z=$jscomp.makeIterator(z[1]);for(var E=z.next();!E.done;E=z.next()){E=E.value;var F=$(C).find(".enemy_content").eq(A++);0<E?setEnemyDiv(F,E):0>E?setEnemyDiv(F,
-1,-E):clearEnemyDiv(F);F.addClass("d-flex").removeClass("d-none")}}});if(6<=a.length){var t=a[5];if(t&&3===t.length){var x=$("#air_raid_enemies"),v=castInt(t[1]);x.find(".cell_type").val([0,5].includes(v)?5:3);cell_type_Changed(x.find(".cell_type"),!1);x.find(".formation").val(t[2]);d=0;for(var r=$jscomp.makeIterator(t[0]),w=r.next();!w.done;w=r.next()){var B=w.value,D=x.find(".enemy_content").eq(d++);0<B?setEnemyDiv(D,B):0>B?setEnemyDiv(D,-1,-B):clearEnemyDiv(D);D.addClass("d-flex").removeClass("d-none")}}}7<=
a.length?$("#admin_lv").val(a[6]):$("#admin_lv").val(120)}catch(y){$(".ohuda_select").val(-1),$(".lb_plane").each(function(C,z){setLBPlaneDiv($(z))}),clearShipDivAll(6),clearEnemyDivAll(1),$("#battle_count").val(1),sendErrorLog(y)}}
...
0,p=100;else if(5===f.$jscomp$loop$prop$status$334){f.$jscomp$loop$prop$abbr$331=$("#shoot_down_table").find(".cond.battle"+(displayBattle+1)).text();var t=AIR_STATUS.find(function(v){return function(r){return r.abbr===v.$jscomp$loop$prop$abbr$331}}(f));t&&(f.$jscomp$loop$prop$status$334=t.id);0===f.$jscomp$loop$prop$status$334&&(p=100)}p=Math.min(100,p);var x=l.data("airstatus");t="bar_status"+f.$jscomp$loop$prop$status$334;l.removeClass("bar_status"+x).addClass(t).css({width:p+"%"}).data("airstatus",
f.$jscomp$loop$prop$status$334);k.find(".rate_td_ap").text(f.$jscomp$loop$prop$ap$332);x=k.find(".rate_td_eap");x.find(".avg_eap").text(m);x.find(".eap_border").html("( "+q.slice(0,4).map(function(v){return function(r){return r<=v.$jscomp$loop$prop$ap$332?"<b>"+r+"</b>":r.toString()}}(f)).join(" / ")+" )");x=100;for(m=0;m<n.length;m++)k.find(".rate_td_status"+m).text("-"),isDefMode?k.find(".rate_td_status"+f.$jscomp$loop$prop$status$334).text("100 %"):0<n[m]&&(u=Math.floor(1E3*n[m])/10,q=0,m+1<n.length&&
0<n[m+1]?(q=u,x-=u):q=x.toFixed(1),k.find(".rate_td_status"+m).text(q+" %"),u=!0);if(u||isDefMode&&8===h)k.removeClass("d-none disabled_tr"),l.closest(".progress_area").addClass("d-flex").removeClass("d-none disabled_bar");l="bar_ex_status"+f.$jscomp$loop$prop$status$334;isDefMode&&(k=$("#simple_lb_def_bar"),m=k.find(".result_bar"),x=m.data("airstatus"),m.removeClass("bar_status"+x+" bar_ex_status"+x).addClass(t+" "+l).css({width:p+"%"}).data("airstatus",f.$jscomp$loop$prop$status$334),k.find(".simple_label").text(AIR_STATUS.find(function(v){return function(r){return r.id===
v.$jscomp$loop$prop$status$334}}(f)).abbr),k.removeClass("d-none"));7===h&&u&&-1<f.$jscomp$loop$prop$status$334&&(h=$("#fleet_simple_bar"),u=h.find(".result_bar"),x=u.data("airstatus"),u.removeClass("bar_status"+x+" bar_ex_status"+x).addClass(t+" "+l).css({width:p+"%"}).data("airstatus",f.$jscomp$loop$prop$status$334),n=AIR_STATUS.find(function(v){return function(r){return r.id===v.$jscomp$loop$prop$status$334}}(f)).abbr+"("+Math.floor(100*n[f.$jscomp$loop$prop$status$334])+"%)",h.find(".simple_label").text(n),
...
function getAirStatusIndex(a,b){if(1E3>=a)return b<AIR_STATUS_TABLE[a].length-1?AIR_STATUS_TABLE[a][b]:4;b=getAirStatusBorder(b);for(var c=b.length,d=0;d<c;d++)if(a>=b[d])return 0!==a?d:4}function getAirStatusBorder(a){return 0===a?[0,0,0,0,0]:[3*a,Math.ceil(1.5*a),Math.floor(a/1.5)+1,Math.floor(a/3)+1,0]}
...
function updateFleetView(){var a=inputItems.fleet,b=document.getElementById("shoot_down_table"),c=document.getElementById("shoot_down_table_tbody"),d=-1;b.getElementsByTagName("tbody")[0].innerHTML="";b=$jscomp.makeIterator(b.getElementsByTagName("tfoot")[0].getElementsByClassName("td_battle"));for(var e=b.next();!e.done;e=b.next())e.value.innerHTML="";$("#fleet_formation").find("option").each(function(y,C){y=castInt($(C).val());a.isUnion&&10<y?$(C).removeClass("d-none"):!a.isUnion&&10<y&&$(C).addClass("d-none")});
$("#fleet_antiair_formation").find("option").each(function(y,C){y=castInt($(C).val());a.isUnion&&10<y?$(C).removeClass("d-none"):!a.isUnion&&10<y&&$(C).addClass("d-none")});setting.visibleAntiAirStatus?($(".anti_air_status").removeClass("d-none"),$(".anti_air_status").addClass("d-flex")):($(".anti_air_status").addClass("d-none"),$(".anti_air_status").removeClass("d-flex"));b=document.getElementsByClassName("fleet_ap")[0];e=document.getElementsByClassName("fleet_ap")[1];for(var f=document.getElementsByClassName("contact_start_rate")[0],
...
p=document.createElement("tr");for(x=0;x<k.$jscomp$loop$prop$ship$339.items.length;x++)if(w=k.$jscomp$loop$prop$ship$339.items[x],v=w.name,k.$jscomp$loop$prop$rawShip$340||w.isPlane&&0!==w.slot){if(k.$jscomp$loop$prop$rawShip$340&&x===k.$jscomp$loop$prop$rawShip$340.slot.length)break;if(!k.$jscomp$loop$prop$rawShip$340||!t||w.isPlane&&0!==w.slot){w=getItemCss(w.itype).replace("css","shoot_table");if(r){r=document.createElement("tr");r.id="s"+k.$jscomp$loop$prop$ship$339.shipNo+"sl"+x;r.dataset.shipid=
k.$jscomp$loop$prop$rawShip$340?k.$jscomp$loop$prop$rawShip$340.id:0;r.dataset.si=d;r.dataset.slot=x;r.dataset.css=w+"_hover";q=document.createElement("td");q.rowSpan=n;q.className="td_name align-middle";q.textContent=k.$jscomp$loop$prop$rawShip$340?k.$jscomp$loop$prop$rawShip$340.name:"\u672a\u6307\u5b9a";r.appendChild(q);q=document.createElement("td");q.className="pl-1 td_plane_name align-middle "+w;q.textContent=v;r.appendChild(q);p.appendChild(q.cloneNode(!1));for(v=1;10>=v;v++)w=document.createElement("td"),
w.className="td_battle battle"+v+" align-middle",r.appendChild(w),p.appendChild(w.cloneNode(!1));v=document.createElement("td");v.className="td_battle battle_end align-middle";r.appendChild(v);p.appendChild(v.cloneNode(!1));v=document.createElement("td");v.className="td_battle battle_death align-middle";r.appendChild(v);p.appendChild(v.cloneNode(!1));u.appendChild(r)}else r=p.cloneNode(!0),r.id="s"+k.$jscomp$loop$prop$ship$339.shipNo+"sl"+x,r.dataset.shipid=k.$jscomp$loop$prop$rawShip$340?k.$jscomp$loop$prop$rawShip$340.id:
0,r.dataset.si=d,r.dataset.slot=x,r.dataset.css=w+"_hover",q=r.getElementsByClassName("td_plane_name")[0],q.className="pl-1 td_plane_name align-middle "+w,q.textContent=v,u.appendChild(r);r=!1}}c.appendChild(u)}}d=[];k=$jscomp.makeIterator(a.ships);for(n=k.next();!n.done;n=k.next())d=d.concat(n.value.items);for(k=1;4>=k;k++)m=document.getElementsByClassName("fleet_scout"+k)[0],n=document.getElementsByClassName("fleet_scout"+k)[1],m.textContent=a.scoutScores[k-1],n.textContent=a.scoutScores[k-1];d=
...
u=p.next())if(q.$jscomp$loop$prop$enemy$342=u.value,b+=q.$jscomp$loop$prop$enemy$342.antiAirBonus,!d&&ANTIAIR_CUTIN_ENEMY.includes(q.$jscomp$loop$prop$enemy$342.id)&&(d=!0),!e&&UNKNOWN_ENEMY.includes(q.$jscomp$loop$prop$enemy$342.id)&&(e=!0),m===l&&!q.$jscomp$loop$prop$enemy$342.onlyScout&&q.$jscomp$loop$prop$enemy$342.slots.length){for(var t=u="",x=!0,v=-1,r={$jscomp$loop$prop$j$344:0};r.$jscomp$loop$prop$j$344<q.$jscomp$loop$prop$enemy$342.equipments.length;r={$jscomp$loop$prop$j$344:r.$jscomp$loop$prop$j$344},
r.$jscomp$loop$prop$j$344++){var w=ENEMY_ITEM.find(function(E,F){return function(G){return G.id===E.$jscomp$loop$prop$enemy$342.equipments[F.$jscomp$loop$prop$j$344].id}}(q,r));if(w&&PLANE_TYPE.includes(w.type)){v+=1;var B=q.$jscomp$loop$prop$enemy$342.slots[v],D=getItemCss(w.itype).replace("css","shoot_table"),y=v===q.$jscomp$loop$prop$enemy$342.slots.length-1?" last_slot":"",C="",z="",A="";u+='<img src="../img/plane/'+w.id+'.png" class="img-size-36" data-planeid="'+w.id+'">';x&&(C='\n\t\t\t\t\t<td class="td_name align-middle last_slot" rowspan="'+
q.$jscomp$loop$prop$enemy$342.slots.length+'">\n\t\t\t\t\t\t<div>'+drawEnemyGradeColor(q.$jscomp$loop$prop$enemy$342.name)+'</div>\n\t\t\t\t\t\t<div class="d-none d-md-block">xxPxx</div>\n\t\t\t\t\t</td>',A='<td class="td_all_dead align-middle last_slot" rowspan="'+q.$jscomp$loop$prop$enemy$342.slots.length+'"></td>',z='<td class="td_detail_slot align-middle last_slot" rowspan="'+q.$jscomp$loop$prop$enemy$342.slots.length+'"><i class="fas fa-file-text"></i></td>',x=!1);t+='\n\t\t\t\t<tr class="enemy_no_'+
n+" slot_"+v+'" data-rowindex="'+n+'" data-slot="'+v+'" data-css="'+D+'_hover">\n\t\t\t\t\t'+C+'\n\t\t\t\t\t<td class="pl-1 td_plane_name align-middle '+(D+y)+'">'+w.name+'</td>\n\t\t\t\t\t<td class="td_init_slot align-middle'+y+'">'+B+'</td>\n\t\t\t\t\t<td class="td_avg_slot align-middle'+y+'">'+B+'</td>\n\t\t\t\t\t<td class="td_max_slot align-middle'+y+'">'+B+'</td>\n\t\t\t\t\t<td class="td_min_slot align-middle'+y+'">'+B+"</td>\n\t\t\t\t\t"+(isDefMode?"":A)+"\n\t\t\t\t\t"+(isDefMode?"":z)+"\n\t\t\t\t</tr>"}}h.push(t.replace("xxPxx",
...
f=$jscomp.makeIterator(c.getElementsByClassName("enemy_content"));for(var g=f.next();!g.done;g=f.next()){g=g.value;var h=castInt(g.dataset.enemyid),l=castInt(g.getElementsByClassName("enemy_index")[0].dataset.index);h=new Enemy(h);if(!g.classList.contains("d-none")){if(!d.isUnion&&6===d.enemies.length)break;e&&h.id&&!h.type.includes(18)&&(e=!1);h.isDead=g.classList.contains("is_dead");h.isEscort=d.isUnion&&6<l;d.enemies.push(h)}}d.enemies.length&&(d.isAllSubmarine=e);d.formation=castInt(c.getElementsByClassName("formation")[0].value);
d.createStage2Table();a.battles.push(d)}}return a}
...
function shootDownFleet(a,b,c,d,e){e=void 0===e?!0:e;var f=c.stage2[0][0].length;5===a&&(a=0);for(var g=c.enabledStage2,h=b.ships.length,l=0;l<h;l++){var k=b.ships[l];if(!k.ignoreSlot){for(var m=0,n=!c.isUnion&&k.isEscort||4===c.cellType,q=k.items.length,p=0;p<q;p++){var u=k.items[p];e&&(u.results[d]+=u.slot);if(!(0>=u.slot||!u.canBattle||n)&&f){var t=getShootDownSlotFF(a,u.slot);u.slot-=Math.floor(u.isJet?.6*t:t);if(u.isAttacker&&g){t=Math.floor(Math.random()*f);var x=c.stage2[u.avoid];Math.floor(2*
Math.random())&&(u.slot-=Math.floor(x[0][t]*u.slot));Math.floor(2*Math.random())&&(u.slot-=x[1][t])}u.canBattle&&0<u.slot?u.airPower=Math.floor(u.antiAir*Math.sqrt(u.slot)+u.bonusAirPower):(0>=u.slot&&(u.slot=0),u.airPower=0)}m+=u.airPower}k.airPower=m}}b.updateAirPower()}
...
function rateCalculate(){var a=inputItems.landBase,b=inputItems.fleet,c=inputItems.battleInfo,d=castInt($("#calculate_count").val());d=0>=d?1:d;$("#calculate_count").val(d);for(var e=b.ships.length,f=c.battles,g=isDefMode?0:displayBattle,h=[0,0,0,0,0,0],l=[],k=[],m=[],n=[],q=[[],[]],p=isDefMode?1:battleCount,u=0,t=[],x=$jscomp.makeIterator(f),v=x.next();!v.done;v=x.next())v=v.value,v.enemies=v.enemies.filter(function(H){return-1===H.id||0<H.slots.length});g=g<c.battles.length?g:c.battles.length-1;
for(c=0;c<p;c++)l.push(0),k.push(0);for(c=0;c<e;c++)if(x=b.ships[c],!x.ignoreSlot)for(v=0;v<x.items.length;v++)x.items[v].isPlane&&m.push(Array(d));for(c=0;c<f[g].enemies.length;c++)if(x=f[g].enemies[c],!(x.onlyScout||0>x.id)){v=0===n.length?0:n[n.length-1][0]+1;for(var r=0;r<x.slots.length;r++)n.push([v,r,0,0,999]);t.push(0)}c=f[0].cellType===CELL_TYPE.highAirRaid?a.defense2AirPower:a.defenseAirPower;resultData.defenseAirPower=c;isDefMode&&(b.fullAirPower=c,b.airPower=c,b.fullUnionAirPower=c,b.unionAirPower=
c);c=0;if(isDefMode||f[0])c=f[0].getAirPower();x=!isDefMode&&document.getElementById("adapt_stage2").checked;v=[];x&&(v=castInt($("#fleet_formation").val()),b.isUnion&&f[g].cellType===CELL_TYPE.airRaid?b.updateStage2All(v,!0):b.updateStage2All(v),v=b.allStage2);for(r=0;r<d;r++){for(var w=0;w<p;w++){var B=f[w],D=B.enemies,y=!1;y=calculateLandBase(a,B,w);u+=doJetPhase(b,B);var C=B.isUnion?b.unionAirPower:b.airPower,z=B.getAirPower(),A=getAirStatusIndex(C,z);w===g&&(h[A]+=1);l[w]+=C;k[w]+=z;shootDownFleet(A,
b,B,w);B.cellType===CELL_TYPE.aerialCombat&&(shootDownEnemy(A,D,x,v),y=!0,A=getAirStatusIndex(b.airPower,B.getAirPower()),shootDownFleet(A,b,B,w,!1));if(w===g){shootDownEnemy(A,D,x,v);for(A=C=y=B=0;A<D.length;A++){var E=D[A];if(!E.onlyScout&&E.id){for(var F=E.slots,G=E.attackers.concat(),I=0;I<F.length;I++){var K=F[I],L=n[B];B+=1;L[2]+=K;L[3]<K&&(L[3]=K);L[4]>K&&(L[4]=K);0===K&&(G[I]=!1)}0>G.indexOf(!0)&&(t[y]+=1);y+=1;C+=E.airPower}E.slots=E.fullSlots.concat();E.airPower=E.fullAirPower;E.landBaseAirPower=
E.fullLandBaseAirPower}q[0].push(z);q[1].push(C);y=!1}if(y)for(D=$jscomp.makeIterator(D),z=D.next();!z.done;z=D.next())z=z.value,z.slots=z.fullSlots.concat(),z.airPower=z.fullAirPower,z.landBaseAirPower=z.fullLandBaseAirPower}for(w=0;3>w;w++)if(D=a.landBases[w],0!==D.mode){for(z=0;z<D.planes.length;z++)B=D.planes[z],B.slot=B.fullSlot,B.airPower=B.fullAirPower;D.airPower=D.fullAirPower}for(D=w=0;D<e;D++)if(z=b.ships[D],!z.ignoreSlot){B=z.items.length;for(y=0;y<B;y++)C=z.items[y],C.isPlane&&(m[w++][r]=
C.slot,C.slot=C.fullSlot,C.airPower=C.fullAirPower);z.airPower=z.fullAirPower}b.airPower=b.fullAirPower;b.escortAirPower=b.fullEscortAirPower;b.unionAirPower=b.fullUnionAirPower}for(x=f=0;x<e;x++)if(v=b.ships[x],!v.ignoreSlot)for(r=0;r<v.items.length;r++)v.items[r].isPlane&&(w=m[f],D={shipNo:v.shipNo,slot:v.items[r].slotNo,max:getArrayMax(w),min:getArrayMin(w),avg:w.reduce(function(H,M){return H+M})/d,death:w.filter(function(H){return 0===H}).length,deathRate:0},D.deathRate=D.death/w.length,resultData.fleetSlots.push(D),
f+=1);e=$("#shoot_down_table");for(m=0;m<p;m++)for(f=Math.round(l[m]/d),x=Math.round(k[m]/d),v=getAirStatusIndex(f,x),e.find(".fap.battle"+(m+1)).text(f),e.find(".eap.battle"+(m+1)).text(x),e.find(".cond.battle"+(m+1)).html("<span>"+AIR_STATUS[v].abbr+"</span>").removeClass("airStatus0 airStatus1 airStatus2 airStatus3 airStatus4").addClass("airStatus"+v),f=$jscomp.makeIterator(b.ships),x=f.next();!x.done;x=f.next())if(x=x.value,!x.ignoreSlot)for(v=$jscomp.makeIterator(x.items),r=v.next();!r.done;r=
v.next())r=r.value,r.isPlane&&(w=r.results[m],$("#s"+x.shipNo+"sl"+r.slotNo).find(".battle"+(m+1)).text(Math.round(w/d)));e.find("td").removeClass("d-none");for(b=battleCount+1;10>=b;b++)e.find(".battle"+b).addClass("d-none");isDefMode||(resultData.fleetAirPower=Math.round(l[g]/d));l=[];b=$jscomp.makeIterator(a.landBases);for(p=b.next();!p.done;p=b.next())for(p=p.value,e=0;2>e;e++)l.push(p.results[displayBattle].airStatusIndex[e].map(function(H){return H/d}));a=$jscomp.makeIterator(a.landBases);for(p=
...
function fleetSlotDetailCalculate(a,b,c){c=void 0===c?0:c;setPreCalculateTable();var d=createLandBaseInstance(),e=createFleetInstance(),f=updateEnemyFleetInfo(!1),g=castInt($("#detail_calculate_count").val()),h=[],l=e.ships[a];if(!ITEM_DATA.find(function(B){return B.id===l.items[b].id&&PLANE_TYPE.includes(B.type)}))return!1;var k=isDefMode?0:f.battles.length-1,m=castInt(document.getElementById("target_enemy").value,-1);0<=m&&m<f.battles.length&&(k=m);m=document.getElementById("adapt_stage2").checked;
var n=[];m&&(n=castInt($("#fleet_formation").val()),e.isUnion&&f.battles[k].cellType===CELL_TYPE.airRaid?e.updateStage2All(n,!0):e.updateStage2All(n),n=e.allStage2);for(var q=l.items[b],p=$jscomp.makeIterator(f.battles),u=p.next();!u.done;u=p.next())u=u.value,u.enemies=u.enemies.filter(function(B){return 0<B.slots.length});for(p=0;p<g;p++){for(u=0;u<=k;u++){var t=f.battles[u],x=t.enemies;calculateLandBase(d,t,u);doJetPhase(e,t);var v=t.cellType,r=t.isUnion?e.unionAirPower:e.airPower,w=t.getAirPower();
r=getAirStatusIndex(r,w);shootDownFleet(r,e,t,u);v===CELL_TYPE.aerialCombat&&(shootDownEnemy(r,x,m,n),r=getAirStatusIndex(e.airPower,t.getAirPower()),shootDownFleet(r,e,t,u,!1));t=$jscomp.makeIterator(x);for(x=t.next();!x.done;x=t.next())x=x.value,x.slots=x.fullSlots.concat(),x.airPower=x.fullAirPower,x.landBaseAirPower=x.fullLandBaseAirPower}for(u=0;3>u;u++){t=d.landBases[u];for(x=0;x<t.planes.length;x++)v=t.planes[x],v.slot=v.fullSlot,v.updateAirPower();t.updateAirPower()}for(u=0;u<e.ships.length;u++)for(t=
e.ships[u],x=t.items.length,v=t.airPower=0;v<x;v++)r=t.items[v],u===a&&r.slotNo===b&&h.push(r.slot),r.slot=r.fullSlot,r.updateAirPower(),t.airPower+=r.airPower;e.updateAirPower()}chartSlotDistribution=updateDetailChart(h,"\u6b8b\u6a5f\u6570 [\u6a5f]",{mode:"index",callbacks:{title:function(B,D){B=castInt(B[0].xLabel);return"\u6b8b\u6a5f\u6570\uff1a"+B+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a"+ShipItem.getFirePower(q,B).join(" or ")},label:function(B,D){return[B.yLabel+" %"]}}});e=SHIP_DATA.find(function(B){return B.id===
...
a={};k=$jscomp.makeIterator(l.items);for(h=k.next();!h.done;a={$jscomp$loop$prop$pl$352:a.$jscomp$loop$prop$pl$352},h=k.next()){a.$jscomp$loop$prop$pl$352=h.value;h=ITEM_DATA.find(function(B){return function(D){return D.id===B.$jscomp$loop$prop$pl$352.id}}(a));if(g===(e?e.slot.length:4))break;m=h?h.name:"-";n=PLANE_TYPE.includes(h?h.type:0);p="";n&&FIGHTERS.includes(h.type)?p=h.antiAir?'<span class="mx-1">\u5bfe\u7a7a: '+h.antiAir+"</span>":"":n&&RECONNAISSANCES.includes(h.type)?(p=h.antiAir?'<span class="mx-1">\u5bfe\u7a7a: '+
...
0;n<f;n++){for(var q=0;q<=h;q++){for(var p=e.battles[q],u=p.enemies,t=0;3>t;t++){var x=d.landBases[t],v=x.target[0],r=x.target[1];if(v===q||r===q){if(v===q){doLandBaseJetPhase(x,p);var w=x.airPower,B=p.getLandBaseAirPower();w=0===w&&0===B?0:getAirStatusIndex(w,B);shootDownEnemy(w,u);v!==r&&shootDownLandBase(w,x,p)}r===q&&(v!==r&&doLandBaseJetPhase(x,p),v=x.airPower,r=p.getLandBaseAirPower(),v=0===v&&0===r?0:getAirStatusIndex(v,r),shootDownEnemy(v,u),shootDownLandBase(v,x,p))}}p=$jscomp.makeIterator(u);
for(u=p.next();!u.done;u=p.next())u=u.value,u.slots=u.fullSlots.concat(),u.airPower=u.fullAirPower,u.landBaseAirPower=u.fullLandBaseAirPower}for(q=0;3>q;q++){p=d.landBases[q];for(t=u=0;t<p.planes.length;t++)x=p.planes[t],a+1===p.baseNo&&b===t&&g.push(x.slot),u+=x.slot,x.slot=x.fullSlot,x.updateAirPower();a+1===p.baseNo&&m.push(u);p.updateAirPower()}}var D=l[b].id,y=l[b].remodel,C=l.find(function(A){return 312===A.id})?1.15:l.find(function(A){return 311===A.id})?1.125:1,z=e.battles[h].isUnion?1.1:
1;chartSlotDistribution=updateDetailChart(g,"\u6b8b\u6a5f\u6570 [\u6a5f]",{mode:"index",callbacks:{title:function(A,E){A=A[0].xLabel;E="\u822a\u7a7a\u6226\u706b\u529b\uff1a"+(LandBaseItem.getFirePower(D,A,y)*C*z).toFixed(1);return A?"\u6b8b\u6a5f\u6570\uff1a"+A+" \u6a5f\n"+E:"\u6b8b\u6a5f\u6570\uff1a0 \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0"},label:function(A,E){return[A.yLabel+" %"]}}});n=h=g=0;m=$jscomp.makeIterator(m);for(q=m.next();!q.done;q=m.next())q=q.value,p=q/k,0===q?n+=1:.25>p?h+=
1:.4>p&&(g+=1);$("#status_s").text(100*g/f+" %");$("#status_m").text(100*h/f+" %");$("#status_l").text(100*n/f+" %");f="";k=0;document.getElementById("graph_title").textContent="\u51fa\u6483\u7d42\u4e86\u6642\u70b9\u3067\u306e\u6b8b\u6a5f\u6570\u5206\u5e03";$("#land_base_detail_table").removeClass("d-none");$(".detail_fire").addClass("d-none").removeClass("d-flex");$(".detail_wave").removeClass("d-none").addClass("d-flex");$("#detail_info").data("mode","land_base_detail");$("#detail_info").data("base_no",
...
f);document.getElementById("stage3_all_slot_label").textContent="\u6b8b\u6a5f\u6570\u5206\u5e03\u306e\u7d50\u679c\u3092\u5229\u7528\uff08"+getArrayMin(b)+"\u6a5f ~ "+f+"\u6a5f\uff09";document.getElementById("stage3_remodel").value=y?y:0;l.some(function(A){return 311===A.id})&&(document.getElementById("enabled_rikutei").checked=!0);l.some(function(A){return 312===A.id})&&(document.getElementById("enabled_rikutei_z").checked=!0);l='<option value="99">\u4efb\u610f</option>';e=e.battles.length-1;b=castInt(document.getElementById("target_enemy").value,
...
var t=document.getElementById("aa_cutin_miss").checked;document.getElementById("aa_cutin_miss").parentElement.classList.remove("d-none");u=e.allStage2;t&&(u=[u[u.length-1]])}else document.getElementById("aa_cutin_miss").parentElement.classList.add("d-none");t=$jscomp.makeIterator(f.battles);for(var x=t.next();!x.done;x=t.next())x=x.value,x.enemies=x.enemies.filter(function(y){return 0<y.slots.length});for(t=0;t<g;t++){for(x=0;x<=h;x++){var v=f.battles[x],r=v.enemies;calculateLandBase(d,v,x);doJetPhase(e,
v);var w=v.cellType,B=v.isUnion?e.unionAirPower:e.airPower,D=v.getAirPower();B=getAirStatusIndex(B,D);shootDownFleet(B,e,v,x);w===CELL_TYPE.aerialCombat&&(shootDownEnemy(B,r,q,u),B=getAirStatusIndex(e.airPower,v.getAirPower()),shootDownFleet(B,e,v,x,!1));x===h&&(shootDownEnemy(B,r,q,u),v=m.slots[b],k.push(v),c||(v=p.getFirePower(v),2===v.length?(l.push(v[0]),l.push(v[1])):l.push(v[0])));for(v=0;v<r.length;v++)w=r[v],w.onlyScout||(w.slots=w.fullSlots.concat(),w.airPower=w.fullAirPower,w.landBaseAirPower=
w.fullLandBaseAirPower)}for(x=0;3>x;x++){r=d.landBases[x];for(v=0;v<r.planes.length;v++)w=r.planes[v],w.slot=w.fullSlot,w.updateAirPower();r.updateAirPower()}for(x=0;x<e.ships.length;x++)for(r=e.ships[x],v=r.items.length,w=r.airPower=0;w<v;w++)B=r.items[w],B.slot=B.fullSlot,B.updateAirPower(),r.airPower+=B.airPower;e.updateAirPower()}chartSlotDistribution=updateDetailChart(c?k:l,c?"\u6b8b\u6a5f\u6570 [\u6a5f]":"\u822a\u7a7a\u6226\u706b\u529b",{mode:"index",callbacks:{title:function(y){y=y[0].xLabel;
if($("#slot_detail").prop("checked")){if(!y)return"\u6b8b\u6a5f\u6570\uff1a"+y+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0";var C="\u822a\u7a7a\u6226\u706b\u529b\uff1a"+p.getFirePower(y).join(" or ");return"\u6b8b\u6a5f\u6570\uff1a"+y+" \u6a5f\n"+C}return"\u822a\u7a7a\u6226\u706b\u529b\uff1a"+y},label:function(y){return[y.yLabel+" %"]}}});if(!c)for(chartSlotDistribution=[],d=getArrayMax(k),f=getArrayMin(k),e=k.length,f={$jscomp$loop$prop$i$269$356:f};f.$jscomp$loop$prop$i$269$356<=d;f={$jscomp$loop$prop$i$269$356:f.$jscomp$loop$prop$i$269$356},
f.$jscomp$loop$prop$i$269$356++)(g=k.filter(function(y){return function(C){return C===y.$jscomp$loop$prop$i$269$356}}(f)).length)&&chartSlotDistribution.push({slot:f.$jscomp$loop$prop$i$269$356,rate:g/e});$("#land_base_detail_table").addClass("d-none");$(".detail_fire").removeClass("d-none").addClass("d-flex");$(".detail_wave").addClass("d-none").removeClass("d-flex");$("#detail_info").data("mode","enemy_slot_detail");$("#detail_info").data("base_no",a);$("#detail_info").data("slot_no",b);$("#detail_info").data("battle_no",
h);$("#detail_warning").text("\u203b \u30b0\u30e9\u30d5\u5185\u306e\u822a\u7a7a\u6226\u706b\u529b\u306f\u30ad\u30e3\u30c3\u30d7\u524d\u3067\u30af\u30ea\u30c6\u30a3\u30ab\u30eb\u3001\u89e6\u63a5\u88dc\u6b63\u306a\u3057\u3002\u5c0f\u6570\u5207\u6368\u3066");a=document.getElementById("graph_title").textContent="";d=h=0;e={};f=$jscomp.makeIterator(n.eqp);for(g=f.next();!g.done;e={$jscomp$loop$prop$id$358:e.$jscomp$loop$prop$id$358},g=f.next())e.$jscomp$loop$prop$id$358=g.value,g=ENEMY_ITEM.find(function(y){return function(C){return C.id===
...
document.getElementById("jet_phase").checked,n=aerialSetting.isAllSlot,q="",p=chartSlotDistribution,u=0;u<a.length;u++){var t=a[u];if(!(0>=t.id)){var x=t.armor+t.sumItemArmor,v=[];v=d?f&&t.type.includes(16)?getLandBaseAerialPowersMod(p):g&&t.type.includes(13)?getLandBaseAerialPowersMod2(p):t.type.includes(2)?getLandBaseAerialPowersLandBase(p,t):t.type.includes(18)&&h?getLandBaseASWPower(p):!e||86!==t.id&&120!==t.id?getLandBaseAerialPowers(p):getLandBaseAerialPowers(p,3.2):c?getFleetAerialPower(p,
t,1,m):getEnemyAerialPowers(p,t);var r=t.hp,w=createDamageDistribution(v,x,k,c,r),B=w.map(function(E){return E.damage}),D=getArrayMin(B),y=getArrayMax(B);B=[{min:r,max:getArrayMax(w.map(function(E){return E.damage}))+1,value:0},{min:Math.ceil(.75*r),max:r,value:0},{min:Math.ceil(.5*r),max:Math.ceil(.75*r),value:0}];v=v.map(function(E){return E.power}).some(function(E){return 0>=E});D=(1<=D?D:v?"0":"\u5272\u5408")+"~"+(1<=y?y:"\u5272\u5408");n||0!==l||(D="-");v="";y={};for(var C=0;C<B.length;y={$jscomp$loop$prop$borderMin$370:y.$jscomp$loop$prop$borderMin$370,
$jscomp$loop$prop$borderMax$371:y.$jscomp$loop$prop$borderMax$371},C++){var z=B[C];y.$jscomp$loop$prop$borderMin$370=z.min;y.$jscomp$loop$prop$borderMax$371=z.max;var A=w.filter(function(E){return function(F){return F.damage>=E.$jscomp$loop$prop$borderMin$370&&F.damage<E.$jscomp$loop$prop$borderMax$371}}(y)).map(function(E){return E.rate});A=A.length===w.length?1:getArraySum(A);z.value=A;if(b.isAttacker||!c||t.type.includes(18)&&h){if(c&&t.type.includes(18)&&!h||!c&&[13,14].includes(t.type)){v+='<td class="text-center" colspan="'+
B.length+'">\u5bfe\u6f5c\u653b\u6483\u4e0d\u53ef</td>';D="";w=$jscomp.makeIterator(B);for(y=w.next();!y.done;y=w.next())y.value.value=0;break}if(1!==A||0!==C||n)1===A?v+='<td>100 <span class="font_size_11 font_color_half">%</span></td>':(z=Math.floor(1E3*A)/10,v+="<td>"+(0===z&&0!==A?.1:z)+' <span class="font_size_11 font_color_half">%</span></td>');else{v+='<td colspan="'+B.length+'" class="text-center text_red">\u78ba\u6bba</td>';break}}else v+="<td>-</td>",D=""}w=Math.floor(1E3*(B[0].value+B[1].value))/
...
function getLandBaseAerialPowers(a,b){var c=aerialSetting.item,d=aerialSetting.isCritical,e=aerialSetting.levelCriticalBonus,f=aerialSetting.isAllSlot,g=aerialSetting.manualSlot;b=aerialSetting.nishikiBonus*aerialSetting.contactBonus*aerialSetting.unionBonus*(void 0===b?1:b);var h=[],l={};a=$jscomp.makeIterator(a);for(var k=a.next();!k.done;l={$jscomp$loop$prop$a$373:l.$jscomp$loop$prop$a$373},k=a.next()){var m=k.value;k=LandBaseItem.getFirePower(c.id,f?m.slot:g,c.remodel,d,e);m=f?m.rate:1;l.$jscomp$loop$prop$a$373=
k*b;(k=h.find(function(n){return function(q){return q.power===n.$jscomp$loop$prop$a$373}}(l)))?k.rate+=m:h.push({power:l.$jscomp$loop$prop$a$373,rate:m});if(!f)break}return h}
function getLandBaseAerialPowersMod(a,b){var c=aerialSetting.item,d=aerialSetting.isCritical,e=aerialSetting.levelCriticalBonus,f=aerialSetting.isAllSlot,g=aerialSetting.manualSlot;b=aerialSetting.nishikiBonus*aerialSetting.contactBonus*aerialSetting.unionBonus*(void 0===b?1:b);var h=[],l={};a=$jscomp.makeIterator(a);for(var k=a.next();!k.done;l={$jscomp$loop$prop$a$375:l.$jscomp$loop$prop$a$375},k=a.next()){var m=k.value;k=LandBaseItem.getFirePower_SP(c,f?m.slot:g,d,e);m=f?m.rate:1;l.$jscomp$loop$prop$a$375=
k*b;(k=h.find(function(n){return function(q){return q.power===n.$jscomp$loop$prop$a$375}}(l)))?k.rate+=m:h.push({power:l.$jscomp$loop$prop$a$375,rate:m});if(!f)break}return h}
function getLandBaseAerialPowersMod2(a,b){var c=aerialSetting.item,d=aerialSetting.isCritical,e=aerialSetting.levelCriticalBonus,f=aerialSetting.isAllSlot,g=aerialSetting.manualSlot;b=aerialSetting.nishikiBonus*aerialSetting.contactBonus*aerialSetting.unionBonus*(void 0===b?1:b);var h=[],l={};a=$jscomp.makeIterator(a);for(var k=a.next();!k.done;l={$jscomp$loop$prop$a$377:l.$jscomp$loop$prop$a$377},k=a.next()){var m=k.value;k=LandBaseItem.getFirePower_SP2(c,f?m.slot:g,d,e);m=f?m.rate:1;l.$jscomp$loop$prop$a$377=
k*b;(k=h.find(function(n){return function(q){return q.power===n.$jscomp$loop$prop$a$377}}(l)))?k.rate+=m:h.push({power:l.$jscomp$loop$prop$a$377,rate:m});if(!f)break}return h}
...
function getLandBaseASWPower(a,b){var c=aerialSetting.item,d=aerialSetting.isCritical,e=aerialSetting.levelCriticalBonus,f=aerialSetting.isAllSlot,g=aerialSetting.manualSlot;b=aerialSetting.contactBonus*(void 0===b?1:b);var h=[];a=$jscomp.makeIterator(a);for(var l=a.next();!l.done;l=a.next()){l=l.value;var k=f?l.slot:g;k=0<k?c.asw*Math.sqrt(1.8*k)+25:0;for(var m=10<=c.asw?.7:.35,n=10<=c.asw?.3:.45,q=1/(100*n+1),p={},u=0;u<=100*n;p={$jscomp$loop$prop$power$381:p.$jscomp$loop$prop$power$381},u++){var t=
f?l.rate*q:q,x=Math.floor(softCap(k*(m+u/100),AS_CAP));d&&(x=Math.floor(1.5*x*e));p.$jscomp$loop$prop$power$381=x*(47===c.type?1.8:1)*b;RECONNAISSANCES.includes(c.type)&&(p.$jscomp$loop$prop$power$381=1);(x=h.find(function(v){return function(r){return r.power===v.$jscomp$loop$prop$power$381}}(p)))?x.rate+=t:h.push({power:p.$jscomp$loop$prop$power$381,rate:t})}if(!f)break}return h}
...
{$jscomp$loop$prop$a$383:p.$jscomp$loop$prop$a$383},u=q.next())p.$jscomp$loop$prop$a$383=u.value*c,p.$jscomp$loop$prop$a$383=f?Math.floor(1.5*p.$jscomp$loop$prop$a$383*g):p.$jscomp$loop$prop$a$383,(u=m.find(function(t){return function(x){return x.power===t.$jscomp$loop$prop$a$383}}(p)))?u.rate+=n:m.push({power:p.$jscomp$loop$prop$a$383,rate:n});if(!h)break}return m}
...
g*n.rate*q,x=f.find(function(v){return function(r){return r.damage===v.$jscomp$loop$prop$damage2$387}}(p));x?x.rate+=t:f.push({damage:p.$jscomp$loop$prop$damage2$387,rate:t})}else n=g*n.rate,(q=f.find(function(v){return function(r){return r.damage===v.$jscomp$loop$prop$damage$389}}(k)))?q.rate+=n:f.push({damage:k.$jscomp$loop$prop$damage$389,rate:n});return f}
function updateDetailChart(a,b,c){for(var d=[],e=[],f=[],g=[],h=getArrayMax(a),l=getArrayMin(a),k=a.length,m=0,n=0,q=0,p=0,u=0,t={$jscomp$loop$prop$i$391:l};t.$jscomp$loop$prop$i$391<=h;t={$jscomp$loop$prop$i$391:t.$jscomp$loop$prop$i$391},t.$jscomp$loop$prop$i$391++){var x=a.filter(function(v){return function(r){return r===v.$jscomp$loop$prop$i$391}}(t)).length;x&&(d.push(t.$jscomp$loop$prop$i$391),e.push((100*x/k).toFixed(1E5<=k?5:1E4<=k?4:1E3<=k?3:2)),g.push({slot:t.$jscomp$loop$prop$i$391,rate:x/
k}),u+=100*x/k,f.push(u.toFixed(1)),!m&&50<=u&&(m=t.$jscomp$loop$prop$i$391),!n&&90<=u&&(n=t.$jscomp$loop$prop$i$391),!q&&95<=u&&(q=t.$jscomp$loop$prop$i$391),!p&&99<=u&&(p=t.$jscomp$loop$prop$i$391))}$("#detail_max").text(h);$("#detail_min").text(l);$("#detail_50").text(m);$("#detail_90").text(n);$("#detail_95").text(q);$("#detail_99").text(p);chartInstance?(chartInstance.data.labels=d,chartInstance.data.datasets[0].data=e,chartInstance.data.datasets[1].data=f,chartInstance.options.scales.xAxes[0].scaleLabel.labelString=
...
function showItemToolTip(a,b){b=a.closest((void 0===b?0:b)?".lb_plane":".ship_plane");if(b.length&&b[0].dataset.planeid&&!b.hasClass("ui-draggable-dragging")){var c=castInt(b[0].dataset.planeid),d=castInt(b[0].getElementsByClassName("slot")[0].textContent),e=castInt(b[0].getElementsByClassName("remodel_value")[0].textContent),f=castInt(b[0].getElementsByClassName("prof_select")[0].dataset.prof);b=getItemTooltipContext(c,b[0].classList.contains("lb_plane"),d,e,f);showTooltip(a[0],b,"right")}}
...
n=g.range2&&n?n.name:"";d=d?'<span class="text_remodel">\u2605'+d+"</span>":"";var q=b?'<span class="ml-1 text_remodel">(+'+b.toFixed(2)+")</span>":"",p=c?'<span class="ml-1 text_remodel">(+'+c.toFixed(2)+")</span>":"",u=l?'<span class="ml-1 text_remodel">(+'+l.toFixed(2)+")</span>":"",t=m?'<span class="ml-1 text_remodel">(+'+m.toFixed(2)+")</span>":"",x=k?'<span class="ml-1 text_remodel">(+'+k.toFixed(2)+")</span>":"",v=h?'<span class="ml-1 text_remodel">(+'+h.toFixed(2)+")</span>":"",r=e?'<span class="ml-1 text_remodel">(+'+
e.toFixed(2)+")</span>":"",w=0<g.avoid?AVOID_TYPE.find(function(y){return y.id===g.avoid}):null,B=f.selectRate.length?f.selectRate.map(function(y){return(100*y).toFixed(0)+"%"}):null,D=PLANE_TYPE.includes(g.type)?getRequiredgBattleCountString(g.grow):null;return('\n\t<div class="text-left m-1 item_tooltip">\n\t\t<div>\n\t\t\t<img src="../img/type/icon'+g.itype+'.png" alt="'+g.itype+'" class="img-size-25">\n\t\t\t<span>'+g.name+"</span>"+d+'\n\t\t</div>\n\t\t<div class="d-flex flex-wrap my-1">\n\t\t\t'+
...
b[c][4].toFixed(1);chartInstance?(chartInstance.data.datasets[0].data=d,chartInstance.update()):chartInstance=new Chart(g,{type:"doughnut",data:{labels:e,datasets:[{backgroundColor:f,data:d,borderColor:h}]},plugins:[ChartDataLabels],options:{legend:{display:!1},title:{display:!0,text:"\u7b2c"+(a+1)+"\u57fa\u5730\u822a\u7a7a\u968a \u89e6\u63a5\u7387",fontColor:l},tooltips:{callbacks:{label:function(k,m){return m.labels[k.index]+": "+m.datasets[0].data[k.index].toFixed(1)+" %"}}},plugins:{datalabels:{color:mainColor,
formatter:function(k,m){return 0<k?k.toFixed(1)+"%":""}}}}});$("#modal_contact_detail").data("dispIndex",a);$("#modal_contact_detail").modal("show")}
...
function enemy_index_Clicked(a){a.closest(".enemy_content").hasClass("is_dead")?a.closest(".enemy_content").removeClass("is_dead"):a.closest(".enemy_content").addClass("is_dead");calculate(!1,!1,!0)}function enemy_name_Clicked(a){$target=a;a=castInt($target[0].dataset.enemyid);$("#enemy_type_select").change();0!==a?$("#modal_enemy_select").find(".btn_remove").prop("disabled",!1):$("#modal_enemy_select").find(".btn_remove").prop("disabled",!0);$("#modal_enemy_select").modal("show")}
...
0<d[0][0].length,!0,!1),document.getElementById("adapt_stage2").checked=!0,calculate(),showDeckSelector()&&inform_success("\u7de8\u6210\u306e\u53cd\u6620\u306b\u6210\u529f\u3057\u307e\u3057\u305f\u3002")):readEquipmentJson(c)?(updateItemList(),inform_success("\u6240\u6301\u88c5\u5099\u306e\u53cd\u6620\u306b\u6210\u529f\u3057\u307e\u3057\u305f\u3002")):readShipJson(c)?inform_success("\u6240\u6301\u8266\u5a18\u306e\u53cd\u6620\u306b\u6210\u529f\u3057\u307e\u3057\u305f\u3002"):inform_danger("\u5165\u529b\u5024\u306e\u8aad\u307f\u8fbc\u307f\u306b\u5931\u6557\u3057\u307e\u3057\u305f\u3002"),
...
document.addEventListener("DOMContentLoaded",function(){initialize(function(){var c=getUrlParams(),d=!1;if(c.hasOwnProperty("d"))expandMainPreset(decodePreset(c.d)),d=!0;else if(c.hasOwnProperty("predeck")||c.hasOwnProperty("lb")){c.hasOwnProperty("predeck")&&(d=readDeckBuilder(c.predeck))&&(expandMainPreset(d,0<d[0][0].length,!0,!1),document.getElementById("adapt_stage2").checked=!0);if(c.hasOwnProperty("lb"))try{var e=JSON.parse(decodeURIComponent(c.lb));2<=e.length&&expandMainPreset([e,[],[]],
!0,!1,!1)}catch(f){}d=!0}else if(c.hasOwnProperty("p"))expandMainPreset(decodePreset(c.p));else if(c=getActivePreset(),c.id)try{expandMainPreset(decodePreset(c.history.histories[c.history.index]))}catch(f){deleteLocalStorage("activePresets");window.location.href="../list/";return}else{window.location.href="../list/";return}d&&(c={id:getUniqueId(),name:"\u5916\u90e8\u30c7\u30fc\u30bf",history:{index:0,histories:[]},memo:""},d=loadLocalStorage("activePresets")||{activeId:"",presets:[]},5<d.presets.filter(function(f){return"\u5916\u90e8\u30c7\u30fc\u30bf"===
f.name}).length&&4===Math.floor(5*Math.random())&&(c.name="\u304a\u305d\u3068\u30c7\u30fc\u30bf"),d.presets.push(c),d.activeId=c.id,saveLocalStorage("activePresets",d),setTab());calculate();showDeckSelector()});$(document).keyup(function(c){isCtrlPress&&17===c.keyCode&&(isCtrlPress=!1)});$(document).keydown(function(c){isCtrlPress||17!==c.keyCode||(isCtrlPress=!0);isCtrlPress&&"z"===c.key?btn_undo_Clicked():isCtrlPress&&"y"===c.key&&btn_redo_Clicked()});document.getElementById("btn_top").addEventListener("click",
...
$("#friendFleet_content").on("click",".btn_ship_create",function(){btn_ship_create_Clicked($(this))});$("#friendFleet_content").on("click",".btn_reset_ship_plane",function(){btn_reset_ship_plane_Clicked($(this))});$("#friendFleet_content").on("click",".ship_name_span",function(){ship_name_span_Clicked($(this))});$("#friendFleet_content").on("click",".btn_remove_ship",function(){btn_remove_ship_Clicked($(this))});$("#friendFleet_content").on("click",".btn_remove_plane",function(){btn_remove_ship_plane_Clicked($(this))});
...
"#btn_reset_selected_plane_history",btn_reset_selected_plane_history_Clicked);$("#config_content").on("click","#btn_reset_selected_ship_history",btn_reset_selected_ship_history_Clicked);$("#ship_stock").on("click",".stock_td_fav",function(c){ship_stock_td_fav_Clicked($(this));c.stopPropagation()});$("#plane_stock").on("click",".stock_td_fav",function(c){stock_td_fav_Clicked($(this));c.stopPropagation()});$("#manage_manual_data").on("show.bs.collapse",".collapse",manage_manual_enemu_content_Shown);
...
"#plane_filter_key_select",plane_filter_Changed);$("#modal_plane_select").on("change","#plane_filter_value",plane_filter_Changed);$("#modal_plane_select").on("click",".toggle_display_type",function(){plane_type_select_Changed()});$("#modal_plane_select").on("click","#plane_table_thead .sortable_td",function(){sortable_td_Clicked($(this))});$("#modal_plane_preset").on("click",".preset_tr",function(){plane_preset_tr_Clicked($(this))});$("#modal_plane_preset").on("click",".btn_commit_preset",function(){btn_commit_plane_preset_Clicked($(this))});
...
"#frequent_ship",createShipTable);$("#modal_ship_select").on("click","#fav_only_ship",createShipTable);$("#modal_ship_select").on("click","#disp_in_stock_ship",createShipTable);$("#modal_ship_select").on("click","#disp_equipped_ship",createShipTable);$("#modal_ship_select").on("click","#ship_type_select .ship_type",function(){ship_type_select_Changed($(this))});$("#modal_ship_select").on("click",".toggle_display_type",createShipTable);$("#modal_ship_select").on("input","#ship_word",ship_word_TextChanged);
$("#modal_enemy_select").on("click",".modal-body .enemy",function(){modal_enemy_Selected($(this))});$("#modal_enemy_select").on("click",".btn_remove",function(){modal_enemy_select_btn_remove($(this))});$("#modal_enemy_select").on("click",".toggle_display_type",createEnemyTable);$("#modal_enemy_select").on("change","#enemy_type_select",createEnemyTable);$("#modal_enemy_select").on("input","#enemy_word",enemy_word_TextChanged);$("#modal_enemy_pattern").on("show.bs.modal",modal_enemy_pattern_Shown);
$("#modal_enemy_pattern").on("click",".node_tr",function(){node_tr_Clicked($(this))});$("#modal_enemy_pattern").on("click","#btn_zoom_map",btn_zoom_map_Clicked);$("#modal_enemy_pattern").on("change","#map_select",createNodeSelect);$("#modal_enemy_pattern").on("change","#node_select",createEnemyPattern);$("#modal_enemy_pattern").on("change","#select_difficulty",createNodeSelect);$("#modal_enemy_pattern").on("click","#btn_expand_enemies",btn_expand_enemies);$("#modal_enemy_pattern").on("click","#btn_continue_expand",
...
".reserve_ship",function(){reserve_ship_Clicked($(this))});$("#modal_ship_reserve").on("click",".btn_remove_reserve",function(c){btn_remove_reserve_Clicked($(this),c)});$("#modal_ship_reserve").on("click",".btn_reset",btn_reset_reserve_Clicked);$("#modal_include_selector").on("click",".fleet_selectable",function(){fleet_selectable_Clicked($(this))});$("#modal_include_selector").on("click","#btn_include_deck",btn_include_deck_Clicked);$("#modal_reference").on("click","#anti_air_ranking_normal_registed",
...
$("#modal_plane_select").on({mouseenter:function(){var c=$(this);timer=setTimeout(function(){showPlaneBasicToolTip(c)},500)},mouseleave:function(){timer&&clearTimeout(timer);hideTooltip($(this)[0])}},".plane_tr, .plane_tr_disabled");$("#modal_plane_select").on({mouseenter:function(){showTooltip($(this)[0],$(this).find("img").attr("alt"))},mouseleave:function(){hideTooltip($(this)[0])}},".item_type");$(window).resize(function(){timer&&clearTimeout(timer);timer=setTimeout(function(){"none"===$("#lb_tab_select").css("display")||
...
Sortable.create($("#battle_result")[0],{handle:".drag_handle",animation:200,scroll:!0});Sortable.create($("#lb_tab_parent")[0],{handle:".sortable_handle",animation:200,onEnd:function(){$(".lb_tab").each(function(c,d){$(d).attr("id","lb_item"+(c+1));$(d).find(".baseNo").html("<span>\u7b2c"+(c+1)+"\u57fa\u5730\u822a\u7a7a\u968a</span>");$(d).find(".simple_lb_progress").each(function(e,f){$(f).attr("id","simple_lb_bar_"+(c+1)+"_"+(e+1))});$(d).find(".btn_show_contact_rate_lb")[0].dataset.lb=c+1});inform_warning("\u57fa\u5730\u822a\u7a7a\u968a\u306e\u5165\u308c\u66ff\u3048\u304c\u884c\u308f\u308c\u307e\u3057\u305f\u3002\u57fa\u5730\u822a\u7a7a\u968a\u306e\u6d3e\u9063\u5148\u3092\u78ba\u8a8d\u3057\u3066\u304f\u3060\u3055\u3044\u3002");
$(".btn_lb_target").removeClass("btn-outline-success");$(".btn_lb_target").addClass("btn-outline-danger");$(".lb_target_alert").removeClass("d-none");calculate(!0,!1,!1)}});Sortable.create(document.getElementById("fleet_tabs_container"),{delay:80,animation:200,onEnd:function(){if("#friendFleet_item1"!==$("#fleet_tabs_container a:first").attr("href")){var c=1;$("#fleet_tabs_container a").each(function(h,l){$(l).text("\u7b2c"+(h+1)+"\u8266\u968a");$(l).attr("href","#friendFleet_item"+(h+1));$(l).hasClass("active")&&
(c=h+1)});for(var d=getMaxShipCount(),e=createFriendFleetPreset(),f=$jscomp.makeIterator(e),g=f.next();!g.done;g=f.next())g=g.value,g[2]+=g[2]>=d?-d:d;$(".friendFleet_tab").removeClass("show active");document.getElementById("friendFleet_item"+c).classList.add("show","active");expandMainPreset([[],e,[],document.getElementById("adapt_stage2").checked?selectedFormations:null,isDefMode,createAirRaidEnemyFleetPreset(),castInt($("#admin_lv").val())],!1,!0,!1);calculate(!1,!0,!1)}}});$(".ship_tab").draggable({delay:100,
helper:"clone",handle:".sortable_handle",zIndex:1E3});$(".ship_tab").droppable({accept:".ship_tab",hoverClass:"ship_tab_hover",tolerance:"pointer",drop:function(c,d){c=d.draggable;d=$(this);if(c.length&&d.length){var e=d.parent()[0],f=c[0],g=d[0],h=f.nextSibling;TradeHTMLWithAnimation(f,g,function(){e.insertBefore(f,g);e.insertBefore(g,h);calculate(!1,!0,!1)})}}});$(".lb_plane").draggable({delay:100,helper:"clone",handle:".drag_handle",zIndex:1E3,start:function(c,d){$(d.helper).css("width",320);$(d.helper).find(".dropdown-menu").remove()},
stop:function(){lb_plane_DragEnd($(this))}});$(".lb_plane").droppable({accept:".lb_plane",hoverClass:"plane_draggable_hover",tolerance:"pointer",drop:function(c,d){lb_plane_Drop($(this),d)}});$("#landBase_content_main").droppable({accept:".lb_plane",tolerance:"pointer",over:function(c,d){d.helper.animate({opacity:"1.0"},100);isOut=!1},out:function(c,d){d.helper.animate({opacity:"0.2"},100);isOut=!0}});$(".ship_plane_draggable").draggable({delay:100,helper:"clone",handle:".drag_handle",zIndex:1E3,
cursorAt:{left:55},start:function(c,d){ship_plane_DragStart(d)},stop:function(){ship_plane_DragEnd($(this))}});$(".ship_plane").droppable({accept:".ship_plane_draggable",hoverClass:"plane_draggable_hover",tolerance:"pointer",over:function(c,d){ship_plane_DragOver($(this),d)},drop:function(c,d){ship_plane_Drop($(this),d)}});$(".friendFleet_tab").droppable({accept:".ship_plane_draggable",tolerance:"pointer",over:function(c,d){isOut=!1;d.helper.stop().animate({opacity:"1.0"},100)},out:function(c,d){isOut=
!0;d.helper.stop().animate({opacity:"0.2"},100)}});Sortable.create($("#battle_container")[0],{handle:".sortable_handle",animation:150,onEnd:function(){$(".battle_content").each(function(c,d){$(d).find(".battle_no").text(c+1)});calculate()}});$("#main").fadeIn();$("#simulator_loading").remove();initSanma=setting.sanma?castInt(setting.sanma):0;var a=document.getElementById("sanma_parent"),b=document.createElement("img");b.src="../img/util/ammo.png";a.appendChild(b);a.onclick=function(){if(!a.classList.contains("avoid")||
8>Math.floor(11*Math.random()))a.classList.add("avoid"),$(a).animate({top:Math.max(Math.floor(Math.random()*(document.getElementById("main").clientHeight-50)),60)+"px",left:Math.max(Math.floor(Math.random()*(window.innerWidth-30)),30)+"px"},100);else{var c=document.createElement("div");b.classList.add("fadeOut");c.className="fadeUp fadeUp2";c.innerHTML="+1";a.appendChild(c);inform_success("\u6355\u307e\u3048\u305f\u79cb\u5200\u9b5a: "+(initSanma+1));setting.sanma=initSanma+1;initSanma++;saveSetting();
setTimeout(function(){b.classList.remove("fadeOut");a.removeChild(c);a.style.top="";a.style.left="";a.classList.remove("avoid")},1E3)}}});
