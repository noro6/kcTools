...
var AntiAirCutIn=function(){this.fixCorr=this.id=0;this.rateCorr=1;this.rate=0},FleetStage2=function(){this.cutin=null;this.border=this.rate=0;this.table=[]},ShootDownStatus=function(){this.minimumDown=this.fixDown=this.rateDown=0},LandBaseAll=function(){this.landBases=[];this.rocketBonus=this.defense2AirPower=this.defenseAirPower=0;this.manualCorr=1;this.superDefenseCorrC=this.superDefenseCorrB=this.superDefenseCorrA=0;this.superRocketCorr1=1;this.superRocketCorr2=0;this.sumSuperCorr=1;this.defense3AirPower=
0};
LandBaseAll.prototype.updateDefenseAirPower=function(){for(var a=0,b=0,c=0,d=0,e=$jscomp.makeIterator(this.landBases),f=e.next();!f.done;f=e.next())if(f=f.value,0===f.mode){b+=f.defenseAirPower;f=$jscomp.makeIterator(f.planes);for(var g=f.next();!g.done;g=f.next())g=g.value,ROCKETS.includes(g.id)&&(a+=1),[445,175,177,333].includes(g.id)&&(this.superDefenseCorrA+=.07),[263,354].includes(g.id)&&(c+=1),[334,452,446].includes(g.id)&&(d+=1)}this.rocketBonus=0===a?.5:1===a?.8:2===a?1.1:1.2;this.defenseAirPower=Math.floor(this.manualCorr*
b);this.defense2AirPower=Math.floor(this.rocketBonus*b);switch(c){case 0:this.superDefenseCorrB=0;break;case 1:this.superDefenseCorrB=.11;break;case 2:this.superDefenseCorrB=.14;break;default:this.superDefenseCorrB=.287}switch(d){case 0:this.superDefenseCorrC=0;break;case 1:this.superDefenseCorrC=.177;break;case 2:this.superDefenseCorrC=.287;break;default:this.superDefenseCorrC=.287}switch(a){case 0:this.superRocketCorr1=1;this.superRocketCorr2=.3;break;case 1:this.superRocketCorr1=1;this.superRocketCorr2=
.55;break;case 2:this.superRocketCorr1=1;this.superRocketCorr2=.85;break;case 3:this.superRocketCorr2=this.superRocketCorr1=1;break;default:this.superRocketCorr1=1,this.superRocketCorr2=1.07}this.sumSuperCorr=(this.superDefenseCorrA+this.superDefenseCorrB+this.superDefenseCorrC)*this.superRocketCorr1+this.superRocketCorr2;this.defense3AirPower=Math.floor(b*this.sumSuperCorr)};
...
c;isDefMode&&(b.fullAirPower=c,b.airPower=c,b.fullUnionAirPower=c,b.unionAirPower=c);c=0;if(isDefMode||f[0])c=f[0].getAirPower();u=!isDefMode&&document.getElementById("adapt_stage2").checked;r=[];u&&(r=castInt($("#fleet_formation").val()),b.isUnion&&f[g].cellType===CELL_TYPE.airRaid?b.updateStage2All(r,!0):b.updateStage2All(r),r=b.allStage2);for(w=0;w<d;w++){for(var y=0;y<q;y++){var C=f[y],z=C.enemies,A=!1;A=calculateLandBase(a,C,y);v+=doJetPhase(b,C);var B=C.isUnion?b.unionAirPower:b.airPower,D=
C.getAirPower(),E=getAirStatusIndex(B,D);y===g&&(h[E]+=1);l[y]+=B;k[y]+=D;shootDownFleet(E,b,C,y);C.cellType===t&&(shootDownEnemy(E,z,u,r),A=!0,E=getAirStatusIndex(b.airPower,C.getAirPower()),shootDownFleet(E,b,C,y,!1));if(y===g){shootDownEnemy(E,z,u,r);for(E=B=A=C=0;E<z.length;E++){var F=z[E];if(!F.onlyScout&&F.id){for(var G=F.slots,J=F.attackers.concat(),M=0;M<G.length;M++){var K=G[M],L=m[C];C+=1;L[2]+=K;L[3]<K&&(L[3]=K);L[4]>K&&(L[4]=K);0===K&&(J[M]=!1)}0>J.indexOf(!0)&&(x[A]+=1);A+=1;B+=F.airPower}F.slots=
F.fullSlots.concat();F.airPower=F.fullAirPower;F.landBaseAirPower=F.fullLandBaseAirPower}p[0].push(D);p[1].push(B);A=!1}if(A)for(z=$jscomp.makeIterator(z),D=z.next();!D.done;D=z.next())D=D.value,D.slots=D.fullSlots.concat(),D.airPower=D.fullAirPower,D.landBaseAirPower=D.fullLandBaseAirPower}for(y=0;3>y;y++)if(z=a.landBases[y],0!==z.mode){for(D=0;D<z.planes.length;D++)C=z.planes[D],C.slot=C.fullSlot,C.airPower=C.fullAirPower;z.airPower=z.fullAirPower}for(z=y=0;z<e;z++)if(D=b.ships[z],!D.ignoreSlot){C=
D.items.length;for(A=0;A<C;A++)B=D.items[A],B.isPlane&&(n[y++][w]=B.slot,B.slot=B.fullSlot,B.airPower=B.fullAirPower);D.airPower=D.fullAirPower}b.airPower=b.fullAirPower;b.escortAirPower=b.fullEscortAirPower;b.unionAirPower=b.fullUnionAirPower}for(t=f=0;t<e;t++)if(u=b.ships[t],!u.ignoreSlot)for(r=0;r<u.items.length;r++)u.items[r].isPlane&&(w=n[f],y={shipNo:u.shipNo,slot:u.items[r].slotNo,max:getArrayMax(w),min:getArrayMin(w),avg:w.reduce(function(H,I){return H+I})/d,death:w.filter(function(H){return 0===
H}).length,deathRate:0},y.deathRate=y.death/w.length,resultData.fleetSlots.push(y),f+=1);e=$("#shoot_down_table");for(n=0;n<q;n++)for(f=Math.round(l[n]/d),t=Math.round(k[n]/d),u=getAirStatusIndex(f,t),e.find(".fap.battle"+(n+1)).text(f),e.find(".eap.battle"+(n+1)).text(t),e.find(".cond.battle"+(n+1)).html("<span>"+AIR_STATUS[u].abbr+"</span>").removeClass("airStatus0 airStatus1 airStatus2 airStatus3 airStatus4").addClass("airStatus"+u),f=$jscomp.makeIterator(b.ships),t=f.next();!t.done;t=f.next())if(t=
t.value,!t.ignoreSlot)for(u=$jscomp.makeIterator(t.items),r=u.next();!r.done;r=u.next())r=r.value,r.isPlane&&(w=r.results[n],$("#s"+t.shipNo+"sl"+r.slotNo).find(".battle"+(n+1)).text(Math.round(w/d)));e.find("td").removeClass("d-none");for(b=battleCount+1;10>=b;b++)e.find(".battle"+b).addClass("d-none");isDefMode||(resultData.fleetAirPower=Math.round(l[g]/d));l=[];b=$jscomp.makeIterator(a.landBases);for(q=b.next();!q.done;q=b.next())for(q=q.value,e=0;2>e;e++)l.push(q.results[displayBattle].airStatusIndex[e].map(function(H){return H/
d}));a=$jscomp.makeIterator(a.landBases);for(q=a.next();!q.done;q=a.next())b=q.value,resultData.landBaseAirPowers.push(Math.round(b.results[g].mainAirPower[0]/d)),resultData.landBaseAirPowers.push(Math.round(b.results[g].mainAirPower[1]/d)),resultData.enemyAirPowers.push(Math.round(b.results[g].enemyAirPower[0]/d)),resultData.enemyAirPowers.push(Math.round(b.results[g].enemyAirPower[1]/d));isDefMode?resultData.enemyAirPowers.push(c):resultData.enemyAirPowers.push(Math.round(k[g]/d));resultData.landBaseAirStatus=
l;resultData.fleetAirStatus=h.map(function(H){return H/d});resultData.enemySlots=m;resultData.enemyAirPowerResults=p;resultData.enemySlotAllDead=x;resultData.usedSteels=v}
