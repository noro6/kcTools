...
for(var m=a.next();!m.done;h={$jscomp$loop$prop$o$411:h.$jscomp$loop$prop$o$411},m=a.next()){h.$jscomp$loop$prop$o$411=m.value;m=f.find(function(q){return function(v){return v.id===q.$jscomp$loop$prop$o$411.id}}(h));for(var k=0;10>=k;k++){var n=m.num[k];0!==n&&(n=Object.assign({remodel:k,count:n},h.$jscomp$loop$prop$o$411),n.antiAir+=Plane.getBonusAntiAir(n.id,n.type,n.remodel,n.antiAir),g.push(n))}}a=g.concat()}var l=$("#plane_sort_select").val();switch(l){case "battle_anti_air":a.sort(function(q,
v){return v.antiAir+1.5*v.interception-(q.antiAir+1.5*q.interception)});break;case "defense_anti_air":a.sort(function(q,v){return v.antiAir+v.interception+2*v.antiBomber-(q.antiAir+q.interception+2*q.antiBomber)});break;case "land_base_power":a.sort(function(q,v){return getLandBasePower(v.id,18,0)-getLandBasePower(q.id,18,0)});break;case "radius":a.sort(function(q,v){return v.radius-q.radius});break;case "history":if(setting.selectedHistory[0]){var r=setting.selectedHistory[0];a.sort(function(q,v){return r.filter(function(x){return x===
v.id}).length-r.filter(function(x){return x===q.id}).length})}case "id":case "cost":a.sort(function(q,v){return q[l]-v[l]});break;case "default":break;default:a.sort(function(q,v){return v[l]-q[l]})}var p=$("#plane_filter_key_select").val(),u=castInt($("#plane_filter_value").val());switch(p){case "battle_anti_air":a=a.filter(function(q){return q.antiAir+1.5*q.interception>=u});break;case "defense_anti_air":a=a.filter(function(q){return q.antiAir+q.interception+2*q.antiBomber>=u});break;default:a=
a.filter(function(q){return q[p]>=u})}createPlaneTable(a)}function plane_word_TextChanged(){!1!==timer&&clearTimeout(timer);timer=setTimeout(function(){plane_type_select_Changed()},250)}function fav_only_Checked_Chenged(){setting.favoriteOnly=$("#fav_only").prop("checked");saveSetting();plane_type_select_Changed()}function disp_in_stock_Checked_Chenged(){setting.inStockOnly=$("#disp_in_stock").prop("checked");saveSetting();plane_type_select_Changed()}
