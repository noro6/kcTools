...
M.$jscomp$loop$prop$item$304.id}}(y));if(k){if(-1===y.$jscomp$loop$prop$item$304.stock)continue;r=y.$jscomp$loop$prop$item$304.stock-(z?z.num[y.$jscomp$loop$prop$item$304.remodel]:0);if(!m&&1>r)continue;d=y.$jscomp$loop$prop$item$304.remodel}else{if(0<=y.$jscomp$loop$prop$item$304.stock)continue;var B=h.find(function(M){return function(K){return K.id===M.$jscomp$loop$prop$item$304.id}}(y)),C=z?getArraySum(z.num):0;x=B?getArraySum(B.num):0;if(0===x||!m&&C>=x)continue;r=x-C;for(x=10;0<=x;x--)if(z&&
(B.num[x]-=z.num[x]),0<B.num[x]){d=x;break}}}else if(0<y.$jscomp$loop$prop$item$304.remodel||0<=y.$jscomp$loop$prop$item$304.stock)continue;B="";B="multi"===g?"tr_multi px-1":"px-2";z=document.createElement("div");z.className="plane plane_tr general_tr d-flex py-2 py-lg-1 "+B;z.dataset.planeid=y.$jscomp$loop$prop$item$304.id;z.dataset.type=y.$jscomp$loop$prop$item$304.type;z.dataset.remodel=d;l&&m&&0>=r&&(z.classList.remove("plane_tr","plane","general_tr"),z.classList.add("plane_tr_disabled"));x=
document.createElement("div");x.className="align-self-center size-25";C=document.createElement("canvas");B=C.getContext("2d");C.width=25;C.height=25;B.drawImage(IMAGES["type"+y.$jscomp$loop$prop$item$304.itype],0,0,25,25);B=document.createElement("div");B.className="pl-1 d-flex align-self-center flex-grow-1 relative";var A=!1;if(16===y.$jscomp$loop$prop$item$304.itype&&8<=y.$jscomp$loop$prop$item$304.antiAir)A=!0;else if(21===y.$jscomp$loop$prop$item$304.type&&9<=y.$jscomp$loop$prop$item$304.antiAir)A=
!0;else if(350===y.$jscomp$loop$prop$item$304.id||351===y.$jscomp$loop$prop$item$304.id||352===y.$jscomp$loop$prop$item$304.id)A=!0;var E=document.createElement("div");E.className="pl-1 plane_td_name align-self-center"+(A?" special_item":"");E.textContent=y.$jscomp$loop$prop$item$304.name;B.appendChild(E);if(PLANE_TYPE.includes(y.$jscomp$loop$prop$item$304.type))if(SPECIAL_A.includes(y.$jscomp$loop$prop$item$304.id))A=document.createElement("div"),A.className="item_special flash A",A.textContent=
"A",B.appendChild(A);else if(SPECIAL_B.includes(y.$jscomp$loop$prop$item$304.id))A=document.createElement("div"),A.className="item_special flash B",A.textContent="B",B.appendChild(A);else if(SPECIAL_C1.includes(y.$jscomp$loop$prop$item$304.id))A=document.createElement("div"),A.className="item_special flash C1",A.innerHTML='C<span class="font_size_8">1</span>',B.appendChild(A);else if(SPECIAL_C2.includes(y.$jscomp$loop$prop$item$304.id))A=document.createElement("div"),A.className="item_special flash C2",
A.innerHTML='C<span class="font_size_8">2</span>',B.appendChild(A);else if(SPECIAL_C3.includes(y.$jscomp$loop$prop$item$304.id))A=document.createElement("div"),A.className="item_special flash C3",A.innerHTML='C<span class="font_size_8">3</span>',B.appendChild(A);else if(SPECIAL_C4.includes(y.$jscomp$loop$prop$item$304.id))A=document.createElement("div"),A.className="item_special flash C4",A.innerHTML='C<span class="font_size_8">4</span>',B.appendChild(A);else{if(t)continue}else if(t)continue;A=document.createElement("div");
A.className="ml-2 plane_td_stock align-self-center";A.textContent="\u00d7"+(0<r?r:0);E=document.createDocumentFragment();var F=document.createElement("div");F.className="ml-auto plane_td_basic align-self-center";var H=document.createElement("div");H.className="plane_td_basic align-self-center";var G=document.createElement("div");G.className="plane_td_basic align-self-center d-none d-lg-block";var I=document.createElement("div");I.className="plane_td_basic align-self-center d-none d-lg-block";var L=
document.createElement("div");L.className="plane_td_basic align-self-center d-none d-lg-block";F.textContent=sFormat(y.$jscomp$loop$prop$item$304[u[0]],u[0]);H.textContent=sFormat(y.$jscomp$loop$prop$item$304[u[1]],u[1]);G.textContent=sFormat(y.$jscomp$loop$prop$item$304[u[2]],u[2]);I.textContent=sFormat(y.$jscomp$loop$prop$item$304[u[3]],u[3]);L.textContent=sFormat(y.$jscomp$loop$prop$item$304[u[4]],u[4]);if("multi"===g&&v!==y.$jscomp$loop$prop$item$304.type&&"default"===q){v=document.createElement("div");
v.className="type_divider mt-3 mb-1";var J=document.createElement("div");J.className="font_size_12 font_color_half align-self-center";J.textContent=ITEM_API_TYPE.find(function(M){return function(K){return K.id===M.$jscomp$loop$prop$item$304.type}}(y)).name;var N=document.createElement("div");N.className="flex-grow-1 border-bottom align-self-center mx-2";v.appendChild(J);v.appendChild(N);f.appendChild(v)}x.appendChild(C);z.appendChild(x);k&&0<y.$jscomp$loop$prop$item$304.remodel&&(v=document.createElement("div"),
v.className="plane_td_remodel text_remodel align-self-center",v.dataset.remodel=y.$jscomp$loop$prop$item$304.remodel,x="",x="multi"===g?"\u2605"+y.$jscomp$loop$prop$item$304.remodel:10>y.$jscomp$loop$prop$item$304.remodel?"\u2605+"+y.$jscomp$loop$prop$item$304.remodel:"\u2605MAX",v.textContent=x,B.appendChild(v),B.appendChild(A));0<r&&B.appendChild(A);z.appendChild(B);"single"===g&&(E.appendChild(F),E.appendChild(H),E.appendChild(G),E.appendChild(I),E.appendChild(L),z.appendChild(E));f.appendChild(z);
v=y.$jscomp$loop$prop$item$304.type}f.childNodes.length||(g=document.createElement("div"),g.className="text-center",l&&!m&&(g.textContent+="\u88c5\u5099\u6240\u6301\u6570\u304c\u767b\u9332\u3055\u308c\u3066\u3044\u306a\u3044\u304b\u3001\u5168\u3066\u306e\u88c5\u5099\u304c\u914d\u5099\u6e08\u307f\u3067\u3059\u3002"),l&&!m&&(g.textContent+="\u88c5\u5099\u6240\u6301\u6570\u304c\u767b\u9332\u3055\u308c\u3066\u3044\u306a\u3044\u304b\u3001\u5168\u3066\u306e\u88c5\u5099\u304c\u914d\u5099\u6e08\u307f\u3067\u3059\u3002"));
...
l;saveSetting();g?$("#disp_equipped_ship").closest("div").removeClass("d-none"):$("#disp_equipped_ship").closest("div").addClass("d-none");var q=SHIP_DATA.filter(function(C){return e?C.name.includes(e):n.includes(C.type)&&(d?0<C["final"]:!0)});setting.hasSlotOnly&&(q=q.filter(function(C){return C.slot.some(function(A){return 0<A})}));if(l){var p=setting.favoriteShip;q=q.filter(function(C){return p.includes(C.id)})}if(g){l={};m=$jscomp.makeIterator(k);for(var t=m.next();!t.done;l={$jscomp$loop$prop$stock$306:l.$jscomp$loop$prop$stock$306,
$jscomp$loop$prop$baseLv$307:l.$jscomp$loop$prop$baseLv$307,$jscomp$loop$prop$baseArea$308:l.$jscomp$loop$prop$baseArea$308},t=m.next())if(l.$jscomp$loop$prop$stock$306=t.value,!(0>=l.$jscomp$loop$prop$stock$306.details.length||(l.$jscomp$loop$prop$stock$306.details.sort(function(C,A){return C.lv-A.lv}),t=q.findIndex(function(C){return function(A){return A.id===C.$jscomp$loop$prop$stock$306.id}}(l)),0>t))){var u=q[t];l.$jscomp$loop$prop$baseLv$307=l.$jscomp$loop$prop$stock$306.details[0].lv;l.$jscomp$loop$prop$baseArea$308=
l.$jscomp$loop$prop$stock$306.details[0].area<=MAX_AREA?l.$jscomp$loop$prop$stock$306.details[0].area:0;u.lv=l.$jscomp$loop$prop$baseLv$307;u.area=l.$jscomp$loop$prop$baseArea$308;u.stock=l.$jscomp$loop$prop$stock$306.details.filter(function(C){return function(A){return A.lv===C.$jscomp$loop$prop$baseLv$307&&A.area===C.$jscomp$loop$prop$baseArea$308}}(l)).length;var w=[{lv:l.$jscomp$loop$prop$baseLv$307,area:l.$jscomp$loop$prop$baseArea$308}];if(2<=l.$jscomp$loop$prop$stock$306.details.length)for(var v=
{},r=1;r<l.$jscomp$loop$prop$stock$306.details.length;v={$jscomp$loop$prop$lv$312:v.$jscomp$loop$prop$lv$312,$jscomp$loop$prop$area$313:v.$jscomp$loop$prop$area$313},r++)if(v.$jscomp$loop$prop$lv$312=l.$jscomp$loop$prop$stock$306.details[r].lv,v.$jscomp$loop$prop$area$313=l.$jscomp$loop$prop$stock$306.details[r].area<=MAX_AREA?l.$jscomp$loop$prop$stock$306.details[r].area:0,!w.find(function(C){return function(A){return A.lv===C.$jscomp$loop$prop$lv$312&&A.area===C.$jscomp$loop$prop$area$313}}(v))){var x=
Object.assign({},u);x.lv=v.$jscomp$loop$prop$lv$312;x.area=v.$jscomp$loop$prop$area$313;x.stock=l.$jscomp$loop$prop$stock$306.details.filter(function(C){return function(A){return A.lv===C.$jscomp$loop$prop$lv$312&&A.area===C.$jscomp$loop$prop$area$313}}(v)).length;q.splice(t,0,x);w.push({lv:v.$jscomp$loop$prop$lv$312,area:v.$jscomp$loop$prop$area$313})}}}if(f&&setting.selectedHistory[1]){var y=setting.selectedHistory[1];q.sort(function(C,A){return y.filter(function(E){return E===A.id}).length-y.filter(function(E){return E===
C.id}).length})}q.sort(function(C,A){return 7===C.type&&7!==A.type||7===A.type&&7!==C.type?A.type-C.type:6===C.type&&6!==A.type||6===A.type&&6!==C.type?A.type-C.type:10===C.type&&10!==A.type||10===A.type&&10!==C.type?A.type-C.type:C.type-A.type});l=document.createDocumentFragment();t=w=0;m={};q=$jscomp.makeIterator(q);for(u=q.next();!u.done;m={$jscomp$loop$prop$ship$317:m.$jscomp$loop$prop$ship$317},u=q.next()){m.$jscomp$loop$prop$ship$317=u.value;v=1;if(g&&k){if(!m.$jscomp$loop$prop$ship$317.lv)continue;
m.$jscomp$loop$prop$ship$317.stock&&(v=m.$jscomp$loop$prop$ship$317.stock);u=usedShip.filter(function(C){return function(A){return A.id===C.$jscomp$loop$prop$ship$317.id&&A.lv===C.$jscomp$loop$prop$ship$317.lv&&A.area===C.$jscomp$loop$prop$ship$317.area}}(m)).length;v-=u;if(0>=v&&!h)continue}u=document.createElement("div");u.className=g&&h&&!v?"ship_tr ship_tr_disabled d-flex "+("multi"===c?"tr_multi":""):"ship_tr general_tr d-flex "+("multi"===c?"tr_multi":"");u.dataset.shipid=m.$jscomp$loop$prop$ship$317.id;
...
D});if(B){var C=$(z);setShipDiv(C,B[0],99,8<=B.length&&B[7]?B[7]:0);C.find(".ship_plane").each(function(E,F){var H=$(F),G=B[1].find(function(I){return I[4]===E});!G&&H.hasClass("expanded_slot")&&(G=B[1].find(function(I){return-1===I[4]}));G&&!H.hasClass("d-none")&&(setPlaneDiv(H,{id:G[0],prof:G[1],remodel:G[2],slot:G[3]},!0),6<=G.length&&G[5]&&plane_unlock_Clicked(H.find(".plane_unlock")))});if(4<=B.length){var A=C.find(".ship_disabled");B[3]?(A.addClass("disabled"),A.children().addClass("fa-eye-slash").removeClass("fa-eye no_capture"),
C.addClass("opacity4")):(A.removeClass("disabled"),A.children().removeClass("fa-eye-slash").addClass("fa-eye no_capture"),C.removeClass("opacity4"))}5<=B.length&&(B[4]?C.find(".ship_level").text(B[4]):C.find(".ship_level").text(99));7<=B.length&&0<B[6]&&C.find(".ship_luck").text(B[6]);D<g?h+=1:l+=1}});display_ship_count_Changed($("#friendFleet_item1"),Math.max(h,1),!0);display_ship_count_Changed($("#friendFleet_item2"),Math.max(l,1),!0)}document.getElementById("fleet_formation").value=1;if(4<=a.length&&
...
castInt(B[3]);p?$(z).find(".cell_type").val([0,5].includes(C)?5:3):$(z).find(".cell_type").val(C);cell_type_Changed($(z).find(".cell_type"),!1)}else $(z).find(".cell_type").val(1);5<=B.length?$(z).find(".formation").val(B[4]):$(z).find(".formation").val(1);6<=B.length?$(z).find(".enemy_range").text(B[5]):$(z).find(".enemy_range").text(0);C=0;B=$jscomp.makeIterator(B[1]);for(var A=B.next();!A.done;A=B.next()){A=A.value;var E=$(z).find(".enemy_content").eq(C++);0<A?setEnemyDiv(E,A):0>A?setEnemyDiv(E,
-1,-A):clearEnemyDiv(E);E.addClass("d-flex").removeClass("d-none")}}});if(6<=a.length){var t=a[5];if(t&&3===t.length){var u=$("#air_raid_enemies"),w=castInt(t[1]);u.find(".cell_type").val([0,5].includes(w)?5:3);cell_type_Changed(u.find(".cell_type"),!1);u.find(".formation").val(t[2]);d=0;for(var v=$jscomp.makeIterator(t[0]),r=v.next();!r.done;r=v.next()){var x=r.value,y=u.find(".enemy_content").eq(d++);0<x?setEnemyDiv(y,x):0>x?setEnemyDiv(y,-1,-x):clearEnemyDiv(y);y.addClass("d-flex").removeClass("d-none")}}}7<=
...
function updateFleetView(){var a=inputItems.fleet,b=document.getElementById("shoot_down_table"),c=document.getElementById("shoot_down_table_tbody"),d=-1;b.getElementsByTagName("tbody")[0].innerHTML="";b=$jscomp.makeIterator(b.getElementsByTagName("tfoot")[0].getElementsByClassName("td_battle"));for(var e=b.next();!e.done;e=b.next())e.value.innerHTML="";$("#fleet_formation").find("option").each(function(z,B){var C=castInt($(B).val());a.isUnion&&10<C?$(B).removeClass("d-none"):!a.isUnion&&10<C&&$(B).addClass("d-none")});
$("#fleet_antiair_formation").find("option").each(function(z,B){var C=castInt($(B).val());a.isUnion&&10<C?$(B).removeClass("d-none"):!a.isUnion&&10<C&&$(B).addClass("d-none")});setting.visibleAntiAirStatus?($(".anti_air_status").removeClass("d-none"),$(".anti_air_status").addClass("d-flex")):($(".anti_air_status").addClass("d-none"),$(".anti_air_status").removeClass("d-flex"));b=document.getElementsByClassName("fleet_ap")[0];e=document.getElementsByClassName("fleet_ap")[1];for(var f=document.getElementsByClassName("contact_start_rate")[0],
...
c);c=0;if(isDefMode||f[0])c=f[0].getAirPower();w=!isDefMode&&document.getElementById("adapt_stage2").checked;v=[];w&&(v=castInt($("#fleet_formation").val()),b.isUnion&&f[g].cellType===CELL_TYPE.airRaid?b.updateStage2All(v,!0):b.updateStage2All(v),v=b.allStage2);for(r=0;r<d;r++){for(var x=0;x<p;x++){var y=f[x],D=y.enemies,z=!1;z=calculateLandBase(a,y,x);t+=doJetPhase(b,y);var B=y.isUnion?b.unionAirPower:b.airPower,C=y.getAirPower(),A=getAirStatusIndex(B,C);x===g&&(h[A]+=1);l[x]+=B;k[x]+=C;shootDownFleet(A,
b,y,x);y.cellType===CELL_TYPE.aerialCombat&&(shootDownEnemy(A,D,w,v),z=!0,A=getAirStatusIndex(b.airPower,y.getAirPower()),shootDownFleet(A,b,y,x,!1));if(x===g){shootDownEnemy(A,D,w,v);for(A=B=z=y=0;A<D.length;A++){var E=D[A];if(!E.onlyScout&&E.id){for(var F=E.slots,H=E.attackers.concat(),G=0;G<F.length;G++){var I=F[G],L=n[y];y+=1;L[2]+=I;L[3]<I&&(L[3]=I);L[4]>I&&(L[4]=I);0===I&&(H[G]=!1)}0>H.indexOf(!0)&&(u[z]+=1);z+=1;B+=E.airPower}E.slots=E.fullSlots.concat();E.airPower=E.fullAirPower;E.landBaseAirPower=
E.fullLandBaseAirPower}q[0].push(C);q[1].push(B);z=!1}if(z)for(D=$jscomp.makeIterator(D),C=D.next();!C.done;C=D.next())C=C.value,C.slots=C.fullSlots.concat(),C.airPower=C.fullAirPower,C.landBaseAirPower=C.fullLandBaseAirPower}for(x=0;3>x;x++)if(D=a.landBases[x],0!==D.mode){for(C=0;C<D.planes.length;C++)y=D.planes[C],y.slot=y.fullSlot,y.airPower=y.fullAirPower;D.airPower=D.fullAirPower}for(D=x=0;D<e;D++)if(C=b.ships[D],!C.ignoreSlot){y=C.items.length;for(z=0;z<y;z++)B=C.items[z],B.isPlane&&(m[x++][r]=
B.slot,B.slot=B.fullSlot,B.airPower=B.fullAirPower);C.airPower=C.fullAirPower}b.airPower=b.fullAirPower;b.escortAirPower=b.fullEscortAirPower;b.unionAirPower=b.fullUnionAirPower}for(w=f=0;w<e;w++)if(v=b.ships[w],!v.ignoreSlot)for(r=0;r<v.items.length;r++)v.items[r].isPlane&&(x=m[f],D={shipNo:v.shipNo,slot:v.items[r].slotNo,max:getArrayMax(x),min:getArrayMin(x),avg:x.reduce(function(J,N){return J+N})/d,death:x.filter(function(J){return 0===J}).length,deathRate:0},D.deathRate=D.death/x.length,resultData.fleetSlots.push(D),
...
function landBaseDetailCalculate(a,b,c){c=void 0===c?!1:c;setPreCalculateTable();var d=createLandBaseInstance(),e=updateEnemyFleetInfo(!1),f=castInt($("#detail_calculate_count").val()),g=[],h=isDefMode?0:displayBattle,l=d.landBases[a].planes,k=0;c&&0===b&&!ATTACKERS.includes(l[b].type)&&l.some(function(A){return ATTACKERS.includes(A.type)})&&(b=l.findIndex(function(A){return ATTACKERS.includes(A.type)}));for(var m=0;m<l.length;m++){var n=l[m];0===l[b].id&&0!==n.id&&(b=m);k+=n.fullSlot}m=[];for(n=
...
for(t=p.next();!t.done;t=p.next())t=t.value,t.slots=t.fullSlots.concat(),t.airPower=t.fullAirPower,t.landBaseAirPower=t.fullLandBaseAirPower}for(q=0;3>q;q++){p=d.landBases[q];for(u=t=0;u<p.planes.length;u++)w=p.planes[u],a+1===p.baseNo&&b===u&&g.push(w.slot),t+=w.slot,w.slot=w.fullSlot,w.updateAirPower();a+1===p.baseNo&&m.push(t);p.updateAirPower()}}var D=l[b].id,z=l[b].remodel,B=l.find(function(A){return 312===A.id})?1.15:l.find(function(A){return 311===A.id})?1.125:1,C=e.battles[h].isUnion?1.1:
1;chartSlotDistribution=updateDetailChart(g,"\u6b8b\u6a5f\u6570 [\u6a5f]",{mode:"index",callbacks:{title:function(A,E){var F=A[0].xLabel,H="\u822a\u7a7a\u6226\u706b\u529b\uff1a"+(LandBaseItem.getFirePower(D,F,z)*B*C).toFixed(1);return F?"\u6b8b\u6a5f\u6570\uff1a"+F+" \u6a5f\n"+H:"\u6b8b\u6a5f\u6570\uff1a0 \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0"},label:function(A,E){return[A.yLabel+" %"]}}});n=h=g=0;m=$jscomp.makeIterator(m);for(q=m.next();!q.done;q=m.next())q=q.value,p=q/k,0===q?n+=1:.25>p?
...
m={};g=$jscomp.makeIterator(d.landBases[a].planes);for(h=g.next();!h.done;m={$jscomp$loop$prop$pl$354:m.$jscomp$loop$prop$pl$354},h=g.next())m.$jscomp$loop$prop$pl$354=h.value,n=(h=ITEM_DATA.find(function(A){return function(E){return E.id===A.$jscomp$loop$prop$pl$354.id}}(m)))?h.name:"-",q="",FIGHTERS.includes(h.type)?q=h.antiAir?'<span class="mx-1">\u5bfe\u7a7a: '+h.antiAir+"</span>":"":RECONNAISSANCES.includes(h.type)?(q=h.antiAir?'<span class="mx-1">\u5bfe\u7a7a: '+h.antiAir+"</span>":"",q+='<span class="mx-1">\u7d22\u6575: '+
...
f);document.getElementById("stage3_all_slot_label").textContent="\u6b8b\u6a5f\u6570\u5206\u5e03\u306e\u7d50\u679c\u3092\u5229\u7528\uff08"+getArrayMin(b)+"\u6a5f ~ "+f+"\u6a5f\uff09";document.getElementById("stage3_remodel").value=z?z:0;l.some(function(A){return 311===A.id})&&(document.getElementById("enabled_rikutei").checked=!0);l.some(function(A){return 312===A.id})&&(document.getElementById("enabled_rikutei_z").checked=!0);l='<option value="99">\u4efb\u610f</option>';e=e.battles.length-1;b=castInt(document.getElementById("target_enemy").value,
...
$jscomp$loop$prop$borderMax$371:z.$jscomp$loop$prop$borderMax$371},B++){var C=y[B];z.$jscomp$loop$prop$borderMin$370=C.min;z.$jscomp$loop$prop$borderMax$371=C.max;var A=x.filter(function(E){return function(F){return F.damage>=E.$jscomp$loop$prop$borderMin$370&&F.damage<E.$jscomp$loop$prop$borderMax$371}}(z)).map(function(E){return E.rate});A=A.length===x.length?1:getArraySum(A);C.value=A;if(b.isAttacker||!c||u.type.includes(18)&&h){if(c&&u.type.includes(18)&&!h||!c&&[13,14].includes(u.type)){v+='<td class="text-center" colspan="'+
y.length+'">\u5bfe\u6f5c\u653b\u6483\u4e0d\u53ef</td>';D="";x=$jscomp.makeIterator(y);for(z=x.next();!z.done;z=x.next())z.value.value=0;break}if(1!==A||0!==B||n)1===A?v+='<td>100 <span class="font_size_11 font_color_half">%</span></td>':(C=Math.floor(1E3*A)/10,v+="<td>"+(0===C&&0!==A?.1:C)+' <span class="font_size_11 font_color_half">%</span></td>');else{v+='<td colspan="'+y.length+'" class="text-center text_red">\u78ba\u6bba</td>';break}}else v+="<td>-</td>",D=""}x=Math.floor(1E3*(y[0].value+y[1].value))/
