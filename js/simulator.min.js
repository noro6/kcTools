...
function shootDownFriend(a,b,c,d){var e=c.stage2[0][0].length;5===a&&(a=0);for(var f=SHOOT_DOWN_TABLE,g=c.cellType<=CELL_TYPE.normal,h=c.cellType<=CELL_TYPE.grand,m=b.ships.length,k=0;k<m;k++){for(var n=b.ships[k],l=g&&n.isEscort,u=n.planes.length,p=0;p<u;p++){var r=n.planes[p];r.results[d]+=r.slot;if(!(0>=r.slot||!r.canBattle||l)&&e){var q=f[r.slot][a];q=q[Math.floor(Math.random()*q.length)];r.slot-=Math.floor(9===r.type?.6*q:q);if(r.isAttacker&&h){q=Math.floor(Math.random()*e);var v=c.stage2[r.avoid];
Math.floor(2*Math.random())&&(r.slot-=Math.floor(v[0][q]*r.slot));Math.floor(2*Math.random())&&(r.slot-=v[1][q])}r.canBattle&&0<r.slot?r.airPower=Math.floor(r.antiAir*Math.sqrt(r.slot)+r.bonusAirPower):(0>=r.slot&&(r.slot=0),r.airPower=0)}}n.updateAirPower()}b.updateAirPower()}
