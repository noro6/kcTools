...
444===b.id&&(t=getLandBaseAerialPowers(n,1,1,1.15)):(p.type.includes(12)||p.type.includes(15)||p.type.includes(17))&&444===b.id&&(t=getLandBaseAerialPowers(n,1,1,1.15)),t.length||(t=getLandBaseAerialPowers(n))):t=getLandBaseAerialPowers(n,1,3.2):t=c?getFleetAerialPower(n,p,1,l):getEnemyAerialPowers(n,p);var x=p.hp,u=createDamageDistribution(t,v,h,c,x),q=u.map(function(B){return B.damage}),w=getArrayMin(q),y=getArrayMax(q);q=[{min:x,max:getArrayMax(u.map(function(B){return B.damage}))+1,value:0},{min:Math.ceil(.75*
x),max:x,value:0},{min:Math.ceil(.5*x),max:Math.ceil(.75*x),value:0}];t=t.map(function(B){return B.power}).some(function(B){return 0>=B});w=(1<=w?w:t?"0":"\u5272\u5408")+"~"+(1<=y?y:"\u5272\u5408");k||0!==g||(w="-");t="";y={};for(var C=0;C<q.length;y={$jscomp$loop$prop$borderMin$368:y.$jscomp$loop$prop$borderMin$368,$jscomp$loop$prop$borderMax$369:y.$jscomp$loop$prop$borderMax$369},C++){var z=q[C];y.$jscomp$loop$prop$borderMin$368=z.min;y.$jscomp$loop$prop$borderMax$369=z.max;var A=u.filter(function(B){return function(D){return D.damage>=
...
(z=Math.floor(1E3*A)/10,t+="<td>"+(0===z&&0!==A?.1:z)+' <span class="font_size_11 font_color_half">%</span></td>');else{t+='<td colspan="'+q.length+'" class="text-center text_red">\u78ba\u6bba</td>';break}}else t+="<td>-</td>",w=""}u=Math.floor(1E3*(q[0].value+q[1].value))/10;y=Math.floor(1E3*(q[0].value+q[1].value+q[2].value))/10;m+='\n\t\t<tr class="stage3_calc_detail '+(90<=Math.floor(1E3*q[0].value)/10?"tr_death":70<=u?"tr_damaged":50<=y?"tr_half_damaged":"")+'"\n\t\t\tdata-toggle="tooltip" data-html="true" title="\n\t\t<div class=\'text-left\'>\n\t\t\t<div>'+
