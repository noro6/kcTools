...
[1,17,19,20,21,22];break;default:v.push(p)}setting.visibleFinal=d;setting.orderByFrequency=f;setting.inStockOnlyShip=g;setting.visibleEquippedShip=h;setting.favoriteOnlyShip=n;saveSetting();g?($("#disp_equipped_ship").closest("div").removeClass("d-none"),$("#disp_slot_ex_ship").closest("div").removeClass("d-none")):($("#disp_equipped_ship").closest("div").addClass("d-none"),$("#disp_slot_ex_ship").closest("div").addClass("d-none"));p=SHIP_DATA.filter(function(C){return e?C.id==e||C.name.includes(e):
v.includes(C.type)&&(d?0<C.final:!0)});if(n){var u=setting.favoriteShip;p=p.filter(function(C){return u.includes(C.id)})}if(g){n={};for(var y=$jscomp.makeIterator(q),t=y.next();!t.done;n={$jscomp$loop$prop$stock$302:n.$jscomp$loop$prop$stock$302,$jscomp$loop$prop$baseLv$303:n.$jscomp$loop$prop$baseLv$303,$jscomp$loop$prop$baseArea$304:n.$jscomp$loop$prop$baseArea$304},t=y.next())if(n.$jscomp$loop$prop$stock$302=t.value,t=m?n.$jscomp$loop$prop$stock$302.details.filter(function(C){return 1===C.ex||
-1===C.ex}):n.$jscomp$loop$prop$stock$302.details,0>=t.length)p=p.filter(function(C){return function(F){return F.id!==C.$jscomp$loop$prop$stock$302.id}}(n));else{t.sort(function(C,F){return C.lv-F.lv});var r=p.findIndex(function(C){return function(F){return F.id===C.$jscomp$loop$prop$stock$302.id}}(n));if(!(0>r)){var w=p[r];n.$jscomp$loop$prop$baseLv$303=t[0].lv;var z=t[0].st[4];n.$jscomp$loop$prop$baseArea$304=t[0].area<=MAX_AREA&&0<t[0].area?t[0].area:-1;w.lv=n.$jscomp$loop$prop$baseLv$303;w.area=
n.$jscomp$loop$prop$baseArea$304;w.upLuck=z;w.stock=t.filter(function(C){return function(F){return F.lv===C.$jscomp$loop$prop$baseLv$303&&F.area===C.$jscomp$loop$prop$baseArea$304}}(n)).length;w.ex=t[0].ex;z=[{lv:n.$jscomp$loop$prop$baseLv$303,area:n.$jscomp$loop$prop$baseArea$304}];if(2<=t.length)for(var A={},x=1;x<t.length;A={$jscomp$loop$prop$lv$308:A.$jscomp$loop$prop$lv$308,$jscomp$loop$prop$area$309:A.$jscomp$loop$prop$area$309},x++){A.$jscomp$loop$prop$lv$308=t[x].lv;var B=t[x].st[4];A.$jscomp$loop$prop$area$309=
t[x].area<=MAX_AREA&&0<t[x].area?t[x].area:-1;if(!z.find(function(C){return function(F){return F.lv===C.$jscomp$loop$prop$lv$308&&F.area===C.$jscomp$loop$prop$area$309}}(A))){var E=Object.assign({},w);E.lv=A.$jscomp$loop$prop$lv$308;E.area=A.$jscomp$loop$prop$area$309;E.stock=t.filter(function(C){return function(F){return F.lv===C.$jscomp$loop$prop$lv$308&&F.area===C.$jscomp$loop$prop$area$309}}(A)).length;E.upLuck=B;E.ex=t[x].ex;p.splice(r,0,E);z.push({lv:A.$jscomp$loop$prop$lv$308,area:A.$jscomp$loop$prop$area$309})}}}}}if(f&&
...
'<div class="font_size_12">\u8266\u968a\u9632\u7a7a: '+f.antiAirBonus.toFixed(2)+"</div>":"")+"\n\t\t"+(f.isJet&&f.slot?'<div class="font_size_12">\u92fc\u6750\u6d88\u8cbb (\u5674\u5f0f\u5f37\u8972\u767a\u751f\u6642): '+Math.round(f.slot*f.cost*.2):"")+"\n\t\t"+(z&&z.length?'<div class="font_size_12">\u89e6\u63a5\u9078\u629e\u7387(\u78ba\u4fdd\u6642): '+z[0]+"</div>":"")+"\n\t\t"+(A?'<div class="font_size_12">\u719f\u7df4\u5ea6<img class="img-size-18_lock" alt=">>" src="../img/util/prof7.png">\u307e\u3067: '+
A+"</div>":"")+"\n\t</div>").trim()}
...
9>Math.floor(11*Math.random())){a.classList.add("avoid");var c=$(a);c.find("img").css({transform:"rotate("+360*Math.random()+"deg)"});c.animate({top:Math.max(Math.floor(Math.random()*(document.getElementById("main").clientHeight-50)),60)+"px",left:Math.max(Math.floor(Math.random()*(window.innerWidth-30)),30)+"px"},100)}else{c=a.classList.contains("gold")?10:1;var d=document.createElement("div");b.classList.add("fadeOut");d.className="fadeUp fadeUp2 "+(1<c?"text-warning":"");d.innerHTML="+"+c;a.appendChild(d);
a.classList.remove("gold");inform_success("\u6355\u307e\u3048\u305f\u79cb\u5200\u9b5a: "+(initSanma+c));setting.sanma=initSanma+c;initSanma+=c;saveSetting();setTimeout(function(){b.classList.remove("fadeOut");a.removeChild(d);a.style.top="";a.style.left="";a.classList.remove("avoid");6>Math.floor(101*Math.random())&&a.classList.add("gold")},1E3)}}});
