...
k=c.length,m=usedPlane.concat(),l=0,u=-1,p=0;if(setting.inStockOnly&&setting.isDivideStock){var q=loadPlaneStock(),r=[],v={};c=$jscomp.makeIterator(c);for(var x=c.next();!x.done;v={$jscomp$loop$prop$o$323:v.$jscomp$loop$prop$o$323},x=c.next()){v.$jscomp$loop$prop$o$323=x.value;x=q.find(function(C){return function(F){return F.id===C.$jscomp$loop$prop$o$323.id}}(v));for(var w=0;10>=w;w++){var t=x.num[w];0!==t&&(t=Object.assign({remodel:w,count:t},v.$jscomp$loop$prop$o$323),t.antiAir=Plane.getBonusAntiAir(t.id,
t.type,t.remodel,t.antiAir),r.push(t))}}c=r.concat()}var y=$("#draggable_plane_sort_select").val();switch(y){case "battle_anti_air":c.sort(function(C,F){return F.antiAir+1.5*F.interception-(C.antiAir+1.5*C.interception)});break;case "defense_anti_air":c.sort(function(C,F){return F.antiAir+F.interception+2*F.antiBomber-(C.antiAir+C.interception+2*C.antiBomber)});break;case "land_base_power":c.sort(function(C,F){return getLandBasePower(F.id,18,0)-getLandBasePower(C.id,18,0)});break;case "radius":c.sort(function(C,
F){return F.radius-C.radius});break;case "history":if(setting.selectedHistory[0]){var z=setting.selectedHistory[0];c.sort(function(C,F){return z.filter(function(G){return G===F.id}).length-z.filter(function(G){return G===C.id}).length})}case "default":break;default:c.sort(function(C,F){return F[y]-C[y]})}q={};for(r=0;r<k;q={$jscomp$loop$prop$plane$143$326:q.$jscomp$loop$prop$plane$143$326},r++)if(q.$jscomp$loop$prop$plane$143$326=c[r],v=q.$jscomp$loop$prop$plane$143$326.antiAir+1.5*q.$jscomp$loop$prop$plane$143$326.interception,
x=q.$jscomp$loop$prop$plane$143$326.antiAir+q.$jscomp$loop$prop$plane$143$326.interception+2*q.$jscomp$loop$prop$plane$143$326.antiBomber,w=0<q.$jscomp$loop$prop$plane$143$326.antiBomber||0<q.$jscomp$loop$prop$plane$143$326.interception,!n||setting.favoritePlane.includes(q.$jscomp$loop$prop$plane$143$326.id)){if(f&&e)if(t=m.find(function(C){return function(F){return F.id===C.$jscomp$loop$prop$plane$143$326.id}}(q)),g){u=q.$jscomp$loop$prop$plane$143$326.count-(t?t.num[q.$jscomp$loop$prop$plane$143$326.remodel]:
0);if(1>u)continue;l=q.$jscomp$loop$prop$plane$143$326.remodel}else{var A=e.find(function(C){return function(F){return F.id===C.$jscomp$loop$prop$plane$143$326.id}}(q)),B=t?getArraySum(t.num):0;p=A?getArraySum(A.num):0;if(0>=p||!h&&B>=p)continue;u=p-B;for(p=10;0<=p;p--)if(t&&(A.num[p]-=t.num[p]),0<A.num[p]){l=p;break}}t=document.createElement("div");t.className="plane plane_tr_draggable d-flex py-2 py-lg-1";t.dataset.planeid=q.$jscomp$loop$prop$plane$143$326.id;t.dataset.type=q.$jscomp$loop$prop$plane$143$326.type;
t.dataset.abbr=q.$jscomp$loop$prop$plane$143$326.name;t.dataset.remodel=l;f&&h&&0>=u&&(t.classList.remove("plane_tr_draggable","plane"),t.classList.add("plane_tr_disabled"));A=document.createElement("div");A.className="align-self-center size-25";p=document.createElement("canvas");B=p.getContext("2d");p.width=25;p.height=25;B.drawImage(IMAGES["type"+q.$jscomp$loop$prop$plane$143$326.type],0,0,25,25);var D=document.createElement("div");D.className="pl-1 plane_td_name align-self-center";D.textContent=
...
0;2>=f;f++)document.getElementById("lb"+(f+1)+"_target_1").value=e[2][2*f],document.getElementById("lb"+(f+1)+"_target_2").value=e[2][2*f+1];5<=a.length?toggleDefenseMode(a[4]):toggleDefenseMode(!1);if(c){clearShipDivAll(6);var g=0,h=0;$(".ship_tab").each(function(x,w){var t=a[1].find(function(A){return A[2]===x});if(t){var y=$(w);setShipDiv(y,t[0]);y.find(".ship_plane").each(function(A,B){var D=$(B),E=t[1].find(function(C){return C[4]===A});E&&!D.hasClass("d-none")&&(setPlaneDiv(D,{id:E[0],prof:E[1],
remodel:E[2],slot:E[3]},!0),6<=E.length&&E[5]&&plane_unlock_Clicked(D.find(".plane_unlock")))});if(4<=t.length){var z=y.find(".ship_disabled");t[3]?(z.addClass("disabled"),z.children().addClass("fa-eye-slash").removeClass("fa-eye no_capture"),y.addClass("opacity4")):(z.removeClass("disabled"),z.children().removeClass("fa-eye-slash").addClass("fa-eye no_capture"),y.removeClass("opacity4"))}5<=t.length&&(t[4]?y.find(".ship_level").text(t[4]):y.find(".ship_level").text(99));6>x?g++:h++}});$("#friendFleet_item1").find(".display_ship_count").val(Math.max(g,
...
x});if(t){3<=t.length?$(w)[0].dataset.celldata=t[2]:$(w)[0].dataset.celldata="";if(4<=t.length){var y=castInt(t[3]);n?$(w).find(".cell_type").val([0,5].includes(y)?5:3):$(w).find(".cell_type").val(y);cell_type_Changed($(w).find(".cell_type"),!1)}else $(w).find(".cell_type").val(1);5<=t.length?$(w).find(".formation").val(t[4]):$(w).find(".formation").val(1);6<=t.length?$(w).find(".enemy_range").text(t[5]):$(w).find(".enemy_range").text(0);y=0;t=$jscomp.makeIterator(t[1]);for(var z=t.next();!z.done;z=
t.next()){z=z.value;var A=$(w).find(".enemy_content").eq(y++);0<z?setEnemyDiv(A,z):0>z?setEnemyDiv(A,-1,-z):clearEnemyDiv(A);A.addClass("d-flex").removeClass("d-none")}}});if(6<=a.length){var k=a[5];if(k&&3===k.length){var m=$("#air_raid_enemies"),l=castInt(k[1]);m.find(".cell_type").val([0,5].includes(l)?5:3);cell_type_Changed(m.find(".cell_type"),!1);m.find(".formation").val(k[2]);d=0;for(var u=$jscomp.makeIterator(k[0]),p=u.next();!p.done;p=u.next()){var q=p.value,r=m.find(".enemy_content").eq(d++);
...
0<u.id&&0<u.slot&&m++;l=h.getElementsByClassName("ap")[0];u=castInt(l.textContent);drawChangeValue(l,u,k.airPower);l=f.$jscomp$loop$prop$shipId$374?SHIP_DATA.find(function(E){return function(C){return C.id===E.$jscomp$loop$prop$shipId$374}}(f)):null;f.$jscomp$loop$prop$shipRange$375=3===c?3:l?l.range:0;if(c=h.getElementsByClassName("ship_range")[0])c.textContent=RANGES.find(function(E){return function(C){return C.id===E.$jscomp$loop$prop$shipRange$375}}(f)).name;if(0<m){p=document.createDocumentFragment();
u=document.createDocumentFragment();var x=!0;r=document.getElementById("empty_slot_invisible").checked;l&&!r&&(m=l.slot.length);c=(l?l.name:"\u672a\u6307\u5b9a")+"\uff1a";v=document.createElement("tr");q=document.createElement("tr");for(var w=0;w<k.planes.length;w++){var t=k.planes[w],y=t.name;if(l||0!==t.id&&0!==t.slot){if(l&&w===l.slot.length)break;c+=" "+(y?y+" "+getProfString(t.level):"\u672a\u88c5\u5099")+",";if(!l||!r||0!==t.id&&0!==t.slot){var z=getPlaneCss(t.type).replace("css","shoot_table");
if(x){var A=document.createElement("tr");A.className="slot"+w+" shipNo"+n;A.dataset.shipid=l?l.id:0;A.dataset.shipindex=a.ships.length-1;A.dataset.slotindex=w;A.dataset.css=z+"_hover";var B=document.createElement("td");B.rowSpan=m;B.className="td_name align-middle";B.textContent=l?l.name:"\u672a\u6307\u5b9a";A.appendChild(B);B=document.createElement("td");B.className="pl-1 td_plane_name align-middle "+z;B.textContent=y;A.appendChild(B);v.appendChild(B.cloneNode(!1));for(B=1;10>=B;B++){var D=document.createElement("td");
D.className="td_battle battle"+B+" align-middle";A.appendChild(D);v.appendChild(D.cloneNode(!1))}B=document.createElement("td");B.className="td_battle battle_end align-middle";A.appendChild(B);v.appendChild(B.cloneNode(!1));B=document.createElement("td");B.className="td_battle battle_death align-middle";A.appendChild(B);v.appendChild(B.cloneNode(!1));p.appendChild(A)}else A=v.cloneNode(!0),A.className="slot"+w+" shipNo"+n,A.dataset.shipid=l?l.id:0,A.dataset.shipindex=a.ships.length-1,A.dataset.slotindex=
w,A.dataset.css=z+"_hover",B=A.getElementsByClassName("td_plane_name")[0],B.className="pl-1 td_plane_name align-middle "+z,B.textContent=y,p.appendChild(A);y+=0!==t.remodel?"\u2605"+t.remodel:"";x?(x=document.createElement("tr"),x.className="ship_info_tr slot"+(w+1),A=document.createElement("td"),A.rowSpan=m,A.className="info_name",A.textContent=l?l.name:"\u672a\u6307\u5b9a",x.appendChild(A),A=document.createElement("td"),A.className="info_plane "+z,A.innerHTML=y+" "+getProfString(t.level),x.appendChild(A),
q.appendChild(A.cloneNode(!1)),z=document.createElement("td"),z.className="info_slot",z.textContent=t.slot,x.appendChild(z),q.appendChild(z.cloneNode(!1)),z=document.createElement("td"),z.className="info_ap",z.textContent=t.airPower,x.appendChild(z),q.appendChild(z.cloneNode(!1)),t=document.createElement("td"),t.rowSpan=m,t.className="info_sumAp",t.textContent=k.airPower,x.appendChild(t),u.appendChild(x)):(x=q.cloneNode(!0),x.className="ship_info_tr slot"+(w+1),A=x.getElementsByClassName("info_plane")[0],
A.className="info_plane "+z,A.innerHTML=y+" "+getProfString(t.level),x.getElementsByClassName("info_slot")[0].textContent=t.slot,x.getElementsByClassName("info_ap")[0].textContent=t.airPower,u.appendChild(x));x=!1}}}document.getElementById("shoot_down_table_tbody").appendChild(p);n=document.getElementById("ship_info_table").getElementsByTagName("tbody")[0];n.appendChild(u);n=n.getElementsByTagName("tr");if(0<n.length)for(n=$jscomp.makeIterator(n[n.length-1].getElementsByTagName("td")),m=n.next();!m.done;m=
...
H.id||0<H.slots.length});a=a<d.length?a:d.length-1;for(d=0;d<p;d++)n.push(0),k.push(0);for(d=0;d<f;d++)for(w=0;w<c.ships[d].planes.length;w++)m.push(Array(e));for(d=0;d<g[a].enemies.length;d++)if(w=g[a].enemies[d],!(w.isSpR||0>w.id)){t=0===l.length?0:l[l.length-1][0]+1;for(var y=0;y<w.slots.length;y++)l.push([t,y,0,0,999]);x.push(0)}d=g[0].cellType===CELL_TYPE.highAirRaid?b.defense2AirPower:b.defenseAirPower;resultData.defenseAirPower=d;for(w=0;w<e;w++){for(t=0;t<p;t++){var z=g[t];y=z.enemies;calculateLandBase(b,
z,t);q+=doJetPhase(c,z);var A=isDefMode?d:z.cellType!==CELL_TYPE.grand?c.airPower:c.unionAirPower,B=getEnemyFleetAP(y),D=getAirStatusIndex(A,B);t===a&&h[D]++;n[t]+=A;k[t]+=B;shootDownFriend(D,c,z,t);if(t===a){shootDownEnemy(D,y,v,r);for(var E=D=A=z=0;E<y.length;E++){var C=y[E];if(!(C.isSpR||0>C.id)){for(var F=C.slots,G=C.attackers.concat(),I=0;I<F.length;I++){var J=F[I],K=l[z++];K[2]+=J;K[3]<J&&(K[3]=J);K[4]>J&&(K[4]=J);0===J&&(G[I]=!1)}0>G.indexOf(!0)&&x[A]++;A++;D+=C.ap}}u[0].push(B);u[1].push(D)}y=
$jscomp.makeIterator(y);for(B=y.next();!B.done;B=y.next())B=B.value,B.slots=B.orgSlots.concat(),B.ap=B.origAp,B.lbAp=B.origLbAp}for(t=0;3>t;t++)if(y=b.landBases[t],0!==y.mode){for(B=0;B<y.planes.length;B++)z=y.planes[B],z.slot=z.fullSlot,z.airPower=z.fullAirPower;y.airPower=y.fullAirPower}for(y=t=0;y<f;y++){B=c.ships[y];z=B.planes.length;for(A=B.airPower=0;A<z;A++)D=B.planes[A],m[t++][w]=D.slot,D.slot=D.fullSlot,D.airPower=D.fullAirPower,B.airPower+=D.airPower;B.airPower=B.fullAirPower}c.airPower=
c.fullAirPower;c.escortAirPower=c.fullEscortAirPower;c.unionAirPower=c.fullUnionAirPower}for(r=g=0;r<f;r++)for(v=c.ships[r],d=0;d<v.planes.length;d++)w=m[g],t={shipNo:v.fleetNo,slot:v.planes[d].slotNo,max:getArrayMax(w),min:getArrayMin(w),avg:w.reduce(function(H,L){return H+L})/e,death:w.filter(function(H){return 0===H}).length,deathRate:0},t.deathRate=t.death/w.length,resultData.fleetSlots.push(t),g++;for(f=0;f<p;f++)for(m=Math.round(n[f]/e),g=Math.round(k[f]/e),r=getAirStatusIndex(m,g),$("#shoot_down_table").find(".fap.battle"+
(f+1)).text(m),$("#shoot_down_table").find(".eap.battle"+(f+1)).text(g),$("#shoot_down_table").find(".cond.battle"+(f+1)).text(AIR_STATUS[r].abbr).addClass("airStatus"+r),m=$jscomp.makeIterator(c.ships),g=m.next();!g.done;g=m.next())for(g=g.value,r=$jscomp.makeIterator(g.planes),v=r.next();!v.done;v=r.next())v=v.value,v.id&&(d=v.results[f],$("#shoot_down_table .shipNo"+g.fleetNo+".slot"+v.slotNo+" .battle"+(f+1)).text(Math.round(d/e)));$("#shoot_down_table").find("td").removeClass("d-none");for(c=
battleCount+1;10>=c;c++)$("#shoot_down_table").find(".battle"+c).addClass("d-none");isDefMode||(resultData.fleetAirPower=Math.round(n[a]/e));c=[];n=$jscomp.makeIterator(b.landBases);for(p=n.next();!p.done;p=n.next())for(p=p.value,f=0;2>f;f++)c.push(p.results[displayBattle-1].airStatusIndex[f].map(function(H){return H/e}));b=$jscomp.makeIterator(b.landBases);for(p=b.next();!p.done;p=b.next())n=p.value,resultData.landBaseAirPowers.push(Math.round(n.results[a].mainAirPower[0]/e)),resultData.landBaseAirPowers.push(Math.round(n.results[a].mainAirPower[1]/
e)),resultData.enemyAirPowers.push(Math.round(n.results[a].enemyAirPower[0]/e)),resultData.enemyAirPowers.push(Math.round(n.results[a].enemyAirPower[1]/e));resultData.enemyAirPowers.push(Math.round(k[a]/e));resultData.landBaseAirStatus=c;resultData.fleetAirStatus=h.map(function(H){return H/e});resultData.enemySlots=l;resultData.enemyAirPowerResults=u;resultData.enemySlotAllDead=x;resultData.usedSteels=q;return null}
...
k={};c=$jscomp.makeIterator(c.landBases[a].planes);for(d=c.next();!d.done;k={$jscomp$loop$prop$pl$387:k.$jscomp$loop$prop$pl$387},d=c.next())k.$jscomp$loop$prop$pl$387=d.value,d=PLANE_DATA.find(function(D){return function(E){return E.id===D.$jscomp$loop$prop$pl$387.id}}(k)),e+='\n\t\t<div class="row mx-1 py-0_5 '+(b===n?"selected":"")+" "+(d?"general_tr btn_show_detail":"")+'" data-slot_no="'+n+'">\n\t\t\t<div class="col-1 align-self-center font_size_11 text-left">'+(n+1)+'</div>\n\t\t\t<div class="col d-flex align-self-center font_size_12 pl-0">\n\t\t\t\t<img src="../img/type/'+
...
$("#detail_info").html(e)}
function enemySlotDetailCalculate(a,b){setPreCalculateTable();var c=new LandBaseAll,d=new Fleet,e=[];updateLandBaseInfo(c,!1);updateFriendFleetInfo(d,!1);updateEnemyFleetInfo(e,!1);var f=castInt($("#calculate_count").val()),g=isDefMode?0:displayBattle-1;g=g<e.length?g:e.length-1;var h=[],n=e[g].enemies.filter(function(z){return 0<z.slots.length&&!z.isSpR})[a],k=ENEMY_DATA.find(function(z){return z.id===n.id}),m=ENEMY_PLANE_DATA.find(function(z){return z.id===k.eqp[b]}),l=2===Math.abs(m.type)?[.8,
1.5]:1===Math.abs(m.type)?[0]:[1],u=2===Math.abs(m.type)?m.torpedo:m.bomber;m=$("#slot_detail").prop("checked");for(var p=fleetStage2Table.concat(),q=!document.getElementById("ignore_stage2").checked&&0<p.length,r=$jscomp.makeIterator(e),v=r.next();!v.done;v=r.next())v=v.value,v.enemies=v.enemies.filter(function(z){return 0<z.slots.length});for(r=0;r<f;r++){for(v=0;v<=g;v++){var x=e[v],w=x.enemies;calculateLandBase(c,x,v);doJetPhase(d,x);var t=x.cellType;t=isDefMode?defAp:t!==CELL_TYPE.grand?d.airPower:
...
d.ships[v],x=w.planes.length,t=w.airPower=0;t<x;t++)y=w.planes[t],y.slot=y.fullSlot,y.updateAirPower(),w.airPower+=y.airPower;d.updateAirPower()}updateDetailChart(h,m?"\u6b8b\u6a5f\u6570 [\u6a5f]":"\u822a\u7a7a\u6226\u706b\u529b",{mode:"index",callbacks:{title:function(z,A){var B=z[0].xLabel;if($("#slot_detail").prop("checked")){if(!B)return"\u6b8b\u6a5f\u6570\uff1a"+B+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a0";var D=2===l.length?Math.floor(l[1]*(u*Math.sqrt(B)+25)):0;return"\u6b8b\u6a5f\u6570\uff1a"+
B+" \u6a5f\n\u822a\u7a7a\u6226\u706b\u529b\uff1a"+(Math.floor(l[0]*(u*Math.sqrt(B)+25))+(D?" or "+D:""))}return"\u822a\u7a7a\u6226\u706b\u529b\uff1a"+B},label:function(z,A){return[z.yLabel+" %"]}}});$("#land_base_detail_table").addClass("d-none");$(".detail_fire").removeClass("d-none").addClass("d-flex");$(".detail_wave").addClass("d-none").removeClass("d-flex");$("#detail_info").data("mode","enemy_slot_detail");$("#detail_info").data("base_no",a);$("#detail_info").data("slot_no",b);$("#detail_info").data("battle_no",
g);$("#detail_warning").text("\u203b \u822a\u7a7a\u6226\u706b\u529b\u306f\u30ad\u30e3\u30c3\u30d7\u524d\u3067\u30af\u30ea\u30c6\u30a3\u30ab\u30eb\u3001\u89e6\u63a5\u88dc\u6b63\u306a\u3057\u5c0f\u6570\u5207\u6368\u3066");c="";d=0;e={};f=$jscomp.makeIterator(k.eqp);for(g=f.next();!g.done;e={$jscomp$loop$prop$id$389:e.$jscomp$loop$prop$id$389},g=f.next())e.$jscomp$loop$prop$id$389=g.value,g=ENEMY_PLANE_DATA.find(function(z){return function(A){return A.id===z.$jscomp$loop$prop$id$389}}(e)),c+='\n\t\t<div class="row mx-1 py-0_5 '+
...
function autoExpandNormal(){var a=loadPlaneStock(),b=$("#allow_fighter").prop("checked"),c=$("#auto_land_base_1").prop("checked"),d=$("#auto_land_base_2").prop("checked"),e=$("#auto_land_base_3").prop("checked"),f=$("#auto_fleet").prop("checked");c||$("#lb_item1").find(".lb_plane").each(function(x,w){var t=a.find(function(z){return z.id===castInt($(w)[0].dataset.planeid)}),y=castInt($(w).find(".remodel_value").text());t&&t.num[y]--});d||$("#lb_item2").find(".lb_plane").each(function(x,w){var t=a.find(function(z){return z.id===
castInt($(w)[0].dataset.planeid)}),y=castInt($(w).find(".remodel_value").text());t&&t.num[y]--});e||$("#lb_item3").find(".lb_plane").each(function(x,w){var t=a.find(function(z){return z.id===castInt($(w)[0].dataset.planeid)}),y=castInt($(w).find(".remodel_value").text());t&&t.num[y]--});f&&autoFleetExpand(a);$(".ship_tab").find(".ship_plane").each(function(x,w){var t=a.find(function(z){return z.id===castInt($(w)[0].dataset.planeid)}),y=castInt($(w).find(".remodel_value").text());t&&t.num[y]--});if(c||
...
w?e[w].sort(function(t,y){return y.antiAir-t.antiAir}):"type2"===w?e[w].sort(function(t,y){return t.torpedo===y.torpedo?y.antiAir-t.antiAir:y.torpedo-t.torpedo}):["type3","type6"].includes(w)&&e[w].sort(function(t,y){return t.avoid===y.avoid?y.bomber-t.bomber:y.avoid-t.avoid})});var l=[],u=0;c.each(function(w,t){var y=SHIP_DATA.find(function(A){return A.id===castInt($(t)[0].dataset.shipid)});if(y){var z=[];$(t).find(".ship_plane:not(.d-none)").each(function(A,B){var D=castInt($(B).find(".slot").text()),
E=$(B).find(".plane_unlock").hasClass("d-none"),C={shipId:y.id,shipType:y.type,slotNo:A,num:D,plane:new ShipPlane(0),order:u++,num_:D,slotId:"ship"+w+"_slot"+A,isLock:E,lockedPlane:null};if(E){var F=PLANE_DATA.find(function(I){return I.id===castInt($(B)[0].dataset.planeid)});if(F){var G=castInt($(B).find(".remodel_value").text());C.lockedPlane={id:F.id,remodel:G};Object.keys(e).forEach(function(I){var J=e[I].findIndex(function(K){return K.id===F.id&&K.remodel===G});-1<J&&e[I].splice(J,1)})}}else 0===
D&&(C.isLock=!0);[1,2,3].includes(y.type)?10>D&&(C.num_=D-100):C.num_=D-100;z.push(C)});l=l.concat(z)}});g=[2,6];f=0;l.sort(function(w,t){return t.num_-w.num_});h=$jscomp.makeIterator(l);for(a=h.next();!a.done;a=h.next())for(n=a.value,m=!1,k=$jscomp.makeIterator(g),a=k.next();!a.done;a=k.next()){a=a.value;if(n.isLock){n.lockedPlane&&(a=getShipPlaneObject(n.num,n.lockedPlane),n.plane=a,f+=a.airPower);m=!0;break}var p=n.shipType;d&&[1,2,3].includes(p)&&1===n.slotNo&&(a=3);p={};var q=$jscomp.makeIterator(e["type"+
...
t){return w.order-t.order});u=0;c.each(function(w,t){$(t).find(".ship_plane:not(.d-none)").each(function(y,z){var A=l.find(function(B){return B.slotId==="ship"+w+"_slot"+y});A?(setPlaneDiv($(z),A.plane),u++):setPlaneDiv($(z))})})}
