		...
		id: "1.12.0", changes: [
			{
				...
				content: "これに伴い、他サイト様からデッキビルダー形式等で編成を取り込んだ場合でも、情報を抜き落とさずに編成を復元できるようになりました。所持数の登録も普通の艦載機と同様に可能です。"
			},
			{
				type: 0, title: "補強増設枠を追加しました。",
				content: "配備した艦は例外なく補強増設枠が用意される石油王プレイ対応です。ここに艦載機、水上機は基本搭載できないため、制空値には関与しませんが、対空砲火を正しく計算したい場合は実際の編成と同じように装備を搭載してください。"
			},
			{
				type: 0, title: "味方の対空砲火情報の設定方法を変更しました。",
				content: "艦隊入力欄で設定した装備や編成から、加重対空値、艦隊防空値、撃墜性能を自動で算出するようになりました。今後、対空砲火情報を更新する際は、加重対空値や艦隊防空値の入力は不要になりますが、艦娘欄は正しく装備を配置するようにしてください。"
			},
			{
				type: 1, title: "装備一覧画面を更新しました。",
				content: "全ての装備に対応しました。艦娘の装備選択時は、実際に搭載できる装備のみ表示するようになっています。一部レイアウトの調整を行いました。"
			},
			{
				type: 1, title: "艦娘一覧画面を更新しました。",
				content: "全ての艦娘に対応しました。艦種選択の方法を変更し、タブ式に変更しました。既存の一覧構築処理を見直し、高速化を行いました。一部レイアウトの調整を行いました。"
			},
			{
				type: 1, title: "「入力情報」欄を廃止しました。",
				content: "各装備の制空値の内訳、防空時の制空値等が一通り各入力欄内で表示できるようになったことから、本機能を廃止しました。"
			},
			{
				type: 2, title: "不具合の修正を行いました。",
				content: "空きスロが存在する基地の詳細計算時の挙動を修正しました。"
			}
		],
	}
];

const LAST_UPDATE_DATE = "2021/1/20";
