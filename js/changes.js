...
let CHANGE_LOG = [
	...
	},
	{
		id: "1.9.0", changes: [
			...
			{
				type: 0, title: "敵艦載機に対する対空砲火計算処理が可能になりました。",
				content: "上記「対空砲火設定」を行うと、計算結果欄の敵艦隊の搭載数の数値が、対空砲火計算処理を行った後の数値に置き換わります。計算結果欄の「対空砲火無視」にチェックを入れると、対空砲火計算を無視し、従来通りの表示も可能です。"
			},
			{
				type: 0, title: "敵艦の棒立ち率の計算が可能になりました。",
				content: "上記「対空砲火設定」を行うと、敵の「攻撃機が全て0機になる確率」、つまり棒立ち率を算出します。なお厳密には敵艦が空母系でない場合、普通に砲撃してくるので棒立ちにはなりません。"
			},
			{
				type: 0, title: "所持装備を、ゲーム同様個別表示できるようになりました。",
				content: "これにより、所持している同じ装備で改修値の違う装備を自由に配備できるようになります。また、対空値ソート時は改修による効果も反映されてソートされます。"
			},
			{
...
const LAST_UPDATE_DATE = "2020/08/03";
