...
x.value;if(5<g.childNodes.length)break;0>=x[0]||(r=document.createElement("img"),r.className="ship_image",r.width=100,r.height=25,r.src="../img/ship/"+x[0]+".png",g.appendChild(r))}q.appendChild(g);g=createDiv("preset_memo_view border mt-1 px-1 py-1 "+(m[3]?"":"d-none"),"",m[3].replace(/\r?\n/g," "));q.appendChild(g);g=document.createElement("textarea");g.className="form-control preset_memo mt-1 w-100 d-none";g.value=m[3]?m[3]:"";u=g.value.match(/\n/g);g.rows=u&&u.length?u.length+2:2;q.appendChild(g);
w.appendChild(q);q=createDiv("d-flex ml-auto pt-2 btns_container");g=createDiv("ml-auto r_btn btn_commit d-none");g.dataset.toggle="tooltip";g.dataset.offset="-50%";g.title="\u7de8\u96c6\u5185\u5bb9\u3092\u78ba\u5b9a\u3057\u307e\u3059\u3002";g.innerHTML='<i class="fas fa-save"></i>';q.appendChild(g);g=createDiv("ml-2 r_btn btn_rollback d-none");g.dataset.toggle="tooltip";g.dataset.offset="-50%";g.title="\u7de8\u96c6\u5185\u5bb9\u3092\u53d6\u308a\u6d88\u3057\u307e\u3059\u3002";g.innerHTML='<i class="fas fa-undo"></i>';
...
g.innerHTML='<i class="fas fa-copy"></i>';q.appendChild(g);g=createDiv("ml-2 r_btn btn_delete");g.dataset.toggle="tooltip";g.dataset.offset="-50%";g.title="\u7de8\u6210\u3092\u524a\u9664\u3057\u307e\u3059\u3002";g.innerHTML='<i class="fas fa-trash-o"></i>';q.appendChild(g);b&&w.appendChild(q);p.appendChild(w);b?(m=createDiv("update_date mr-1","","\u66f4\u65b0\u65e5\u6642: "+m[4].split(".")[0]),p.appendChild(m)):(m=createDiv("update_date mr-3","",(m[5]?"\u4f5c\u6210\u8005: "+m[5]+"\u3000":"")+"\u4f5c\u6210\u65e5\u6642: "+
m[4].split(".")[0]),p.appendChild(m));n.appendChild(p);k.appendChild(n)}}h.appendChild(k);!b&&prevSnapShot&&(f=createDiv("d-flex preset_container more_load"),k=createDiv("mx-auto my-auto","","\u3082\u3063\u3068\u8aad\u307f\u8fbc\u3080"),f.appendChild(k),h.appendChild(f));document.getElementById(c).innerHTML="";document.getElementById(c).appendChild(h);$("#"+c+" .r_btn").tooltip();0===d&&!e&&b&&Sortable.create(document.getElementById("preset_sortable_container"),{animation:200,handle:".sortable_handle",
onEnd:function(){for(var v=loadLocalStorage("presets"),t=[],z={},A=$jscomp.makeIterator(document.getElementsByClassName("preset_container")),y=A.next();!y.done;z={$jscomp$loop$prop$c$11:z.$jscomp$loop$prop$c$11},y=A.next())z.$jscomp$loop$prop$c$11=y.value,(y=v.find(function(B){return function(C){return C[0]===B.$jscomp$loop$prop$c$11.dataset.presetid}}(z)))&&t.push(y);t.length&&(saveLocalStorage("presets",t),inform_success("\u7de8\u6210\u30c7\u30fc\u30bf\u306e\u9806\u5e8f\u304c\u66f4\u65b0\u3055\u308c\u307e\u3057\u305f\u3002"))}})}
...
d.id}))?d=b:a.presets.push(d);a.activeId=d.id;saveLocalStorage("activePresets",a);window.location.href="../simulator/?p="+d.history.histories[d.history.index]}function public_preset_Clicked(a){var b=a[0].dataset.presetdata;a=a.find(".preset_name").text();window.location.href="../simulator/?d="+b+"&name="+encodeURIComponent(a)}
