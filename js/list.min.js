...
B.$jscomp$loop$prop$c$11.dataset.presetid}}(z)))&&t.push(y);t.length&&(saveLocalStorage("presets",t),inform_success("\u7de8\u6210\u30c7\u30fc\u30bf\u306e\u9806\u5e8f\u304c\u66f4\u65b0\u3055\u308c\u307e\u3057\u305f\u3002"))}})}function AutoLink(a){return a.replace(/((h?)(ttps?:\/\/[a-zA-Z0-9.\-_@:/~?%&;=+#',()*!]+))/g,function(b,c,d,e){return'<a href="h'+e+'" target="_blank">'+c+"</a>"})}
...
function searchUploadedPreset(){var a=getPublicPresetsQuery(),b=[];fb||initializeFB();if(fb){var c=null;c=a.map?fb.collection("presets").where("map","==",a.map).where("level","==",a.level).orderBy(a.sortKey,a.order).limit(STEP):fb.collection("presets").where("level","==",a.level).orderBy(a.sortKey,a.order).limit(STEP);c.get().then(function(d){prevSnapShot=d.docs[d.docs.length-1];d.forEach(function(e){var f=e.data();e=e.data().createdAt?e.data().createdAt.toDate():new Date;f=["",f.name,f.data,f.memo,
formatDate(e,"yyyy/MM/dd HH:mm"),f.user];b.push(f)});if(b.length<STEP||!a.map)prevSnapShot=null;setPresets(b,!1);saveSessionStorage("search_result",{map:a.map,level:a.level,sortKey:a.sortKey,order:a.order,presets:b})})["catch"](function(d){console.error(d);inform_danger("\u7de8\u6210\u30c7\u30fc\u30bf\u306e\u8aad\u307f\u8fbc\u307f\u306b\u5931\u6557\u3057\u307e\u3057\u305f\u3002")})}}
...
b={},c=$jscomp.makeIterator(WORLD_DATA),d=c.next();!d.done;b={$jscomp$loop$prop$world$13:b.$jscomp$loop$prop$world$13},d=c.next()){d=d.value;b.$jscomp$loop$prop$world$13=d.world;var e=MAP_DATA.filter(function(f){return function(h){return Math.floor(h.area/10)===f.$jscomp$loop$prop$world$13}}(b));a+='<optgroup label="'+d.name+'">';d=$jscomp.makeIterator(e);for(e=d.next();!e.done;e=d.next())e=e.value,a+='<option value="'+e.area+'">'+(20<b.$jscomp$loop$prop$world$13?"E":b.$jscomp$loop$prop$world$13)+
