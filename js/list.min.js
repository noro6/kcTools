...
function searchUploadedPreset(){var a=getPublicPresetsQuery(),b=[];fb||initializeFB();fb&&fb.collection("presets").where("map","==",a.map).where("level","==",a.level).orderBy(a.sortKey,a.order).limit(STEP).get().then(function(c){prevSnapShot=c.docs[c.docs.length-1];c.forEach(function(d){var e=d.data();d=d.data().createdAt?d.data().createdAt.toDate():new Date;e=["",e.name,e.data,e.memo,formatDate(d,"yyyy/MM/dd HH:mm"),e.user];b.push(e)});b.length<STEP&&(prevSnapShot=null);setPresets(b,!1);saveSessionStorage("search_result",
